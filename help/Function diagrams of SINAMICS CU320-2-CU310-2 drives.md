---
title: "Function diagrams of SINAMICS CU320-2/CU310-2 drives"
package: StdrFupOverviewS120enUS
topics: 5
source: Siemens TIA Portal Information System (offline help, en-US)
---


# Function diagrams of SINAMICS CU320-2/CU310-2 drives

This section contains information on the following topics:

- [Function Diagrams for SINAMICS S120 - Overview](#function-diagrams-for-sinamics-s120---overview)
- [Function Diagrams for SINAMICS S150 - Overview](#function-diagrams-for-sinamics-s150---overview)
- [Function Diagrams for SINAMICS G130 - Overview](#function-diagrams-for-sinamics-g130---overview)
- [Function Diagrams for SINAMICS G150 - Overview](#function-diagrams-for-sinamics-g150---overview)

## Function Diagrams for SINAMICS S120 - Overview

The structure of the drive functions and the relationship of the individual parameters with other parameters is displayed in the function diagrams:

Explanations of the function diagrams - Explanation of the symbols (Part 1) - 1020 -
  

Explanations of the function diagrams - Explanation of the symbols (Part 2) - 1021 -
  

Explanations of the function diagrams - Explanation of the symbols (Part 3) - 1022 -
  

Explanations of the function diagrams - Handling BICO technology - 1030 -
  

CU310-2 input/output terminals - Overview - 2019 -
  

CU310-2 input/output terminals - Electrically isolated digital inputs (DI 0 ... DI 3, DI 22) - 2020 -
  

CU310-2 input/output terminals - Electrically isolated digital inputs (DI 16 ... DI 21) - 2021 -
  

CU310-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 8 ... DI/DO 9) - 2030 -
  

CU310-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 10 ... DI/DO 11) - 2031 -
  

CU310-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 12 ... DI/DO 13) - 2032 -
  

CU310-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 14 ... DI/DO 15) - 2033 -
  

CU310-2 input/output terminals - Digital output (DO 16) - 2038 -
  

CU310-2 input/output terminals - Analog input (AI 0) - 2040 -
  

CU320-2 input/output terminals - Overview - 2119 -
  

CU320-2 input/output terminals - Electrically isolated digital inputs (DI 0 ... DI 3, DI 16, DI 17) - 2120 -
  

CU320-2 input/output terminals - Electrically isolated digital inputs (DI 4 ... DI 7, DI 20, DI 21) - 2121 -
  

CU320-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 8 ... DI/DO 9) - 2130 -
  

CU320-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 10 ... DI/DO 11) - 2131 -
  

CU320-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 12 ... DI/DO 13) - 2132 -
  

CU320-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 14 ... DI/DO 15) - 2133 -
  

D455-2 input/output terminals - Overview - 2139 -
  

D455-2 input/output terminals - Electrically isolated digital inputs (DI 0 ... DI 3, DI 16, DI 17) - 2140 -
  

D455-2 input/output terminals - Electrically isolated digital inputs (DI 4 ... DI 7, DI 20, DI 21) - 2141 -
  

D455-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 8 ... DI/DO 9) - 2150 -
  

D455-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 10 ... DI/DO 11) - 2151 -
  

D455-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 12 ... DI/DO 13) - 2152 -
  

D455-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 14 ... DI/DO 15) - 2153 -
  

CX32-2 input/output terminals - Overview - 2179 -
  

CX32-2 input/output terminals - Electrically isolated digital inputs (DI 0 ... DI 3, DI 16, DI 17) - 2180 -
  

CX32-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 8 ... DI/DO 9) - 2190 -
  

CX32-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 10 ... DI/DO 11) - 2191 -
  

Control Unit communication - CU_CX32/CU_LINK - 2194 -
  

Control Unit communication - CU_LINK/CU internal - 2195 -
  

Control Unit communication - SINAMICS Link overview (r0108.31 = 1, p8835 = 3) - 2197 -
  

Control Unit communication - SINAMICS Link configuration (r0108.31 = 1, p8835 = 3) - 2198 -
  

Control Unit communication - SINAMICS Link receive data (r0108.31 = 1, p8835 = 3) - 2199 -
  

Control Unit communication - SINAMICS Link send data (r0108.31 = 1, p8835 = 3) - 2200 -
  

S120M input/output terminals - Digital inputs/outputs bidirectional (DI/DO 0 ... DI/DO 1) - 2201 -
  

PROFIenergy - Control commands / interrogation commands - 2381 -
  

PROFIenergy - States - 2382 -
  

PROFIdrive - Overview - 2401 -
  

PROFIdrive - PROFIBUS (PB) / PROFINET (PN), addresses and diagnostics - 2410 -
  

PROFIdrive - Standard telegrams and process data 1 - 2415 -
  

PROFIdrive - Standard telegrams and process data 2 - 2416 -
  

PROFIdrive - Manufacturer-specific telegrams and process data 1 - 2419 -
  

PROFIdrive - Manufacturer-specific telegrams and process data 2 - 2420 -
  

PROFIdrive - Manufacturer-specific telegrams and process data 3 - 2421 -
  

PROFIdrive - Manufacturer-specific telegrams and process data 4 - 2422 -
  

PROFIdrive - Supplementary telegrams/free telegrams and process data - 2423 -
  

PROFIdrive - STW1_BM control word metal industry interconnection - 2425 -
  

PROFIdrive - STW2_BM control word metal industry interconnection - 2426 -
  

PROFIdrive - E_STW1_BM control word infeed metal industry interconnection - 2427 -
  

PROFIdrive - ZSW1_BM status word metal industry interconnection - 2428 -
  

PROFIdrive - ZSW2_BM status word metal industry interconnection - 2429 -
  

PROFIdrive - E_ZSW1_BM status word infeed metal industry interconnection - 2430 -
  

PROFIdrive - STW2_ENC control word ENCODER interconnection - 2433 -
  

PROFIdrive - ZSW2_ENC status word ENCODER interconnection - 2434 -
  

PROFIdrive - PZD receive signals profile-specific interconnection - 2439 -
  

PROFIdrive - PZD receive signals manufacturer-specific interconnection - 2440 -
  

PROFIdrive - STW1 control word interconnection (p2038 = 2) - 2441 -
  

PROFIdrive - STW1 control word interconnection (p2038 = 0) - 2442 -
  

PROFIdrive - STW1 control word interconnection (p2038 = 1) - 2443 -
  

PROFIdrive - STW2 control word interconnection (p2038 = 0) - 2444 -
  

PROFIdrive - STW2 control word interconnection (p2038 = 1) - 2445 -
  

PROFIdrive - E_STW1 control word infeed interconnection - 2447 -
  

PROFIdrive - STW7 control word interconnection - 2448 -
  

PROFIdrive - PZD send signals profile-specific interconnection - 2449 -
  

PROFIdrive - PZD send signals manufacturer-specific interconnection - 2450 -
  

PROFIdrive - ZSW1 status word interconnection (p2038 = 2) - 2451 -
  

PROFIdrive - ZSW1 status word interconnection (p2038 = 0) - 2452 -
  

PROFIdrive - ZSW1 status word interconnection (p2038 = 1) - 2453 -
  

PROFIdrive - ZSW2 status word interconnection (p2038 = 0) - 2454 -
  

PROFIdrive - ZSW2 status word interconnection (p2038 = 1) - 2455 -
  

PROFIdrive - MELDW status word interconnection - 2456 -
  

PROFIdrive - E_ZSW1 status word infeed interconnection - 2457 -
  

PROFIdrive - ZSW7 status word interconnection - 2458 -
  

PROFIdrive - POS_STW positioning control word interconnection (r0108.4 = 1) - 2462 -
  

PROFIdrive - POS_STW1 positioning control word 1 interconnection (r0108.4 = 1) - 2463 -
  

PROFIdrive - POS_STW2 positioning control word 2 interconnection (r0108.4 = 1) - 2464 -
  

PROFIdrive - POS_ZSW1 positioning status word 1 interconnection (r0108.4 = 1) - 2466 -
  

PROFIdrive - POS_ZSW2 positioning status word 2 interconnection (r0108.4 = 1) - 2467 -
  

PROFIdrive - IF1 receive telegram free interconnection via BICO (p0922 = 999) - 2468 -
  

PROFIdrive - IF1 send telegram free interconnection via BICO (p0922 = 999) - 2470 -
  

PROFIdrive - IF1 status words free interconnection - 2472 -
  

PROFIdrive - STW1 control word 1 interconnection (r0108.4 = 1) - 2475 -
  

PROFIdrive - SATZANW block selection interconnection (r0108.4 = 1) - 2476 -
  

PROFIdrive - ZSW1 status word 1 interconnection (r0108.4 = 1) - 2479 -
  

PROFIdrive - MDI_MOD MDI mode interconnection (r0108.4 = 1) - 2480 -
  

PROFIdrive - IF1 receive telegram free interconnection via BICO (p0922 = 999) - 2481 -
  

PROFIdrive - IF1 send telegram free interconnection via BICO (p0922 = 999) - 2483 -
  

PROFIdrive - IF2 receive telegram free interconnection - 2485 -
  

PROFIdrive - IF2 send telegram free interconnection - 2487 -
  

PROFIdrive - IF2 status words free interconnection - 2489 -
  

PROFIdrive - IF2 receive telegram free interconnection - 2491 -
  

PROFIdrive - IF2 send telegram free interconnection - 2493 -
  

PROFIdrive - CU_STW1 control word 1 Control Unit interconnection - 2495 -
  

PROFIdrive - CU_ZSW1 status word 1 Control Unit interconnection - 2496 -
  

PROFIdrive - O_DIGITAL interconnection - 2497 -
  

PROFIdrive - I_DIGITAL interconnection - 2498 -
  

PROFIdrive - O_DIGITAL_1 interconnection - 2499 -
  

PROFIdrive - I_DIGITAL_1 interconnection - 2500 -
  

Internal control/status words - Control word sequence control - 2501 -
  

Internal control/status words - Status word sequence control - 2503 -
  

Internal control/status words - Control word setpoint channel - 2505 -
  

Internal control/status words - Control word speed controller - 2520 -
  

Internal control/status words - Status word speed controller - 2522 -
  

Internal control/status words - Status word closed-loop control - 2526 -
  

Internal control/status words - Status word current control - 2530 -
  

Internal control/status words - Status word monitoring functions 1 - 2534 -
  

Internal control/status words - Status word monitoring functions 2 - 2536 -
  

Internal control/status words - Status word monitoring functions 3 - 2537 -
  

Internal control/status words - Control word faults/alarms - 2546 -
  

Internal control/status words - Status word faults/alarms 1 and 2 - 2548 -
  

Sequence control - Sequencer - 2610 -
  

Sequence control - Missing enables, line contactor control, logic operation - 2634 -
  

Brake control - Simple brake control (r0108.14 = 0) - 2701 -
  

Brake control - Extended brake control, standstill detection (r0108.14 = 1) - 2704 -
  

Brake control - Extended brake control, open/close brake (r0108.14 = 1) - 2707 -
  

Brake control - Extended brake control, signal outputs (r0108.14 = 1) - 2711 -
  

SI Basic Functions - Parameter manager - 2800 -
  

SI Basic Functions - Monitoring functions and faults/alarms - 2802 -
  

SI Basic Functions - SI status CU, MM, CU + MM, group STO - 2804 -
  

SI Basic Functions - S_STW1/2 safety control word 1/2, S_ZSW1/2 safety status word 1/2 - 2806 -
  

SI Basic Functions - STO (Safe Torque Off), SS1 (Safe Stop 1) - 2810 -
  

SI Basic Functions - STO (Safe Torque Off), safe pulse suppression - 2811 -
  

SI Basic Functions - SBC (Safe Brake Control), SBA (Safe Brake Adapter) - 2814 -
  

SI Extended Functions - Parameter manager - 2818 -
  

SI Extended Functions - SS1, SS2, SOS, internal STOP B, C, D, F - 2819 -
  

SI Extended Functions - SLS (Safely-Limited Speed) - 2820 -
  

SI Extended Functions - Safe referencing - 2821 -
  

SI Advanced Functions - SLP (Safely-Limited Position) - 2822 -
  

SI Extended Functions - SSM (Safe Speed Monitor) - 2823 -
  

SI Extended Functions - SDI (Safe Direction) - 2824 -
  

SI Extended Functions - SAM (Safe Acceleration Monitor), SBR (Safe Brake Ramp) - 2825 -
  

SI Advanced Functions - SCA (Safe Cam) - 2826 -
  

SI Extended Functions - SBT (Safe Brake Test) - 2836 -
  

SI Extended Functions - Selection of the active control word - 2837 -
  

SI Extended Functions - SLA (Safely-Limited Acceleration) - 2838 -
  

SI Extended Functions - SI Motion drive-integrated control signals / status signals - 2840 -
  

SI Extended Functions - S_STW1 safety control word 1, S_ZSW1 safety status word 1 - 2842 -
  

SI Extended Functions - S_STW2 safety control word 2, S_ZSW2 safety status word 2 - 2843 -
  

SI Advanced Functions - S_ZSW_CAM1 Safety status word Safe Cam 1 - 2844 -
  

SI Extended Functions - Control via PROFIsafe (p9601.2 = p9601.3 = 1) - 2858 -
  

SI Extended Functions - CU310-2 (F-DI 0 ... F-DI 2) - 2870 -
  

SI Extended Functions - CU310-2 failsafe digital output (F-DO 0) - 2873 -
  

SI Extended Functions - CU310-2 control interface - 2875 -
  

SI Extended Functions - CU310-2 Safe State selection - 2876 -
  

SI Extended Functions - CU310-2 assignment (F-DO 0) - 2877 -
  

SI TM54F - Overview - 2890 -
  

SI TM54F - Parameter manager - 2891 -
  

SI TM54F - Configuration, F-DI/F-DO test - 2892 -
  

SI TM54F - Failsafe digital inputs (F-DI 0 ... F-DI 4) - 2893 -
  

SI TM54F - Failsafe digital inputs (F-DI 5 ... F-DI 9) - 2894 -
  

SI TM54F - Failsafe digital outputs (F-DO 0 ... 3), digital inputs (DI 20 ... 23) - 2895 -
  

SI TM54F - Basic Functions control interface (p9601.2/3 = 0 & p9601.6 = 1) - 2900 -
  

SI TM54F - Basic Functions Safe State selection - 2901 -
  

SI TM54F - Basic Functions assignment (F-DO 0 ... F-DO 3) - 2902 -
  

SI TM54F - Extended Functions control interface (p9601.2 = 1 & p9601.3 = 0) - 2905 -
  

SI TM54F - Extended Functions Safe State selection - 2906 -
  

SI TM54F - Extended Functions assignment (F-DO 0 ... F-DO 3) - 2907 -
  

SI PROFIsafe - Standard telegrams - 2915 -
  

SI PROFIsafe - Manufacturer-specific telegrams - 2917 -
  

Setpoint channel - Overview - 3001 -
  

Setpoint channel - Fixed speed setpoints - 3010 -
  

Setpoint channel - Motorized potentiometer - 3020 -
  

Setpoint channel - Main setpoint / additional setpoint, setpoint scaling, jog - 3030 -
  

Setpoint channel - Direction limitation and direction reversal - 3040 -
  

Setpoint channel - Skip frequency bands and speed limits - 3050 -
  

Setpoint channel - Basic ramp-function generator - 3060 -
  

Setpoint channel - Extended ramp-function generator - 3070 -
  

Setpoint channel - Ramp-function generator selection, -status word, -tracking - 3080 -
  

Setpoint channel - Extended Stop and Retract (ESR, r0108.9 = 1) - 3082 -
  

Setpoint channel - Dynamic Servo Control (DSC) linear and DSC spline (r0108.6 = 1) - 3090 -
  

Setpoint channel not activated - Generation of the speed limits (r0108.8 = 0) - 3095 -
  

EPOS - Jog mode (r0108.4 = 1) - 3610 -
  

EPOS - Referencing / reference point approach mode (r0108.4 = 1) (p2597 = 0-signal) - 3612 -
  

EPOS - Flying referencing mode (r0108.4 = 1) (p2597 = 1 signal) - 3614 -
  

EPOS - Traversing blocks mode external block change (r0108.4 = 1) - 3615 -
  

EPOS - Traversing blocks mode (r0108.4 = 1) - 3616 -
  

EPOS - Travel to fixed stop (r0108.4 = 1) - 3617 -
  

EPOS - Direct setpoint input/MDI mode, dynamic values (r0108.4 = 1) - 3618 -
  

EPOS - Direct setpoint input/MDI mode (r0108.4 = 1) - 3620 -
  

EPOS - Mode control (r0108.4 = 1) - 3625 -
  

EPOS - Traversing range limits (r0108.4 = 1) - 3630 -
  

EPOS - Interpolator (r0108.4 = 1) - 3635 -
  

EPOS - Control word block selection/MDI selection (r0108.4 = 1) - 3640 -
  

EPOS - Status word 1 (r0108.3 = 1, r0108.4 = 1) - 3645 -
  

EPOS - Status word 2 (r0108.3 = 1, r0108.4 = 1) - 3646 -
  

EPOS - Status word active traversing block/MDI active (r0108.4 = 1) - 3650 -
  

Position control - Position actual value processing (r0108.3 = 1) - 4010 -
  

Position control - Position controller (r0108.3 = 1) - 4015 -
  

Position control - Standstill monitoring / position monitoring (r0108.3 = 1) - 4020 -
  

Position control - Dynamic following error monitoring, cam sequencer (r0108.3 = 1) - 4025 -
  

Encoder evaluation - Servo control, overview - 4700 -
  

Encoder evaluation - Vector control, overview - 4702 -
  

Encoder evaluation - Position and temperature sensing encoder 1 ... 3 - 4704 -
  

Encoder evaluation - Speed actual value and pole position sensing encoder 1 - 4710 -
  

Encoder evaluation - Speed actual value acquisition encoder 2, 3 (r0108.7 = 1, APC activated) - 4711 -
  

Encoder evaluation - Speed actual value and pole position sensing encoder 1, n_act_filter 5 - 4715 -
  

Encoder evaluation - Encoder interface, receive signals encoder 1 … 3 - 4720 -
  

Encoder evaluation - Encoder interface, send signals encoder 1 … 3 - 4730 -
  

Encoder evaluation - Reference mark search with external zero mark encoder 1 ... 3 - 4735 -
  

Encoder evaluation - Measuring probe evaluation, measured value memory encoder 1 ... 3 - 4740 -
  

Encoder evaluation - Absolute value for incremental encoder - 4750 -
  

Hydraulic drive - Velocity controller - 4965 -
  

Hydraulic drive - Knee compensation - 4966 -
  

Hydraulic drive - Force controller - 4970 -
  

Hydraulic drive - Valve characteristic, area adaptation - 4975 -
  

Hydraulic drive - Static friction compensation with force controller (p1400.2 = 1) - 4977 -
  

Hydraulic drive - Static friction compensation with voltage pulse / voltage ramp - 4978 -
  

Hydraulic drive - Sequencer - 4985 -
  

Hydraulic drive - P24 management with shutoff valve - 4990 -
  

Hydraulic drive - P24 management without shutoff valve - 4991 -
  

Servo control - Closed-loop speed control and U/f control, overview - 5019 -
  

Servo control - Speed setpoint filter and speed precontrol - 5020 -
  

Servo control - Reference model / precontrol balancing / speed limitation - 5030 -
  

Servo control - Moment of inertia estimator (r0108.10 = 1, r0108.30 = 1) - 5035 -
  

Servo control - Speed controller with encoder - 5040 -
  

Servo control - Speed controller, torque-speed precontrol with encoder (p1402.4 = 1) - 5042 -
  

Servo control - Online tuning with activated moment of inertia estimator (r0108.10 = 1) - 5045 -
  

Servo control - Speed controller adaptation (Kp_n/Tn_n adaptation) - 5050 -
  

Servo control - Controller parameter adaptation (r0171.29 = 1) - 5055 -
  

Servo control - Torque setpoint, control mode switchover - 5060 -
  

Servo control - Speed controller without encoder - 5210 -
  

Servo control - U/f control for diagnostics - 5300 -
  

Servo control - Variable message function 1, 2, 3 - 5301 -
  

Servo control - Closed-loop speed control configuration - 5490 -
  

Servo control - Generation of the torque limits, overview - 5609 -
  

Servo control - Torque limiting/reduction, interpolator - 5610 -
  

Servo control - Motoring/generating torque limit - 5620 -
  

Servo control - Upper/lower torque limit - 5630 -
  

Servo control - Mode switchover, power/current limitation - 5640 -
  

Servo control - Vdc_max controller and Vdc_min controller - 5650 -
  

Servo control - Current control, overview - 5700 -
  

Servo control - Current setpoint filters 1 … 4 - 5710 -
  

Servo control - Current setpoint filters 5 … 10 (r0108.21 = 1) - 5711 -
  

Servo control - Iq and Id controllers - 5714 -
  

Servo control - Field current / flux specification, flux reduction, flux controller - 5722 -
  

Servo control - Interface to the Motor Module (control signals, actual current values) - 5730 -
  

Vector control - Closed-loop speed control and generation of the torque limits, overview - 6020 -
  

Vector control - Speed setpoint, droop - 6030 -
  

Vector control - Pre-control symmetrization reference/acceleration model - 6031 -
  

Vector control - Moment of inertia estimator (r0108.10 = 1) - 6035 -
  

Vector control - Speed controller with/without encoder - 6040 -
  

Vector control - Speed controller adaptation (Kp_n/Tn_n adaptation) - 6050 -
  

Vector control - Torque setpoint - 6060 -
  

Vector control - Vdc_max controller and Vdc_min controller - 6220 -
  

Vector control - U/f control, overview - 6300 -
  

Vector control - U/f characteristic and voltage boost - 6301 -
  

Vector control - Resonance damping and slip compensation - 6310 -
  

Vector control - Vdc_max controller and Vdc_min controller (U/f) - 6320 -
  

Vector control - Closed-loop speed control configuration - 6490 -
  

Vector control - Flux control configuration - 6491 -
  

Vector control - Excitation (SESM, p0300 = 5) - 6495 -
  

Vector control - Upper/lower torque limit - 6630 -
  

Vector control - Current/power/torque limits - 6640 -
  

Vector control - Current control, overview - 6700 -
  

Vector control - Current setpoint filter - 6710 -
  

Vector control - Iq and Id controllers - 6714 -
  

Vector control - Id setpoint (PMSM, p0300 = 2) - 6721 -
  

Vector control - Field weakening characteristic, Id setpoint (ASM, p0300 = 1) - 6722 -
  

Vector control - Field weakening controller, flux controller (ASM, p0300 = 1) - 6723 -
  

Vector control - Field weakening controller (PMSM, p0300 = 2) - 6724 -
  

Vector control - Flux setpoint, field weakening controller (SESM, p0300 = 5) - 6725 -
  

Vector control - Field weakening controller, flux controller (SESM, p0300 = 5) - 6726 -
  

Vector control - Current model, excitation current monitoring, cos phi (SESM, p0300 = 5) - 6727 -
  

Vector control - Interface to the Motor Module (ASM, p0300 = 1) - 6730 -
  

Vector control - Interface to the Motor Module (PMSM, p0300 = 2) - 6731 -
  

Vector control - Interface to the Motor Module (SESM, p0300 = 5) - 6732 -
  

Vector control - Motor model selection (SESM and p1300 = 20, p0300 = 5) - 6733 -
  

Vector control - Flux setpoint (RESM, p0300 = 6) - 6790 -
  

Vector control - Id setpoint (RESM, p0300 = 6) - 6791 -
  

Vector control - Interface to the Motor Module (RESM, p0300 = 6) - 6792 -
  

Vector control - Display signals - 6799 -
  

Technology functions - kT estimator - 7008 -
  

Technology functions - Friction characteristic - 7010 -
  

Technology functions - Advanced Positioning Control (APC, r0108.7 = 1) - 7012 -
  

Technology functions - APC differential position gain (APC, r0108.7 = 1) - 7013 -
  

Technology functions - External armature short-circuit (EASC, p0300 = 2xx or 4xx) - 7014 -
  

Technology functions - Internal armature short-circuit (IASC, p0300 = 2xx or 4xx) - 7016 -
  

Technology functions - DC braking (p0300 = 1xx) - 7017 -
  

Technology functions - Synchronizing - 7020 -
  

Technology functions - Essential service mode (ESM) - 7033 -
  

General - 7200 -
  

Logic function blocks - AND (AND function block with 4 inputs) - 7210 -
  

Logic function blocks - OR (OR function block with 4 inputs) - 7212 -
  

Logic function blocks - XOR (XOR function block with 4 inputs) - 7214 -
  

Logic function blocks - NOT (inverter) - 7216 -
  

Arithmetic function blocks - ADD (adder with 4 inputs), SUB (subtractor) - 7220 -
  

Arithmetic function blocks - MUL (multiplier), DIV (divider) - 7222 -
  

Arithmetic function blocks - AVA (absolute value generator) - 7224 -
  

Timing function blocks - MFP (pulse generator), PCL (pulse contractor) - 7230 -
  

Timing function blocks - PDE (ON delay device), PDF (OFF delay device) - 7232 -
  

Timing function blocks - PST (pulse stretcher) - 7234 -
  

Memory function blocks - RSR (RS flip-flop), DFR (D flip-flop) timing function blocks - PST (pulse stretcher) - 7240 -
  

Switch function blocks - BSW (binary switch), NSW (numeric switch) - 7250 -
  

Control function blocks - LIM (limiter) - 7260 -
  

Control function blocks - PT1 (smoothing element) - 7262 -
  

Control function blocks - INT (integrator), DIF (derivative-action element) - 7264 -
  

Complex function blocks - LVM (double-sided limit monitor with hysteresis) - 7270 -
  

Technology controller - Fixed values, binary selection (r0108.16 = 1 and p2216 = 2) - 7950 -
  

Technology controller - Fixed values, direct selection (r0108.16 = 1 and p2216 = 1) - 7951 -
  

Technology controller - Motorized potentiometer (r0108.16 = 1) - 7954 -
  

Technology controller - Closed-loop control (r0108.16 = 1) - 7958 -
  

Technology controller - Kp/Tn adaptation (r0108.16 = 1) - 7959 -
  

Technology controller - DC link voltage controller (r0108.16 = 1) - 7960 -
  

Grid droop control (r0108.12 = 1) - Grid droop, voltage adjustment - 7982 -
  

Grid droop control (r0108.12 = 1) - DC component control, harmonic control - 7983 -
  

Line droop control (r0108.12 = 1) - Modulation depth control - 7984 -
  

Line droop control (r0108.12 = 1) - Sequence control overcurrent - 7986 -
  

Line transformer (r0108.4 = 1) - DC component control, negative phase sequence system controller - 7987 -
  

Grid transformer (r0108.4 = 1) - Island grid black start sequence control - 7988 -
  

Grid transformer (r0108.4 = 1) - Island grid synchronization sequence control - 7989
  

Line transformer (r0108.4 = 1) - Transformer model (p5480 = 1) - 7990 -
  

Line transformer (r0108.4 = 1) - Line filter monitoring - 7991 -
  

Line transformer (r0108.4 = 1) - PLL 2 (phase-locked loop 2) - 7992 -
  

Line transformer (r0108.4 = 1) - Transformer magnetization voltage threshold - 7993 -
  

Line transformer (r0108.4 = 1) - Transformer magnetization sequence control - 7994 -
  

Grid transformer (r0108.4 = 1) - Island grid synchronization voltage threshold - 7995
  

Dynamic grid support (r0108.7 = 1) - Characteristic - 7996 -
  

Dynamic grid support (r0108.7 = 1) - Current limits (p5501 = 1) - 7997 -
  

Dynamic grid support (r0108.7 = 1) - Sequence control - 7998 -
  

Dynamic grid support (r0108.7 = 1) - Grid monitoring island grid detection - 7999 -
  

Signals and monitoring functions - Overview - 8005 -
  

Signals and monitoring functions - Speed messages 1 - 8010 -
  

Signals and monitoring functions - Speed messages 2 - 8011 -
  

Signals and monitoring functions - Torque messages, motor blocked/stalled - 8012 -
  

Signals and monitoring functions - Load monitoring (r0108.17 = 1) - 8013 -
  

Signals and monitoring functions - Motor temperature status word faults/alarms - 8015 -
  

Signals and monitoring functions - Thermal monitoring motor - 8016 -
  

Signals and monitoring functions - Motor temperature model 1 (I2t) - 8017 -
  

Signals and monitoring functions - Motor temperature model 2 - 8018 -
  

Signals and monitoring functions - Motor temperature model 3 - 8019 -
  

Signals and monitoring functions - Separately excited synchronous motor (SESM, p0300 = 5) - 8020 -
  

Signals and monitoring functions - Thermal monitoring power unit - 8021 -
  

Signals and monitoring functions - Freely parameterized I2t monitoring (SESM) - 8022 -
  

Diagnostics - Overview - 8050 -
  

Diagnostics - Fault buffer - 8060 -
  

Diagnostics - Alarm buffer - 8065 -
  

Diagnostics - Faults/alarms trigger word (r2129) - 8070 -
  

Diagnostics - Faults/alarms configuration - 8075 -
  

Diagnostics - Measuring sockets (T0, T1, T2) - 8134 -
  

Diagnostics - Recorder overview (r0108.5 = 1) - 8144 -
  

Diagnostics - Recorder sequence control (r0108.5 = 1) - 8145 -
  

Data sets - Command data sets (CDS) - 8560 -
  

Data sets - Drive data sets (DDS) - 8565 -
  

Data sets - Encoder data sets (EDS) - 8570 -
  

Data sets - Motor data sets (MDS) - 8575 -
  

Data sets - Power unit data sets (PDS) - 8580 -
  

Basic Infeed - Overview - 8710 -
  

Basic Infeed - Control word sequence control infeed - 8720 -
  

Basic Infeed - Status word sequence control infeed - 8726 -
  

Basic Infeed - Sequencer - 8732 -
  

Active Infeed - Missing enables, line contactor control - 8738 -
  

Basic Infeed - Interface to the Basic Infeed power unit (control signals, actual values) - 8750 -
  

Basic Infeed - Signals and monitoring functions - 8760 -
  

Smart Infeed - Overview - 8810 -
  

Smart Infeed - Control word sequence control infeed - 8820 -
  

Smart Infeed - Status word sequence control infeed - 8826 -
  

Smart Infeed - Status word infeed - 8828 -
  

Smart Infeed - Sequencer - 8832 -
  

Smart Infeed - Missing enables, line contactor control - 8838 -
  

Smart Infeed - Interface to the Smart Infeed (control signals, actual values) - 8850 -
  

Smart Infeed - Signals and monitoring functions, line voltage monitoring - 8860 -
  

Smart Infeed - Signals and monitoring functions, line frequency and Vdc monitoring - 8864 -
  

Active Infeed - Overview - 8910 -
  

Active Infeed - Control word sequence control infeed - 8920 -
  

Active Infeed - Status word sequence control infeed - 8926 -
  

Active Infeed - Status word infeed - 8928 -
  

Active Infeed - Sequencer - 8932 -
  

Active Infeed - Missing enables, line contactor control - 8938 -
  

Active Infeed - Controller modulation depth reserve / controller DC link voltage (p3400.0 = 0) - 8940 -
  

Active Infeed - Reactive current / apparent current limits (r0108.3 = 1) - 8945 -
  

Active Infeed - Current precontrol / current controller / gating unit (p3400.0 = 0) - 8946 -
  

Active Infeed - Master/slave (r0108.19 = 1) - 8948 -
  

Active Infeed - Interface to the Active Infeed, control signals, actual values (p3400.0 = 0) - 8950 -
  

Active Infeed - Cos phi display (r0108.10 = 1) - 8951 -
  

Active Infeed - Signals and monitoring functions, line voltage monit (p3400.0 = 0) - 8960 -
  

Active Infeed - Signals and monitoring functions, line frequency/Vdc monit (p3400.0 = 0) - 8964 -
  

Terminal Board 30 (TB30) - Overview - 9099 -
  

Terminal Board 30 (TB30) - Electrically isolated digital inputs (DI 0 ... DI 3) - 9100 -
  

Terminal Board 30 (TB30) - Electrically isolated digital outputs (DO 0 ... DO 3) - 9102 -
  

Terminal Board 30 (TB30) - Analog inputs (AI 0 ... AI 1) - 9104 -
  

Terminal Board 30 (TB30) - Analog outputs (AO 0 ... AO 1) - 9106 -
  

Communication Board CAN10 (CBC10) - Receive telegram free PDO mapping (p8744 = 2) - 9204 -
  

Communication Board CAN10 (CBC10) - Receive telegram predef conn set (p8744 = 1) - 9206 -
  

Communication Board CAN10 (CBC10) - Send telegram free PDO mapping (p8744 = 2) - 9208 -
  

Communication Board CAN10 (CBC10) - Send telegram predef conn set (p8744 = 1) - 9210 -
  

Communication Board CAN10 (CBC10) - Control word CANopen - 9220 -
  

Communication Board CAN10 (CBC10) - Status word CANopen - 9226 -
  

Terminal Module 15 (TM15) - Overview TM15 (SIMOTION) - 9389 -
  

Terminal Module 15 (TM15) - Overview TM15DI_DO (SINAMICS) - 9399 -
  

Terminal Module 15 (TM15) - Digital inputs/outputs bidirectional (DI/DO 0 ... DI/DO 7) - 9400 -
  

Terminal Module 15 (TM15) - Digital inputs/outputs bidirectional (DI/DO 8 ... DI/DO 15) - 9401 -
  

Terminal Module 15 (TM15) - Digital inputs/outputs bidirectional (DI/DO 16 ... DI/DO 23) - 9402 -
  

Terminal Module 17 High Feature (TM17 High Feature) - Overview - 9419 -
  

Terminal Module 31 (TM31) - Overview - 9549 -
  

Terminal Module 31 (TM31) - Electrically isolated digital inputs (DI 0 ... DI 3) - 9550 -
  

Terminal Module 31 (TM31) - Electrically isolated digital inputs (DI 4 ... DI 7) - 9552 -
  

Terminal Module 31 (TM31) - Electrically isolated digital relay outputs (DO 0 ... DO 1) - 9556 -
  

Terminal Module 31 (TM31) - Digital inputs/outputs, bidirectional (DI/DO 8 ... DI/DO 9) - 9560 -
  

Terminal Module 31 (TM31) - Digital inputs/outputs, bidirectional (DI/DO 10 ... DI/DO 11) - 9562 -
  

Terminal Module 31 (TM31) - Analog input 0 (AI 0) - 9566 -
  

Terminal Module 31 (TM31) - Analog input 1 (AI 1) - 9568 -
  

Terminal Module 31 (TM31) - Analog outputs (AO 0 ... AO 1) - 9572 -
  

Terminal Module 31 (TM31) - Temperature evaluation - 9576 -
  

Terminal Module 31 (TM31) - Sensor monitoring KTY/PTC/PT1000 - 9577 -
  

Terminal Module 120 (TM120) - Temperature evaluation channels 0 and 1 - 9605 -
  

Terminal Module 120 (TM120) - Temperature evaluation channels 2 and 3 - 9606 -
  

Terminal Module 150 (TM150) - Temperature evaluation structure (channel 0 … 11) - 9625 -
  

Terminal Module 150 (TM150) - Temperature evaluation 1x2-, 3-, 4-wire (channels 0 … 5) - 9626 -
  

Terminal Module 150 (TM150) - Temperature evaluation 2x2-wire (channels 0 … 11) - 9627 -
  

Terminal Module 41 (TM41) - Overview - 9659 -
  

Terminal Module 41 (TM41) - Electrically isolated digital inputs (DI 0 ... DI 3) - 9660 -
  

Terminal Module 41 (TM41) - Digital inputs/outputs, bidirectional (DI/DO 0 ... DI/DO 1) - 9661 -
  

Terminal Module 41 (TM41) - Digital inputs/outputs, bidirectional (DI/DO 2 ... DI/DO 3) - 9662 -
  

Terminal Module 41 (TM41) - Analog input 0 (AI 0) - 9663 -
  

Terminal Module 41 (TM41) - Incremental encoder emulation (p4400 = 0) - 9674 -
  

Terminal Module 41 (TM41) - Incremental encoder emulation (p4400 = 1) - 9676 -
  

Terminal Module 41 (TM41) - STW1 control word interconnection (p0922 = 3) - 9677 -
  

Terminal Module 41 (TM41) - Control word sequence control (p4400 = 0) - 9678 -
  

Terminal Module 41 (TM41) - STW2 control word interconnection (p0922 = 3) - 9679 -
  

Terminal Module 41 (TM41) - Status word sequence control - 9680 -
  

Terminal Module 41 (TM41) - ZSW1 status word interconnection (p0922 = 3) - 9681 -
  

Terminal Module 41 (TM41) - Sequencer (p4400 = 0) - 9682 -
  

Terminal Module 41 (TM41) - ZSW2 status word interconnection (p0922 = 3) - 9683 -
  

Auxiliaries - Cooling unit control and feedback signals (r0108.28 = 1) - 9794 -
  

Auxiliaries - Cooling unit sequence control (r0108.28 = 1) - 9795 -
  

Auxiliaries - Chassis power units 3-phase AC line connection contactor status display - 9814 -
  

Voltage Sensing Module (VSM) - Analog inputs (AI 0 ... AI 3) - 9880 -
  

Voltage Sensing Module (VSM) - Temperature evaluation - 9886 -
  

Basic Operator Panel 20 (BOP20) - Control word interconnection - 9912 -
  

External Braking Module - Sequencer (r0108.26 = 1) - 9951 -

## Function Diagrams for SINAMICS S150 - Overview

The structure of the drive functions and the relationship of the individual parameters with other parameters is displayed in the function diagrams:

Explanations of the function diagrams - Explanation of the symbols (Part 1) - 1020 -
  

Explanations of the function diagrams - Explanation of the symbols (Part 2) - 1021 -
  

Explanations of the function diagrams - Explanation of the symbols (Part 3) - 1022 -
  

Explanations of the function diagrams - Handling BICO technology - 1030 -
  

CU310-2 input/output terminals - Overview - 2019 -
  

CU310-2 input/output terminals - Electrically isolated digital inputs (DI 0 ... DI 3, DI 22) - 2020 -
  

CU310-2 input/output terminals - Electrically isolated digital inputs (DI 16 ... DI 21) - 2021 -
  

CU310-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 8 ... DI/DO 9) - 2030 -
  

CU310-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 10 ... DI/DO 11) - 2031 -
  

CU310-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 12 ... DI/DO 13) - 2032 -
  

CU310-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 14 ... DI/DO 15) - 2033 -
  

CU310-2 input/output terminals - Digital output (DO 16) - 2038 -
  

CU310-2 input/output terminals - Analog input (AI 0) - 2040 -
  

CU320-2 input/output terminals - Overview - 2119 -
  

CU320-2 input/output terminals - Electrically isolated digital inputs (DI 0 ... DI 3, DI 16, DI 17) - 2120 -
  

CU320-2 input/output terminals - Electrically isolated digital inputs (DI 4 ... DI 7, DI 20, DI 21) - 2121 -
  

CU320-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 8 ... DI/DO 9) - 2130 -
  

CU320-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 10 ... DI/DO 11) - 2131 -
  

CU320-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 12 ... DI/DO 13) - 2132 -
  

CU320-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 14 ... DI/DO 15) - 2133 -
  

D455-2 input/output terminals - Overview - 2139 -
  

D455-2 input/output terminals - Electrically isolated digital inputs (DI 0 ... DI 3, DI 16, DI 17) - 2140 -
  

D455-2 input/output terminals - Electrically isolated digital inputs (DI 4 ... DI 7, DI 20, DI 21) - 2141 -
  

D455-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 8 ... DI/DO 9) - 2150 -
  

D455-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 10 ... DI/DO 11) - 2151 -
  

D455-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 12 ... DI/DO 13) - 2152 -
  

D455-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 14 ... DI/DO 15) - 2153 -
  

CX32-2 input/output terminals - Overview - 2179 -
  

CX32-2 input/output terminals - Electrically isolated digital inputs (DI 0 ... DI 3, DI 16, DI 17) - 2180 -
  

CX32-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 8 ... DI/DO 9) - 2190 -
  

CX32-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 10 ... DI/DO 11) - 2191 -
  

Control Unit communication - CU_CX32/CU_LINK - 2194 -
  

Control Unit communication - CU_LINK/CU internal - 2195 -
  

Control Unit communication - SINAMICS Link overview (r0108.31 = 1, p8835 = 3) - 2197 -
  

Control Unit communication - SINAMICS Link configuration (r0108.31 = 1, p8835 = 3) - 2198 -
  

Control Unit communication - SINAMICS Link receive data (r0108.31 = 1, p8835 = 3) - 2199 -
  

Control Unit communication - SINAMICS Link send data (r0108.31 = 1, p8835 = 3) - 2200 -
  

S120M input/output terminals - Digital inputs/outputs bidirectional (DI/DO 0 ... DI/DO 1) - 2201 -
  

PROFIenergy - Control commands / interrogation commands - 2381 -
  

PROFIenergy - States - 2382 -
  

PROFIdrive - Overview - 2401 -
  

PROFIdrive - PROFIBUS (PB) / PROFINET (PN), addresses and diagnostics - 2410 -
  

PROFIdrive - Standard telegrams and process data 1 - 2415 -
  

PROFIdrive - Standard telegrams and process data 2 - 2416 -
  

PROFIdrive - Manufacturer-specific telegrams and process data 1 - 2419 -
  

PROFIdrive - Manufacturer-specific telegrams and process data 3 - 2421 -
  

PROFIdrive - Manufacturer-specific telegrams and process data 4 - 2422 -
  

PROFIdrive - Supplementary telegrams/free telegrams and process data - 2423 -
  

PROFIdrive - STW1_BM control word metal industry interconnection - 2425 -
  

PROFIdrive - STW2_BM control word metal industry interconnection - 2426 -
  

PROFIdrive - E_STW1_BM control word infeed metal industry interconnection - 2427 -
  

PROFIdrive - ZSW1_BM status word metal industry interconnection - 2428 -
  

PROFIdrive - ZSW2_BM status word metal industry interconnection - 2429 -
  

PROFIdrive - E_ZSW1_BM status word infeed metal industry interconnection - 2430 -
  

PROFIdrive - STW2_ENC control word ENCODER interconnection - 2433 -
  

PROFIdrive - ZSW2_ENC status word ENCODER interconnection - 2434 -
  

PROFIdrive - PZD receive signals profile-specific interconnection - 2439 -
  

PROFIdrive - PZD receive signals manufacturer-specific interconnection - 2440 -
  

PROFIdrive - STW1 control word interconnection (p2038 = 2) - 2441 -
  

PROFIdrive - STW1 control word interconnection (p2038 = 0) - 2442 -
  

PROFIdrive - STW1 control word interconnection (p2038 = 1) - 2443 -
  

PROFIdrive - STW2 control word interconnection (p2038 = 0) - 2444 -
  

PROFIdrive - STW2 control word interconnection (p2038 = 1) - 2445 -
  

PROFIdrive - E_STW1 control word infeed interconnection - 2447 -
  

PROFIdrive - PZD send signals profile-specific interconnection - 2449 -
  

PROFIdrive - PZD send signals manufacturer-specific interconnection - 2450 -
  

PROFIdrive - ZSW1 status word interconnection (p2038 = 2) - 2451 -
  

PROFIdrive - ZSW1 status word interconnection (p2038 = 0) - 2452 -
  

PROFIdrive - ZSW2 status word interconnection (p2038 = 0) - 2454 -
  

PROFIdrive - E_ZSW1 status word infeed interconnection - 2457 -
  

PROFIdrive - POS_STW positioning control word interconnection (r0108.4 = 1) - 2462 -
  

PROFIdrive - POS_STW1 positioning control word 1 interconnection (r0108.4 = 1) - 2463 -
  

PROFIdrive - POS_STW2 positioning control word 2 interconnection (r0108.4 = 1) - 2464 -
  

PROFIdrive - POS_ZSW1 positioning status word 1 interconnection (r0108.4 = 1) - 2466 -
  

PROFIdrive - POS_ZSW2 positioning status word 2 interconnection (r0108.4 = 1) - 2467 -
  

PROFIdrive - IF1 receive telegram free interconnection via BICO (p0922 = 999) - 2468 -
  

PROFIdrive - IF1 send telegram free interconnection via BICO (p0922 = 999) - 2470 -
  

PROFIdrive - IF1 status words free interconnection - 2472 -
  

PROFIdrive - STW1 control word 1 interconnection (r0108.4 = 1) - 2475 -
  

PROFIdrive - SATZANW block selection interconnection (r0108.4 = 1) - 2476 -
  

PROFIdrive - ZSW1 status word 1 interconnection (r0108.4 = 1) - 2479 -
  

PROFIdrive - MDI_MOD MDI mode interconnection (r0108.4 = 1) - 2480 -
  

PROFIdrive - IF1 receive telegram free interconnection via BICO (p0922 = 999) - 2481 -
  

PROFIdrive - IF1 send telegram free interconnection via BICO (p0922 = 999) - 2483 -
  

PROFIdrive - IF2 receive telegram free interconnection - 2485 -
  

PROFIdrive - IF2 send telegram free interconnection - 2487 -
  

PROFIdrive - IF2 status words free interconnection - 2489 -
  

PROFIdrive - IF2 receive telegram free interconnection - 2491 -
  

PROFIdrive - IF2 send telegram free interconnection - 2493 -
  

PROFIdrive - CU_STW1 control word 1 Control Unit interconnection - 2495 -
  

PROFIdrive - CU_ZSW1 status word 1 Control Unit interconnection - 2496 -
  

PROFIdrive - O_DIGITAL interconnection - 2497 -
  

PROFIdrive - I_DIGITAL interconnection - 2498 -
  

PROFIdrive - O_DIGITAL_1 interconnection - 2499 -
  

PROFIdrive - I_DIGITAL_1 interconnection - 2500 -
  

Internal control/status words - Control word sequence control - 2501 -
  

Internal control/status words - Status word sequence control - 2503 -
  

Internal control/status words - Control word setpoint channel - 2505 -
  

Internal control/status words - Control word speed controller - 2520 -
  

Internal control/status words - Status word speed controller - 2522 -
  

Internal control/status words - Status word closed-loop control - 2526 -
  

Internal control/status words - Status word current control - 2530 -
  

Internal control/status words - Status word monitoring functions 1 - 2534 -
  

Internal control/status words - Status word monitoring functions 2 - 2536 -
  

Internal control/status words - Status word monitoring functions 3 - 2537 -
  

Internal control/status words - Control word faults/alarms - 2546 -
  

Internal control/status words - Status word faults/alarms 1 and 2 - 2548 -
  

Sequence control - Sequencer - 2610 -
  

Sequence control - Missing enables, line contactor control, logic operation - 2634 -
  

Brake control - Simple brake control (r0108.14 = 0) - 2701 -
  

Brake control - Extended brake control, standstill detection (r0108.14 = 1) - 2704 -
  

Brake control - Extended brake control, open/close brake (r0108.14 = 1) - 2707 -
  

Brake control - Extended brake control, signal outputs (r0108.14 = 1) - 2711 -
  

SI Basic Functions - Parameter manager - 2800 -
  

SI Basic Functions - Monitoring functions and faults/alarms - 2802 -
  

SI Basic Functions - SI status CU, MM, CU + MM, group STO - 2804 -
  

SI Basic Functions - S_STW1/2 safety control word 1/2, S_ZSW1/2 safety status word 1/2 - 2806 -
  

SI Basic Functions - STO (Safe Torque Off), SS1 (Safe Stop 1) - 2810 -
  

SI Basic Functions - STO (Safe Torque Off), safe pulse suppression - 2811 -
  

SI Basic Functions - SBC (Safe Brake Control), SBA (Safe Brake Adapter) - 2814 -
  

SI Extended Functions - Parameter manager - 2818 -
  

SI Extended Functions - SS1, SS2, SOS, internal STOP B, C, D, F - 2819 -
  

SI Extended Functions - SLS (Safely-Limited Speed) - 2820 -
  

SI Extended Functions - Safe referencing - 2821 -
  

SI Advanced Functions - SLP (Safely-Limited Position) - 2822 -
  

SI Extended Functions - SSM (Safe Speed Monitor) - 2823 -
  

SI Extended Functions - SDI (Safe Direction) - 2824 -
  

SI Extended Functions - SAM (Safe Acceleration Monitor), SBR (Safe Brake Ramp) - 2825 -
  

SI Advanced Functions - SCA (Safe Cam) - 2826 -
  

SI Extended Functions - SBT (Safe Brake Test) - 2836 -
  

SI Extended Functions - Selection of the active control word - 2837 -
  

SI Extended Functions - SLA (Safely-Limited Acceleration) - 2838 -
  

SI Extended Functions - SI Motion drive-integrated control signals / status signals - 2840 -
  

SI Extended Functions - S_STW1 safety control word 1, S_ZSW1 safety status word 1 - 2842 -
  

SI Extended Functions - S_STW2 safety control word 2, S_ZSW2 safety status word 2 - 2843 -
  

SI Advanced Functions - S_ZSW_CAM1 Safety status word Safe Cam 1 - 2844 -
  

SI Extended Functions - Control via PROFIsafe (p9601.2 = p9601.3 = 1) - 2858 -
  

SI Extended Functions - CU310-2 (F-DI 0 ... F-DI 2) - 2870 -
  

SI Extended Functions - CU310-2 failsafe digital output (F-DO 0) - 2873 -
  

SI Extended Functions - CU310-2 control interface - 2875 -
  

SI Extended Functions - CU310-2 Safe State selection - 2876 -
  

SI Extended Functions - CU310-2 assignment (F-DO 0) - 2877 -
  

SI TM54F - Overview - 2890 -
  

SI TM54F - Parameter manager - 2891 -
  

SI TM54F - Configuration, F-DI/F-DO test - 2892 -
  

SI TM54F - Failsafe digital inputs (F-DI 0 ... F-DI 4) - 2893 -
  

SI TM54F - Failsafe digital inputs (F-DI 5 ... F-DI 9) - 2894 -
  

SI TM54F - Failsafe digital outputs (F-DO 0 ... 3), digital inputs (DI 20 ... 23) - 2895 -
  

SI TM54F - Basic Functions control interface (p9601.2/3 = 0 & p9601.6 = 1) - 2900 -
  

SI TM54F - Basic Functions Safe State selection - 2901 -
  

SI TM54F - Basic Functions assignment (F-DO 0 ... F-DO 3) - 2902 -
  

SI TM54F - Extended Functions control interface (p9601.2 = 1 & p9601.3 = 0) - 2905 -
  

SI TM54F - Extended Functions Safe State selection - 2906 -
  

SI TM54F - Extended Functions assignment (F-DO 0 ... F-DO 3) - 2907 -
  

SI PROFIsafe - Standard telegrams - 2915 -
  

SI PROFIsafe - Manufacturer-specific telegrams - 2917 -
  

Setpoint channel - Overview - 3001 -
  

Setpoint channel - Fixed speed setpoints - 3010 -
  

Setpoint channel - Motorized potentiometer - 3020 -
  

Setpoint channel - Main setpoint / additional setpoint, setpoint scaling, jog - 3030 -
  

Setpoint channel - Direction limitation and direction reversal - 3040 -
  

Setpoint channel - Skip frequency bands and speed limits - 3050 -
  

Setpoint channel - Basic ramp-function generator - 3060 -
  

Setpoint channel - Extended ramp-function generator - 3070 -
  

Setpoint channel - Ramp-function generator selection, -status word, -tracking - 3080 -
  

Setpoint channel - Extended Stop and Retract (ESR, r0108.9 = 1) - 3082 -
  

EPOS - Jog mode (r0108.4 = 1) - 3610 -
  

EPOS - Referencing / reference point approach mode (r0108.4 = 1) (p2597 = 0-signal) - 3612 -
  

EPOS - Flying referencing mode (r0108.4 = 1) (p2597 = 1 signal) - 3614 -
  

EPOS - Traversing blocks mode external block change (r0108.4 = 1) - 3615 -
  

EPOS - Traversing blocks mode (r0108.4 = 1) - 3616 -
  

EPOS - Travel to fixed stop (r0108.4 = 1) - 3617 -
  

EPOS - Direct setpoint input/MDI mode, dynamic values (r0108.4 = 1) - 3618 -
  

EPOS - Direct setpoint input/MDI mode (r0108.4 = 1) - 3620 -
  

EPOS - Mode control (r0108.4 = 1) - 3625 -
  

EPOS - Traversing range limits (r0108.4 = 1) - 3630 -
  

EPOS - Interpolator (r0108.4 = 1) - 3635 -
  

EPOS - Control word block selection/MDI selection (r0108.4 = 1) - 3640 -
  

EPOS - Status word 1 (r0108.3 = 1, r0108.4 = 1) - 3645 -
  

EPOS - Status word 2 (r0108.3 = 1, r0108.4 = 1) - 3646 -
  

EPOS - Status word active traversing block/MDI active (r0108.4 = 1) - 3650 -
  

Position control - Position actual value processing (r0108.3 = 1) - 4010 -
  

Position control - Position controller (r0108.3 = 1) - 4015 -
  

Position control - Standstill monitoring / position monitoring (r0108.3 = 1) - 4020 -
  

Position control - Dynamic following error monitoring, cam sequencer (r0108.3 = 1) - 4025 -
  

Encoder evaluation - Servo control, overview - 4700 -
  

Encoder evaluation - Vector control, overview - 4702 -
  

Encoder evaluation - Position and temperature sensing encoder 1 ... 3 - 4704 -
  

Encoder evaluation - Speed actual value and pole position sensing encoder 1, n_act_filter 5 - 4715 -
  

Encoder evaluation - Encoder interface, receive signals encoder 1 … 3 - 4720 -
  

Encoder evaluation - Encoder interface, send signals encoder 1 … 3 - 4730 -
  

Encoder evaluation - Reference mark search with external zero mark encoder 1 ... 3 - 4735 -
  

Encoder evaluation - Measuring probe evaluation, measured value memory encoder 1 ... 3 - 4740 -
  

Encoder evaluation - Absolute value for incremental encoder - 4750 -
  

Servo control - Online tuning with activated moment of inertia estimator (r0108.10 = 1) - 5045 -
  

Vector control - Closed-loop speed control and generation of the torque limits, overview - 6020 -
  

Vector control - Speed setpoint, droop - 6030 -
  

Vector control - Pre-control symmetrization reference/acceleration model - 6031 -
  

Vector control - Moment of inertia estimator (r0108.10 = 1) - 6035 -
  

Vector control - Speed controller with/without encoder - 6040 -
  

Vector control - Speed controller adaptation (Kp_n/Tn_n adaptation) - 6050 -
  

Vector control - Torque setpoint - 6060 -
  

Vector control - Vdc_max controller and Vdc_min controller - 6220 -
  

Vector control - U/f control, overview - 6300 -
  

Vector control - U/f characteristic and voltage boost - 6301 -
  

Vector control - Resonance damping and slip compensation - 6310 -
  

Vector control - Vdc_max controller and Vdc_min controller (U/f) - 6320 -
  

Vector control - Closed-loop speed control configuration - 6490 -
  

Vector control - Flux control configuration - 6491 -
  

Vector control - Excitation (SESM, p0300 = 5) - 6495 -
  

Vector control - Upper/lower torque limit - 6630 -
  

Vector control - Current/power/torque limits - 6640 -
  

Vector control - Current control, overview - 6700 -
  

Vector control - Current setpoint filter - 6710 -
  

Vector control - Iq and Id controllers - 6714 -
  

Vector control - Id setpoint (PMSM, p0300 = 2) - 6721 -
  

Vector control - Field weakening characteristic, Id setpoint (ASM, p0300 = 1) - 6722 -
  

Vector control - Field weakening controller, flux controller (ASM, p0300 = 1) - 6723 -
  

Vector control - Field weakening controller (PMSM, p0300 = 2) - 6724 -
  

Vector control - Flux setpoint, field weakening controller (SESM, p0300 = 5) - 6725 -
  

Vector control - Field weakening controller, flux controller (SESM, p0300 = 5) - 6726 -
  

Vector control - Current model, excitation current monitoring, cos phi (SESM, p0300 = 5) - 6727 -
  

Vector control - Interface to the Motor Module (ASM, p0300 = 1) - 6730 -
  

Vector control - Interface to the Motor Module (PMSM, p0300 = 2) - 6731 -
  

Vector control - Interface to the Motor Module (SESM, p0300 = 5) - 6732 -
  

Vector control - Motor model selection (SESM and p1300 = 20, p0300 = 5) - 6733 -
  

Vector control - Flux setpoint (RESM, p0300 = 6) - 6790 -
  

Vector control - Id setpoint (RESM, p0300 = 6) - 6791 -
  

Vector control - Interface to the Motor Module (RESM, p0300 = 6) - 6792 -
  

Vector control - Display signals - 6799 -
  

Technology functions - Friction characteristic - 7010 -
  

Technology functions - External armature short-circuit (EASC, p0300 = 2xx or 4xx) - 7014 -
  

Technology functions - Internal armature short-circuit (IASC, p0300 = 2xx or 4xx) - 7016 -
  

Technology functions - DC braking (p0300 = 1xx) - 7017 -
  

Technology functions - Synchronizing - 7020 -
  

Technology functions - Essential service mode (ESM) - 7033 -
  

General - 7200 -
  

Logic function blocks - AND (AND function block with 4 inputs) - 7210 -
  

Logic function blocks - OR (OR function block with 4 inputs) - 7212 -
  

Logic function blocks - XOR (XOR function block with 4 inputs) - 7214 -
  

Logic function blocks - NOT (inverter) - 7216 -
  

Arithmetic function blocks - ADD (adder with 4 inputs), SUB (subtractor) - 7220 -
  

Arithmetic function blocks - MUL (multiplier), DIV (divider) - 7222 -
  

Arithmetic function blocks - AVA (absolute value generator) - 7224 -
  

Timing function blocks - MFP (pulse generator), PCL (pulse contractor) - 7230 -
  

Timing function blocks - PDE (ON delay device), PDF (OFF delay device) - 7232 -
  

Timing function blocks - PST (pulse stretcher) - 7234 -
  

Memory function blocks - RSR (RS flip-flop), DFR (D flip-flop) timing function blocks - PST (pulse stretcher) - 7240 -
  

Switch function blocks - BSW (binary switch), NSW (numeric switch) - 7250 -
  

Control function blocks - LIM (limiter) - 7260 -
  

Control function blocks - PT1 (smoothing element) - 7262 -
  

Control function blocks - INT (integrator), DIF (derivative-action element) - 7264 -
  

Complex function blocks - LVM (double-sided limit monitor with hysteresis) - 7270 -
  

Technology controller - Fixed values, binary selection (r0108.16 = 1 and p2216 = 2) - 7950 -
  

Technology controller - Fixed values, direct selection (r0108.16 = 1 and p2216 = 1) - 7951 -
  

Technology controller - Motorized potentiometer (r0108.16 = 1) - 7954 -
  

Technology controller - Closed-loop control (r0108.16 = 1) - 7958 -
  

Technology controller - Kp/Tn adaptation (r0108.16 = 1) - 7959 -
  

Technology controller - DC link voltage controller (r0108.16 = 1) - 7960 -
  

Signals and monitoring functions - Overview - 8005 -
  

Signals and monitoring functions - Speed messages 1 - 8010 -
  

Signals and monitoring functions - Speed messages 2 - 8011 -
  

Signals and monitoring functions - Torque messages, motor blocked/stalled - 8012 -
  

Signals and monitoring functions - Load monitoring (r0108.17 = 1) - 8013 -
  

Signals and monitoring functions - Motor temperature status word faults/alarms - 8015 -
  

Signals and monitoring functions - Thermal monitoring motor - 8016 -
  

Signals and monitoring functions - Motor temperature model 1 (I2t) - 8017 -
  

Signals and monitoring functions - Motor temperature model 2 - 8018 -
  

Signals and monitoring functions - Motor temperature model 3 - 8019 -
  

Signals and monitoring functions - Separately excited synchronous motor (SESM, p0300 = 5) - 8020 -
  

Signals and monitoring functions - Thermal monitoring power unit - 8021 -
  

Signals and monitoring functions - Freely parameterized I2t monitoring (SESM) - 8022 -
  

Diagnostics - Overview - 8050 -
  

Diagnostics - Fault buffer - 8060 -
  

Diagnostics - Alarm buffer - 8065 -
  

Diagnostics - Faults/alarms trigger word (r2129) - 8070 -
  

Diagnostics - Faults/alarms configuration - 8075 -
  

Diagnostics - Measuring sockets (T0, T1, T2) - 8134 -
  

Diagnostics - Recorder overview (r0108.5 = 1) - 8144 -
  

Diagnostics - Recorder sequence control (r0108.5 = 1) - 8145 -
  

Data sets - Command data sets (CDS) - 8560 -
  

Data sets - Drive data sets (DDS) - 8565 -
  

Data sets - Encoder data sets (EDS) - 8570 -
  

Data sets - Motor data sets (MDS) - 8575 -
  

Data sets - Power unit data sets (PDS) - 8580 -
  

Basic Infeed - Overview - 8710 -
  

Basic Infeed - Control word sequence control infeed - 8720 -
  

Basic Infeed - Status word sequence control infeed - 8726 -
  

Basic Infeed - Sequencer - 8732 -
  

Active Infeed - Missing enables, line contactor control - 8738 -
  

Basic Infeed - Interface to the Basic Infeed power unit (control signals, actual values) - 8750 -
  

Basic Infeed - Signals and monitoring functions - 8760 -
  

Active Infeed - Overview - 8910 -
  

Active Infeed - Control word sequence control infeed - 8920 -
  

Active Infeed - Status word sequence control infeed - 8926 -
  

Active Infeed - Status word infeed - 8928 -
  

Active Infeed - Sequencer - 8932 -
  

Active Infeed - Missing enables, line contactor control - 8938 -
  

Active Infeed - Controller modulation depth reserve / controller DC link voltage (p3400.0 = 0) - 8940 -
  

Active Infeed - Reactive current / apparent current limits (r0108.3 = 1) - 8945 -
  

Active Infeed - Current precontrol / current controller / gating unit (p3400.0 = 0) - 8946 -
  

Active Infeed - Master/slave (r0108.19 = 1) - 8948 -
  

Active Infeed - Interface to the Active Infeed, control signals, actual values (p3400.0 = 0) - 8950 -
  

Active Infeed - Cos phi display (r0108.10 = 1) - 8951 -
  

Active Infeed - Signals and monitoring functions, line voltage monit (p3400.0 = 0) - 8960 -
  

Active Infeed - Signals and monitoring functions, line frequency/Vdc monit (p3400.0 = 0) - 8964 -
  

Terminal Board 30 (TB30) - Overview - 9099 -
  

Terminal Board 30 (TB30) - Electrically isolated digital inputs (DI 0 ... DI 3) - 9100 -
  

Terminal Board 30 (TB30) - Electrically isolated digital outputs (DO 0 ... DO 3) - 9102 -
  

Terminal Board 30 (TB30) - Analog inputs (AI 0 ... AI 1) - 9104 -
  

Terminal Board 30 (TB30) - Analog outputs (AO 0 ... AO 1) - 9106 -
  

Communication Board CAN10 (CBC10) - Receive telegram free PDO mapping (p8744 = 2) - 9204 -
  

Communication Board CAN10 (CBC10) - Receive telegram predef conn set (p8744 = 1) - 9206 -
  

Communication Board CAN10 (CBC10) - Send telegram free PDO mapping (p8744 = 2) - 9208 -
  

Communication Board CAN10 (CBC10) - Send telegram predef conn set (p8744 = 1) - 9210 -
  

Communication Board CAN10 (CBC10) - Control word CANopen - 9220 -
  

Communication Board CAN10 (CBC10) - Status word CANopen - 9226 -
  

Terminal Module 15 (TM15) - Overview TM15 (SIMOTION) - 9389 -
  

Terminal Module 15 (TM15) - Overview TM15DI_DO (SINAMICS) - 9399 -
  

Terminal Module 15 (TM15) - Digital inputs/outputs bidirectional (DI/DO 0 ... DI/DO 7) - 9400 -
  

Terminal Module 15 (TM15) - Digital inputs/outputs bidirectional (DI/DO 8 ... DI/DO 15) - 9401 -
  

Terminal Module 15 (TM15) - Digital inputs/outputs bidirectional (DI/DO 16 ... DI/DO 23) - 9402 -
  

Terminal Module 17 High Feature (TM17 High Feature) - Overview - 9419 -
  

Terminal Module 31 (TM31) - Overview - 9549 -
  

Terminal Module 31 (TM31) - Electrically isolated digital inputs (DI 0 ... DI 3) - 9550 -
  

Terminal Module 31 (TM31) - Electrically isolated digital inputs (DI 4 ... DI 7) - 9552 -
  

Terminal Module 31 (TM31) - Electrically isolated digital relay outputs (DO 0 ... DO 1) - 9556 -
  

Terminal Module 31 (TM31) - Digital inputs/outputs, bidirectional (DI/DO 8 ... DI/DO 9) - 9560 -
  

Terminal Module 31 (TM31) - Digital inputs/outputs, bidirectional (DI/DO 10 ... DI/DO 11) - 9562 -
  

Terminal Module 31 (TM31) - Analog input 0 (AI 0) - 9566 -
  

Terminal Module 31 (TM31) - Analog input 1 (AI 1) - 9568 -
  

Terminal Module 31 (TM31) - Analog outputs (AO 0 ... AO 1) - 9572 -
  

Terminal Module 31 (TM31) - Temperature evaluation - 9576 -
  

Terminal Module 31 (TM31) - Sensor monitoring KTY/PTC/PT1000 - 9577 -
  

Terminal Module 120 (TM120) - Temperature evaluation channels 0 and 1 - 9605 -
  

Terminal Module 120 (TM120) - Temperature evaluation channels 2 and 3 - 9606 -
  

Terminal Module 150 (TM150) - Temperature evaluation structure (channel 0 … 11) - 9625 -
  

Terminal Module 150 (TM150) - Temperature evaluation 1x2-, 3-, 4-wire (channels 0 … 5) - 9626 -
  

Terminal Module 150 (TM150) - Temperature evaluation 2x2-wire (channels 0 … 11) - 9627 -
  

Terminal Module 41 (TM41) - Overview - 9659 -
  

Terminal Module 41 (TM41) - Electrically isolated digital inputs (DI 0 ... DI 3) - 9660 -
  

Terminal Module 41 (TM41) - Digital inputs/outputs, bidirectional (DI/DO 0 ... DI/DO 1) - 9661 -
  

Terminal Module 41 (TM41) - Digital inputs/outputs, bidirectional (DI/DO 2 ... DI/DO 3) - 9662 -
  

Terminal Module 41 (TM41) - Analog input 0 (AI 0) - 9663 -
  

Terminal Module 41 (TM41) - Incremental encoder emulation (p4400 = 0) - 9674 -
  

Terminal Module 41 (TM41) - Incremental encoder emulation (p4400 = 1) - 9676 -
  

Terminal Module 41 (TM41) - STW1 control word interconnection (p0922 = 3) - 9677 -
  

Terminal Module 41 (TM41) - Control word sequence control (p4400 = 0) - 9678 -
  

Terminal Module 41 (TM41) - STW2 control word interconnection (p0922 = 3) - 9679 -
  

Terminal Module 41 (TM41) - Status word sequence control - 9680 -
  

Terminal Module 41 (TM41) - ZSW1 status word interconnection (p0922 = 3) - 9681 -
  

Terminal Module 41 (TM41) - Sequencer (p4400 = 0) - 9682 -
  

Terminal Module 41 (TM41) - ZSW2 status word interconnection (p0922 = 3) - 9683 -
  

Auxiliaries - Chassis power units 3-phase AC line connection contactor status display - 9814 -
  

Voltage Sensing Module (VSM) - Analog inputs (AI 0 ... AI 3) - 9880 -
  

Voltage Sensing Module (VSM) - Temperature evaluation - 9886 -
  

Basic Operator Panel 20 (BOP20) - Control word interconnection - 9912 -
  

External Braking Module - Sequencer (r0108.26 = 1) - 9951 -

## Function Diagrams for SINAMICS G130 - Overview

The structure of the drive functions and the relationship of the individual parameters with other parameters is displayed in the function diagrams:

Explanations of the function diagrams - Explanation of the symbols (Part 1) - 1020 -
  

Explanations of the function diagrams - Explanation of the symbols (Part 2) - 1021 -
  

Explanations of the function diagrams - Explanation of the symbols (Part 3) - 1022 -
  

Explanations of the function diagrams - Handling BICO technology - 1030 -
  

CU310-2 input/output terminals - Overview - 2019 -
  

CU310-2 input/output terminals - Electrically isolated digital inputs (DI 0 ... DI 3, DI 22) - 2020 -
  

CU310-2 input/output terminals - Electrically isolated digital inputs (DI 16 ... DI 21) - 2021 -
  

CU310-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 8 ... DI/DO 9) - 2030 -
  

CU310-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 10 ... DI/DO 11) - 2031 -
  

CU310-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 12 ... DI/DO 13) - 2032 -
  

CU310-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 14 ... DI/DO 15) - 2033 -
  

CU310-2 input/output terminals - Digital output (DO 16) - 2038 -
  

CU310-2 input/output terminals - Analog input (AI 0) - 2040 -
  

CU320-2 input/output terminals - Overview - 2119 -
  

CU320-2 input/output terminals - Electrically isolated digital inputs (DI 0 ... DI 3, DI 16, DI 17) - 2120 -
  

CU320-2 input/output terminals - Electrically isolated digital inputs (DI 4 ... DI 7, DI 20, DI 21) - 2121 -
  

CU320-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 8 ... DI/DO 9) - 2130 -
  

CU320-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 10 ... DI/DO 11) - 2131 -
  

CU320-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 12 ... DI/DO 13) - 2132 -
  

CU320-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 14 ... DI/DO 15) - 2133 -
  

D455-2 input/output terminals - Overview - 2139 -
  

D455-2 input/output terminals - Electrically isolated digital inputs (DI 0 ... DI 3, DI 16, DI 17) - 2140 -
  

D455-2 input/output terminals - Electrically isolated digital inputs (DI 4 ... DI 7, DI 20, DI 21) - 2141 -
  

D455-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 8 ... DI/DO 9) - 2150 -
  

D455-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 10 ... DI/DO 11) - 2151 -
  

D455-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 12 ... DI/DO 13) - 2152 -
  

D455-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 14 ... DI/DO 15) - 2153 -
  

CX32-2 input/output terminals - Overview - 2179 -
  

CX32-2 input/output terminals - Electrically isolated digital inputs (DI 0 ... DI 3, DI 16, DI 17) - 2180 -
  

CX32-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 8 ... DI/DO 9) - 2190 -
  

CX32-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 10 ... DI/DO 11) - 2191 -
  

Control Unit communication - CU_CX32/CU_LINK - 2194 -
  

Control Unit communication - CU_LINK/CU internal - 2195 -
  

Control Unit communication - SINAMICS Link overview (r0108.31 = 1, p8835 = 3) - 2197 -
  

Control Unit communication - SINAMICS Link configuration (r0108.31 = 1, p8835 = 3) - 2198 -
  

Control Unit communication - SINAMICS Link receive data (r0108.31 = 1, p8835 = 3) - 2199 -
  

Control Unit communication - SINAMICS Link send data (r0108.31 = 1, p8835 = 3) - 2200 -
  

S120M input/output terminals - Digital inputs/outputs bidirectional (DI/DO 0 ... DI/DO 1) - 2201 -
  

PROFIenergy - Control commands / interrogation commands - 2381 -
  

PROFIenergy - States - 2382 -
  

PROFIdrive - Overview - 2401 -
  

PROFIdrive - PROFIBUS (PB) / PROFINET (PN), addresses and diagnostics - 2410 -
  

PROFIdrive - Standard telegrams and process data 1 - 2415 -
  

PROFIdrive - Standard telegrams and process data 2 - 2416 -
  

PROFIdrive - Manufacturer-specific telegrams and process data 2 - 2420 -
  

PROFIdrive - Manufacturer-specific telegrams and process data 3 - 2421 -
  

PROFIdrive - Manufacturer-specific telegrams and process data 4 - 2422 -
  

PROFIdrive - Supplementary telegrams/free telegrams and process data - 2423 -
  

PROFIdrive - STW1_BM control word metal industry interconnection - 2425 -
  

PROFIdrive - STW2_BM control word metal industry interconnection - 2426 -
  

PROFIdrive - E_STW1_BM control word infeed metal industry interconnection - 2427 -
  

PROFIdrive - ZSW1_BM status word metal industry interconnection - 2428 -
  

PROFIdrive - ZSW2_BM status word metal industry interconnection - 2429 -
  

PROFIdrive - E_ZSW1_BM status word infeed metal industry interconnection - 2430 -
  

PROFIdrive - PZD receive signals profile-specific interconnection - 2439 -
  

PROFIdrive - PZD receive signals manufacturer-specific interconnection - 2440 -
  

PROFIdrive - STW1 control word interconnection (p2038 = 2) - 2441 -
  

PROFIdrive - STW1 control word interconnection (p2038 = 0) - 2442 -
  

PROFIdrive - STW2 control word interconnection (p2038 = 0) - 2444 -
  

PROFIdrive - E_STW1 control word infeed interconnection - 2447 -
  

PROFIdrive - STW7 control word interconnection - 2448 -
  

PROFIdrive - PZD send signals profile-specific interconnection - 2449 -
  

PROFIdrive - PZD send signals manufacturer-specific interconnection - 2450 -
  

PROFIdrive - ZSW1 status word interconnection (p2038 = 2) - 2451 -
  

PROFIdrive - ZSW1 status word interconnection (p2038 = 0) - 2452 -
  

PROFIdrive - ZSW1 status word interconnection (p2038 = 1) - 2453 -
  

PROFIdrive - ZSW2 status word interconnection (p2038 = 0) - 2454 -
  

PROFIdrive - ZSW2 status word interconnection (p2038 = 1) - 2455 -
  

PROFIdrive - MELDW status word interconnection - 2456 -
  

PROFIdrive - E_ZSW1 status word infeed interconnection - 2457 -
  

PROFIdrive - ZSW7 status word interconnection - 2458 -
  

PROFIdrive - IF1 receive telegram free interconnection via BICO (p0922 = 999) - 2468 -
  

PROFIdrive - IF1 send telegram free interconnection via BICO (p0922 = 999) - 2470 -
  

PROFIdrive - IF1 status words free interconnection - 2472 -
  

PROFIdrive - IF1 receive telegram free interconnection via BICO (p0922 = 999) - 2481 -
  

PROFIdrive - IF1 send telegram free interconnection via BICO (p0922 = 999) - 2483 -
  

PROFIdrive - IF2 receive telegram free interconnection - 2485 -
  

PROFIdrive - IF2 send telegram free interconnection - 2487 -
  

PROFIdrive - IF2 status words free interconnection - 2489 -
  

PROFIdrive - IF2 receive telegram free interconnection - 2491 -
  

PROFIdrive - IF2 send telegram free interconnection - 2493 -
  

PROFIdrive - CU_STW1 control word 1 Control Unit interconnection - 2495 -
  

PROFIdrive - CU_ZSW1 status word 1 Control Unit interconnection - 2496 -
  

PROFIdrive - O_DIGITAL interconnection - 2497 -
  

PROFIdrive - I_DIGITAL interconnection - 2498 -
  

PROFIdrive - O_DIGITAL_1 interconnection - 2499 -
  

PROFIdrive - I_DIGITAL_1 interconnection - 2500 -
  

Internal control/status words - Control word sequence control - 2501 -
  

Internal control/status words - Status word sequence control - 2503 -
  

Internal control/status words - Control word setpoint channel - 2505 -
  

Internal control/status words - Control word speed controller - 2520 -
  

Internal control/status words - Status word speed controller - 2522 -
  

Internal control/status words - Status word closed-loop control - 2526 -
  

Internal control/status words - Status word current control - 2530 -
  

Internal control/status words - Status word monitoring functions 1 - 2534 -
  

Internal control/status words - Status word monitoring functions 2 - 2536 -
  

Internal control/status words - Status word monitoring functions 3 - 2537 -
  

Internal control/status words - Control word faults/alarms - 2546 -
  

Internal control/status words - Status word faults/alarms 1 and 2 - 2548 -
  

Sequence control - Sequencer - 2610 -
  

Sequence control - Missing enables, line contactor control, logic operation - 2634 -
  

Brake control - Simple brake control (r0108.14 = 0) - 2701 -
  

Brake control - Extended brake control, standstill detection (r0108.14 = 1) - 2704 -
  

Brake control - Extended brake control, open/close brake (r0108.14 = 1) - 2707 -
  

Brake control - Extended brake control, signal outputs (r0108.14 = 1) - 2711 -
  

SI Basic Functions - Parameter manager - 2800 -
  

SI Basic Functions - Monitoring functions and faults/alarms - 2802 -
  

SI Basic Functions - SI status CU, MM, CU + MM, group STO - 2804 -
  

SI Basic Functions - S_STW1/2 safety control word 1/2, S_ZSW1/2 safety status word 1/2 - 2806 -
  

SI Basic Functions - STO (Safe Torque Off), SS1 (Safe Stop 1) - 2810 -
  

SI Basic Functions - STO (Safe Torque Off), safe pulse suppression - 2811 -
  

SI Basic Functions - SBC (Safe Brake Control), SBA (Safe Brake Adapter) - 2814 -
  

SI Extended Functions - Parameter manager - 2818 -
  

SI Extended Functions - SS1, SS2, SOS, internal STOP B, C, D, F - 2819 -
  

SI Extended Functions - SLS (Safely-Limited Speed) - 2820 -
  

SI Extended Functions - Safe referencing - 2821 -
  

SI Advanced Functions - SLP (Safely-Limited Position) - 2822 -
  

SI Extended Functions - SSM (Safe Speed Monitor) - 2823 -
  

SI Extended Functions - SDI (Safe Direction) - 2824 -
  

SI Extended Functions - SAM (Safe Acceleration Monitor), SBR (Safe Brake Ramp) - 2825 -
  

SI Advanced Functions - SCA (Safe Cam) - 2826 -
  

SI Extended Functions - SBT (Safe Brake Test) - 2836 -
  

SI Extended Functions - Selection of the active control word - 2837 -
  

SI Extended Functions - SLA (Safely-Limited Acceleration) - 2838 -
  

SI Extended Functions - SI Motion drive-integrated control signals / status signals - 2840 -
  

SI Extended Functions - S_STW1 safety control word 1, S_ZSW1 safety status word 1 - 2842 -
  

SI Extended Functions - S_STW2 safety control word 2, S_ZSW2 safety status word 2 - 2843 -
  

SI Advanced Functions - S_ZSW_CAM1 Safety status word Safe Cam 1 - 2844 -
  

SI Extended Functions - Control via PROFIsafe (p9601.2 = p9601.3 = 1) - 2858 -
  

SI Extended Functions - CU310-2 (F-DI 0 ... F-DI 2) - 2870 -
  

SI Extended Functions - CU310-2 failsafe digital output (F-DO 0) - 2873 -
  

SI Extended Functions - CU310-2 control interface - 2875 -
  

SI Extended Functions - CU310-2 Safe State selection - 2876 -
  

SI Extended Functions - CU310-2 assignment (F-DO 0) - 2877 -
  

SI TM54F - Overview - 2890 -
  

SI TM54F - Parameter manager - 2891 -
  

SI TM54F - Configuration, F-DI/F-DO test - 2892 -
  

SI TM54F - Failsafe digital inputs (F-DI 0 ... F-DI 4) - 2893 -
  

SI TM54F - Failsafe digital inputs (F-DI 5 ... F-DI 9) - 2894 -
  

SI TM54F - Failsafe digital outputs (F-DO 0 ... 3), digital inputs (DI 20 ... 23) - 2895 -
  

SI TM54F - Basic Functions control interface (p9601.2/3 = 0 & p9601.6 = 1) - 2900 -
  

SI TM54F - Basic Functions Safe State selection - 2901 -
  

SI TM54F - Basic Functions assignment (F-DO 0 ... F-DO 3) - 2902 -
  

SI TM54F - Extended Functions control interface (p9601.2 = 1 & p9601.3 = 0) - 2905 -
  

SI TM54F - Extended Functions Safe State selection - 2906 -
  

SI TM54F - Extended Functions assignment (F-DO 0 ... F-DO 3) - 2907 -
  

SI PROFIsafe - Standard telegrams - 2915 -
  

SI PROFIsafe - Manufacturer-specific telegrams - 2917 -
  

Setpoint channel - Overview - 3001 -
  

Setpoint channel - Fixed speed setpoints - 3010 -
  

Setpoint channel - Motorized potentiometer - 3020 -
  

Setpoint channel - Main setpoint / additional setpoint, setpoint scaling, jog - 3030 -
  

Setpoint channel - Direction limitation and direction reversal - 3040 -
  

Setpoint channel - Skip frequency bands and speed limits - 3050 -
  

Setpoint channel - Basic ramp-function generator - 3060 -
  

Setpoint channel - Extended ramp-function generator - 3070 -
  

Setpoint channel - Ramp-function generator selection, -status word, -tracking - 3080 -
  

Setpoint channel - Extended Stop and Retract (ESR, r0108.9 = 1) - 3082 -
  

Setpoint channel - Dynamic Servo Control (DSC) linear and DSC spline (r0108.6 = 1) - 3090 -
  

Setpoint channel not activated - Generation of the speed limits (r0108.8 = 0) - 3095 -
  

Encoder evaluation - Overview - 4702 -
  

Encoder evaluation - Raw signal acquisition - 4704 -
  

Encoder evaluation - Speed actual value and pole position sensing encoder 1 - 4710 -
  

Encoder evaluation - Speed actual value acquisition encoder 2, 3 (r0108.7 = 1, APC activated) - 4711 -
  

Encoder evaluation - Speed actual value and pole position sensing encoder 1, n_act_filter 5 - 4715 -
  

Encoder evaluation - Absolute value for incremental encoder - 4750 -
  

Hydraulic drive - Velocity controller - 4965 -
  

Hydraulic drive - Knee compensation - 4966 -
  

Hydraulic drive - Force controller - 4970 -
  

Hydraulic drive - Valve characteristic, area adaptation - 4975 -
  

Hydraulic drive - Static friction compensation with force controller (p1400.2 = 1) - 4977 -
  

Hydraulic drive - Static friction compensation with voltage pulse / voltage ramp - 4978 -
  

Hydraulic drive - Sequencer - 4985 -
  

Hydraulic drive - P24 management with shutoff valve - 4990 -
  

Hydraulic drive - P24 management without shutoff valve - 4991 -
  

Servo control - Closed-loop speed control and U/f control, overview - 5019 -
  

Servo control - Speed setpoint filter and speed precontrol - 5020 -
  

Servo control - Reference model / precontrol balancing / speed limitation - 5030 -
  

Servo control - Moment of inertia estimator (r0108.10 = 1, r0108.30 = 1) - 5035 -
  

Servo control - Speed controller with encoder - 5040 -
  

Servo control - Speed controller, torque-speed precontrol with encoder (p1402.4 = 1) - 5042 -
  

Servo control - Online tuning with activated moment of inertia estimator (r0108.10 = 1) - 5045 -
  

Servo control - Speed controller adaptation (Kp_n/Tn_n adaptation) - 5050 -
  

Servo control - Controller parameter adaptation (r0171.29 = 1) - 5055 -
  

Servo control - Torque setpoint, control mode switchover - 5060 -
  

Servo control - Speed controller without encoder - 5210 -
  

Servo control - U/f control for diagnostics - 5300 -
  

Servo control - Variable message function 1, 2, 3 - 5301 -
  

Servo control - Closed-loop speed control configuration - 5490 -
  

Servo control - Generation of the torque limits, overview - 5609 -
  

Servo control - Torque limiting/reduction, interpolator - 5610 -
  

Servo control - Motoring/generating torque limit - 5620 -
  

Servo control - Upper/lower torque limit - 5630 -
  

Servo control - Mode switchover, power/current limitation - 5640 -
  

Servo control - Vdc_max controller and Vdc_min controller - 5650 -
  

Servo control - Current control, overview - 5700 -
  

Servo control - Current setpoint filters 1 … 4 - 5710 -
  

Servo control - Current setpoint filters 5 … 10 (r0108.21 = 1) - 5711 -
  

Servo control - Iq and Id controllers - 5714 -
  

Servo control - Field current / flux specification, flux reduction, flux controller - 5722 -
  

Servo control - Interface to the Motor Module (control signals, actual current values) - 5730 -
  

Vector control - Closed-loop speed control and generation of the torque limits, overview - 6020 -
  

Vector control - Speed setpoint, droop - 6030 -
  

Vector control - Pre-control symmetrization reference/acceleration model - 6031 -
  

Vector control - Moment of inertia estimator (r0108.10 = 1) - 6035 -
  

Vector control - Speed controller with/without encoder - 6040 -
  

Vector control - Speed controller adaptation (Kp_n/Tn_n adaptation) - 6050 -
  

Vector control - Torque setpoint - 6060 -
  

Vector control - Vdc_max controller and Vdc_min controller - 6220 -
  

Vector control - U/f control, overview - 6300 -
  

Vector control - U/f characteristic and voltage boost - 6301 -
  

Vector control - Resonance damping and slip compensation - 6310 -
  

Vector control - Vdc_max controller and Vdc_min controller (U/f) - 6320 -
  

Vector control - Closed-loop speed control configuration - 6490 -
  

Vector control - Flux control configuration - 6491 -
  

Vector control - Excitation (SESM, p0300 = 5) - 6495 -
  

Vector control - Upper/lower torque limit - 6630 -
  

Vector control - Current/power/torque limits - 6640 -
  

Vector control - Current control, overview - 6700 -
  

Vector control - Current setpoint filter - 6710 -
  

Vector control - Iq and Id controllers - 6714 -
  

Vector control - Id setpoint (PMSM, p0300 = 2) - 6721 -
  

Vector control - Field weakening characteristic, Id setpoint (ASM, p0300 = 1) - 6722 -
  

Vector control - Field weakening controller, flux controller (ASM, p0300 = 1) - 6723 -
  

Vector control - Field weakening controller (PMSM, p0300 = 2) - 6724 -
  

Vector control - Interface to the Motor Module (ASM, p0300 = 1) - 6730 -
  

Vector control - Interface to the Motor Module (PMSM, p0300 = 2) - 6731 -
  

Vector control - Flux setpoint (RESM, p0300 = 6) - 6790 -
  

Vector control - Id setpoint (RESM, p0300 = 6) - 6791 -
  

Vector control - Interface to the Motor Module (RESM, p0300 = 6) - 6792 -
  

Vector control - Display signals - 6799 -
  

Technology functions - kT estimator - 7008 -
  

Technology functions - Friction characteristic - 7010 -
  

Technology functions - Advanced Positioning Control (APC, r0108.7 = 1) - 7012 -
  

Technology functions - APC differential position gain (APC, r0108.7 = 1) - 7013 -
  

Technology functions - External armature short-circuit (EASC, p0300 = 2xx or 4xx) - 7014 -
  

Technology functions - Internal armature short-circuit (IASC, p0300 = 2xx or 4xx) - 7016 -
  

Technology functions - DC braking (p0300 = 1xx) - 7017 -
  

Technology functions - Synchronizing - 7020 -
  

Technology functions - Essential service mode (ESM) - 7033 -
  

General - 7200 -
  

Logic function blocks - AND (AND function block with 4 inputs) - 7210 -
  

Logic function blocks - OR (OR function block with 4 inputs) - 7212 -
  

Logic function blocks - XOR (XOR function block with 4 inputs) - 7214 -
  

Logic function blocks - NOT (inverter) - 7216 -
  

Arithmetic function blocks - ADD (adder with 4 inputs), SUB (subtractor) - 7220 -
  

Arithmetic function blocks - MUL (multiplier), DIV (divider) - 7222 -
  

Arithmetic function blocks - AVA (absolute value generator) - 7224 -
  

Timing function blocks - MFP (pulse generator), PCL (pulse contractor) - 7230 -
  

Timing function blocks - PDE (ON delay device), PDF (OFF delay device) - 7232 -
  

Timing function blocks - PST (pulse stretcher) - 7234 -
  

Memory function blocks - RSR (RS flip-flop), DFR (D flip-flop) timing function blocks - PST (pulse stretcher) - 7240 -
  

Switch function blocks - BSW (binary switch), NSW (numeric switch) - 7250 -
  

Control function blocks - LIM (limiter) - 7260 -
  

Control function blocks - PT1 (smoothing element) - 7262 -
  

Control function blocks - INT (integrator), DIF (derivative-action element) - 7264 -
  

Complex function blocks - LVM (double-sided limit monitor with hysteresis) - 7270 -
  

Technology controller - Fixed values, binary selection (r0108.16 = 1 and p2216 = 2) - 7950 -
  

Technology controller - Fixed values, direct selection (r0108.16 = 1 and p2216 = 1) - 7951 -
  

Technology controller - Motorized potentiometer (r0108.16 = 1) - 7954 -
  

Technology controller - Closed-loop control (r0108.16 = 1) - 7958 -
  

Technology controller - Kp/Tn adaptation (r0108.16 = 1) - 7959 -
  

Technology controller - DC link voltage controller (r0108.16 = 1) - 7960 -
  

Grid droop control (r0108.12 = 1) - Grid droop, voltage adjustment - 7982 -
  

Grid droop control (r0108.12 = 1) - DC component control, harmonic control - 7983 -
  

Line droop control (r0108.12 = 1) - Modulation depth control - 7984 -
  

Line droop control (r0108.12 = 1) - Sequence control overcurrent - 7986 -
  

Line transformer (r0108.4 = 1) - DC component control, negative phase sequence system controller - 7987 -
  

Grid transformer (r0108.4 = 1) - Island grid black start sequence control - 7988 -
  

Grid transformer (r0108.4 = 1) - Island grid synchronization sequence control - 7989
  

Line transformer (r0108.4 = 1) - Transformer model (p5480 = 1) - 7990 -
  

Line transformer (r0108.4 = 1) - Line filter monitoring - 7991 -
  

Line transformer (r0108.4 = 1) - PLL 2 (phase-locked loop 2) - 7992 -
  

Line transformer (r0108.4 = 1) - Transformer magnetization voltage threshold - 7993 -
  

Line transformer (r0108.4 = 1) - Transformer magnetization sequence control - 7994 -
  

Grid transformer (r0108.4 = 1) - Island grid synchronization voltage threshold - 7995
  

Dynamic grid support (r0108.7 = 1) - Characteristic - 7996 -
  

Dynamic grid support (r0108.7 = 1) - Current limits (p5501 = 1) - 7997 -
  

Dynamic grid support (r0108.7 = 1) - Sequence control - 7998 -
  

Dynamic grid support (r0108.7 = 1) - Grid monitoring island grid detection - 7999 -
  

Signals and monitoring functions - Overview - 8005 -
  

Signals and monitoring functions - Speed messages 1 - 8010 -
  

Signals and monitoring functions - Speed messages 2 - 8011 -
  

Signals and monitoring functions - Torque messages, motor blocked/stalled - 8012 -
  

Signals and monitoring functions - Load monitoring (r0108.17 = 1) - 8013 -
  

Signals and monitoring functions - Motor temperature status word faults/alarms - 8015 -
  

Signals and monitoring functions - Thermal monitoring motor - 8016 -
  

Signals and monitoring functions - Motor temperature model 1 (I2t) - 8017 -
  

Signals and monitoring functions - Motor temperature model 2 - 8018 -
  

Signals and monitoring functions - Motor temperature model 3 - 8019 -
  

Signals and monitoring functions - Thermal monitoring power unit - 8021 -
  

Signals and monitoring functions - Freely parameterized I2t monitoring (SESM) - 8022 -
  

Diagnostics - Overview - 8050 -
  

Diagnostics - Fault buffer - 8060 -
  

Diagnostics - Alarm buffer - 8065 -
  

Diagnostics - Faults/alarms trigger word (r2129) - 8070 -
  

Diagnostics - Faults/alarms configuration - 8075 -
  

Diagnostics - Measuring sockets (T0, T1, T2) - 8134 -
  

Diagnostics - Recorder overview (r0108.5 = 1) - 8144 -
  

Diagnostics - Recorder sequence control (r0108.5 = 1) - 8145 -
  

Data sets - Command data sets (CDS) - 8560 -
  

Data sets - Drive data sets (DDS) - 8565 -
  

Data sets - Encoder data sets (EDS) - 8570 -
  

Data sets - Motor data sets (MDS) - 8575 -
  

Data sets - Power unit data sets (PDS) - 8580 -
  

Basic Infeed - Overview - 8710 -
  

Basic Infeed - Control word sequence control infeed - 8720 -
  

Basic Infeed - Status word sequence control infeed - 8726 -
  

Basic Infeed - Sequencer - 8732 -
  

Active Infeed - Missing enables, line contactor control - 8738 -
  

Basic Infeed - Interface to the Basic Infeed power unit (control signals, actual values) - 8750 -
  

Basic Infeed - Signals and monitoring functions - 8760 -
  

Smart Infeed - Overview - 8810 -
  

Smart Infeed - Control word sequence control infeed - 8820 -
  

Smart Infeed - Status word sequence control infeed - 8826 -
  

Smart Infeed - Status word infeed - 8828 -
  

Smart Infeed - Sequencer - 8832 -
  

Smart Infeed - Missing enables, line contactor control - 8838 -
  

Smart Infeed - Interface to the Smart Infeed (control signals, actual values) - 8850 -
  

Smart Infeed - Signals and monitoring functions, line voltage monitoring - 8860 -
  

Smart Infeed - Signals and monitoring functions, line frequency and Vdc monitoring - 8864 -
  

Terminal Board 30 (TB30) - Overview - 9099 -
  

Terminal Board 30 (TB30) - Electrically isolated digital inputs (DI 0 ... DI 3) - 9100 -
  

Terminal Board 30 (TB30) - Electrically isolated digital outputs (DO 0 ... DO 3) - 9102 -
  

Terminal Board 30 (TB30) - Analog inputs (AI 0 ... AI 1) - 9104 -
  

Terminal Board 30 (TB30) - Analog outputs (AO 0 ... AO 1) - 9106 -
  

Communication Board CAN10 (CBC10) - Receive telegram free PDO mapping (p8744 = 2) - 9204 -
  

Communication Board CAN10 (CBC10) - Receive telegram predef conn set (p8744 = 1) - 9206 -
  

Communication Board CAN10 (CBC10) - Send telegram free PDO mapping (p8744 = 2) - 9208 -
  

Communication Board CAN10 (CBC10) - Send telegram predef conn set (p8744 = 1) - 9210 -
  

Communication Board CAN10 (CBC10) - Control word CANopen - 9220 -
  

Communication Board CAN10 (CBC10) - Status word CANopen - 9226 -
  

Terminal Module 15 (TM15) - Overview TM15 (SIMOTION) - 9389 -
  

Terminal Module 15 (TM15) - Overview TM15DI_DO (SINAMICS) - 9399 -
  

Terminal Module 15 (TM15) - Digital inputs/outputs bidirectional (DI/DO 0 ... DI/DO 7) - 9400 -
  

Terminal Module 15 (TM15) - Digital inputs/outputs bidirectional (DI/DO 8 ... DI/DO 15) - 9401 -
  

Terminal Module 15 (TM15) - Digital inputs/outputs bidirectional (DI/DO 16 ... DI/DO 23) - 9402 -
  

Terminal Module 17 High Feature (TM17 High Feature) - Overview - 9419 -
  

Terminal Module 31 (TM31) - Overview - 9549 -
  

Terminal Module 31 (TM31) - Electrically isolated digital inputs (DI 0 ... DI 3) - 9550 -
  

Terminal Module 31 (TM31) - Electrically isolated digital inputs (DI 4 ... DI 7) - 9552 -
  

Terminal Module 31 (TM31) - Electrically isolated digital relay outputs (DO 0 ... DO 1) - 9556 -
  

Terminal Module 31 (TM31) - Digital inputs/outputs, bidirectional (DI/DO 8 ... DI/DO 9) - 9560 -
  

Terminal Module 31 (TM31) - Digital inputs/outputs, bidirectional (DI/DO 10 ... DI/DO 11) - 9562 -
  

Terminal Module 31 (TM31) - Analog input 0 (AI 0) - 9566 -
  

Terminal Module 31 (TM31) - Analog input 1 (AI 1) - 9568 -
  

Terminal Module 31 (TM31) - Analog outputs (AO 0 ... AO 1) - 9572 -
  

Terminal Module 31 (TM31) - Temperature evaluation - 9576 -
  

Terminal Module 31 (TM31) - Sensor monitoring KTY/PTC/PT1000 - 9577 -
  

Terminal Module 120 (TM120) - Temperature evaluation channels 0 and 1 - 9605 -
  

Terminal Module 120 (TM120) - Temperature evaluation channels 2 and 3 - 9606 -
  

Terminal Module 150 (TM150) - Temperature evaluation structure (channel 0 … 11) - 9625 -
  

Terminal Module 150 (TM150) - Temperature evaluation 1x2-, 3-, 4-wire (channels 0 … 5) - 9626 -
  

Terminal Module 150 (TM150) - Temperature evaluation 2x2-wire (channels 0 … 11) - 9627 -
  

Terminal Module 41 (TM41) - Overview - 9659 -
  

Terminal Module 41 (TM41) - Electrically isolated digital inputs (DI 0 ... DI 3) - 9660 -
  

Terminal Module 41 (TM41) - Digital inputs/outputs, bidirectional (DI/DO 0 ... DI/DO 1) - 9661 -
  

Terminal Module 41 (TM41) - Digital inputs/outputs, bidirectional (DI/DO 2 ... DI/DO 3) - 9662 -
  

Terminal Module 41 (TM41) - Analog input 0 (AI 0) - 9663 -
  

Terminal Module 41 (TM41) - Incremental encoder emulation (p4400 = 0) - 9674 -
  

Terminal Module 41 (TM41) - Incremental encoder emulation (p4400 = 1) - 9676 -
  

Terminal Module 41 (TM41) - STW1 control word interconnection (p0922 = 3) - 9677 -
  

Terminal Module 41 (TM41) - Control word sequence control (p4400 = 0) - 9678 -
  

Terminal Module 41 (TM41) - STW2 control word interconnection (p0922 = 3) - 9679 -
  

Terminal Module 41 (TM41) - Status word sequence control - 9680 -
  

Terminal Module 41 (TM41) - ZSW1 status word interconnection (p0922 = 3) - 9681 -
  

Terminal Module 41 (TM41) - Sequencer (p4400 = 0) - 9682 -
  

Terminal Module 41 (TM41) - ZSW2 status word interconnection (p0922 = 3) - 9683 -
  

Auxiliaries - Cooling unit control and feedback signals (r0108.28 = 1) - 9794 -
  

Auxiliaries - Cooling unit sequence control (r0108.28 = 1) - 9795 -
  

Voltage Sensing Module (VSM) - Analog inputs (AI 0 ... AI 3) - 9880 -
  

Voltage Sensing Module (VSM) - Temperature evaluation - 9886 -
  

Basic Operator Panel 20 (BOP20) - Control word interconnection - 9912 -

## Function Diagrams for SINAMICS G150 - Overview

The structure of the drive functions and the relationship of the individual parameters with other parameters is displayed in the function diagrams:

Explanations of the function diagrams - Explanation of the symbols (Part 1) - 1020 -
  

Explanations of the function diagrams - Explanation of the symbols (Part 2) - 1021 -
  

Explanations of the function diagrams - Explanation of the symbols (Part 3) - 1022 -
  

Explanations of the function diagrams - Handling BICO technology - 1030 -
  

CU310-2 input/output terminals - Overview - 2019 -
  

CU310-2 input/output terminals - Electrically isolated digital inputs (DI 0 ... DI 3, DI 22) - 2020 -
  

CU310-2 input/output terminals - Electrically isolated digital inputs (DI 16 ... DI 21) - 2021 -
  

CU310-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 8 ... DI/DO 9) - 2030 -
  

CU310-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 10 ... DI/DO 11) - 2031 -
  

CU310-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 12 ... DI/DO 13) - 2032 -
  

CU310-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 14 ... DI/DO 15) - 2033 -
  

CU310-2 input/output terminals - Digital output (DO 16) - 2038 -
  

CU310-2 input/output terminals - Analog input (AI 0) - 2040 -
  

CU320-2 input/output terminals - Overview - 2119 -
  

CU320-2 input/output terminals - Electrically isolated digital inputs (DI 0 ... DI 3, DI 16, DI 17) - 2120 -
  

CU320-2 input/output terminals - Electrically isolated digital inputs (DI 4 ... DI 7, DI 20, DI 21) - 2121 -
  

CU320-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 8 ... DI/DO 9) - 2130 -
  

CU320-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 10 ... DI/DO 11) - 2131 -
  

CU320-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 12 ... DI/DO 13) - 2132 -
  

CU320-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 14 ... DI/DO 15) - 2133 -
  

D455-2 input/output terminals - Overview - 2139 -
  

D455-2 input/output terminals - Electrically isolated digital inputs (DI 0 ... DI 3, DI 16, DI 17) - 2140 -
  

D455-2 input/output terminals - Electrically isolated digital inputs (DI 4 ... DI 7, DI 20, DI 21) - 2141 -
  

D455-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 8 ... DI/DO 9) - 2150 -
  

D455-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 10 ... DI/DO 11) - 2151 -
  

D455-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 12 ... DI/DO 13) - 2152 -
  

D455-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 14 ... DI/DO 15) - 2153 -
  

CX32-2 input/output terminals - Overview - 2179 -
  

CX32-2 input/output terminals - Electrically isolated digital inputs (DI 0 ... DI 3, DI 16, DI 17) - 2180 -
  

CX32-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 8 ... DI/DO 9) - 2190 -
  

CX32-2 input/output terminals - Digital inputs/outputs bidirectional (DI/DO 10 ... DI/DO 11) - 2191 -
  

Control Unit communication - CU_CX32/CU_LINK - 2194 -
  

Control Unit communication - CU_LINK/CU internal - 2195 -
  

Control Unit communication - SINAMICS Link overview (r0108.31 = 1, p8835 = 3) - 2197 -
  

Control Unit communication - SINAMICS Link configuration (r0108.31 = 1, p8835 = 3) - 2198 -
  

Control Unit communication - SINAMICS Link receive data (r0108.31 = 1, p8835 = 3) - 2199 -
  

Control Unit communication - SINAMICS Link send data (r0108.31 = 1, p8835 = 3) - 2200 -
  

S120M input/output terminals - Digital inputs/outputs bidirectional (DI/DO 0 ... DI/DO 1) - 2201 -
  

PROFIenergy - Control commands / interrogation commands - 2381 -
  

PROFIenergy - States - 2382 -
  

PROFIdrive - Overview - 2401 -
  

PROFIdrive - PROFIBUS (PB) / PROFINET (PN), addresses and diagnostics - 2410 -
  

PROFIdrive - Standard telegrams and process data 1 - 2415 -
  

PROFIdrive - Standard telegrams and process data 2 - 2416 -
  

PROFIdrive - Manufacturer-specific telegrams and process data 3 - 2421 -
  

PROFIdrive - Manufacturer-specific telegrams and process data 4 - 2422 -
  

PROFIdrive - Supplementary telegrams/free telegrams and process data - 2423 -
  

PROFIdrive - STW1_BM control word metal industry interconnection - 2425 -
  

PROFIdrive - STW2_BM control word metal industry interconnection - 2426 -
  

PROFIdrive - E_STW1_BM control word infeed metal industry interconnection - 2427 -
  

PROFIdrive - ZSW1_BM status word metal industry interconnection - 2428 -
  

PROFIdrive - ZSW2_BM status word metal industry interconnection - 2429 -
  

PROFIdrive - E_ZSW1_BM status word infeed metal industry interconnection - 2430 -
  

PROFIdrive - PZD receive signals profile-specific interconnection - 2439 -
  

PROFIdrive - PZD receive signals manufacturer-specific interconnection - 2440 -
  

PROFIdrive - STW1 control word interconnection (p2038 = 2) - 2441 -
  

PROFIdrive - STW1 control word interconnection (p2038 = 0) - 2442 -
  

PROFIdrive - STW2 control word interconnection (p2038 = 0) - 2444 -
  

PROFIdrive - E_STW1 control word infeed interconnection - 2447 -
  

PROFIdrive - PZD send signals profile-specific interconnection - 2449 -
  

PROFIdrive - PZD send signals manufacturer-specific interconnection - 2450 -
  

PROFIdrive - ZSW1 status word interconnection (p2038 = 2) - 2451 -
  

PROFIdrive - ZSW1 status word interconnection (p2038 = 0) - 2452 -
  

PROFIdrive - ZSW2 status word interconnection (p2038 = 0) - 2454 -
  

PROFIdrive - E_ZSW1 status word infeed interconnection - 2457 -
  

PROFIdrive - IF1 receive telegram free interconnection via BICO (p0922 = 999) - 2468 -
  

PROFIdrive - IF1 send telegram free interconnection via BICO (p0922 = 999) - 2470 -
  

PROFIdrive - IF1 status words free interconnection - 2472 -
  

PROFIdrive - IF1 receive telegram free interconnection via BICO (p0922 = 999) - 2481 -
  

PROFIdrive - IF1 send telegram free interconnection via BICO (p0922 = 999) - 2483 -
  

PROFIdrive - IF2 receive telegram free interconnection - 2485 -
  

PROFIdrive - IF2 send telegram free interconnection - 2487 -
  

PROFIdrive - IF2 status words free interconnection - 2489 -
  

PROFIdrive - IF2 receive telegram free interconnection - 2491 -
  

PROFIdrive - IF2 send telegram free interconnection - 2493 -
  

PROFIdrive - CU_STW1 control word 1 Control Unit interconnection - 2495 -
  

PROFIdrive - CU_ZSW1 status word 1 Control Unit interconnection - 2496 -
  

PROFIdrive - O_DIGITAL interconnection - 2497 -
  

PROFIdrive - I_DIGITAL interconnection - 2498 -
  

PROFIdrive - O_DIGITAL_1 interconnection - 2499 -
  

PROFIdrive - I_DIGITAL_1 interconnection - 2500 -
  

Internal control/status words - Control word sequence control - 2501 -
  

Internal control/status words - Status word sequence control - 2503 -
  

Internal control/status words - Control word setpoint channel - 2505 -
  

Internal control/status words - Control word speed controller - 2520 -
  

Internal control/status words - Status word speed controller - 2522 -
  

Internal control/status words - Status word closed-loop control - 2526 -
  

Internal control/status words - Status word current control - 2530 -
  

Internal control/status words - Status word monitoring functions 1 - 2534 -
  

Internal control/status words - Status word monitoring functions 2 - 2536 -
  

Internal control/status words - Status word monitoring functions 3 - 2537 -
  

Internal control/status words - Control word faults/alarms - 2546 -
  

Internal control/status words - Status word faults/alarms 1 and 2 - 2548 -
  

Sequence control - Sequencer - 2610 -
  

Sequence control - Missing enables, line contactor control, logic operation - 2634 -
  

Brake control - Simple brake control (r0108.14 = 0) - 2701 -
  

Brake control - Extended brake control, standstill detection (r0108.14 = 1) - 2704 -
  

Brake control - Extended brake control, open/close brake (r0108.14 = 1) - 2707 -
  

Brake control - Extended brake control, signal outputs (r0108.14 = 1) - 2711 -
  

SI Basic Functions - Parameter manager - 2800 -
  

SI Basic Functions - Monitoring functions and faults/alarms - 2802 -
  

SI Basic Functions - SI status CU, MM, CU + MM, group STO - 2804 -
  

SI Basic Functions - S_STW1/2 safety control word 1/2, S_ZSW1/2 safety status word 1/2 - 2806 -
  

SI Basic Functions - STO (Safe Torque Off), SS1 (Safe Stop 1) - 2810 -
  

SI Basic Functions - STO (Safe Torque Off), safe pulse suppression - 2811 -
  

SI Basic Functions - SBC (Safe Brake Control), SBA (Safe Brake Adapter) - 2814 -
  

SI Extended Functions - Parameter manager - 2818 -
  

SI Extended Functions - SS1, SS2, SOS, internal STOP B, C, D, F - 2819 -
  

SI Extended Functions - SLS (Safely-Limited Speed) - 2820 -
  

SI Extended Functions - Safe referencing - 2821 -
  

SI Advanced Functions - SLP (Safely-Limited Position) - 2822 -
  

SI Extended Functions - SSM (Safe Speed Monitor) - 2823 -
  

SI Extended Functions - SDI (Safe Direction) - 2824 -
  

SI Extended Functions - SAM (Safe Acceleration Monitor), SBR (Safe Brake Ramp) - 2825 -
  

SI Advanced Functions - SCA (Safe Cam) - 2826 -
  

SI Extended Functions - SBT (Safe Brake Test) - 2836 -
  

SI Extended Functions - Selection of the active control word - 2837 -
  

SI Extended Functions - SLA (Safely-Limited Acceleration) - 2838 -
  

SI Extended Functions - SI Motion drive-integrated control signals / status signals - 2840 -
  

SI Extended Functions - S_STW1 safety control word 1, S_ZSW1 safety status word 1 - 2842 -
  

SI Extended Functions - S_STW2 safety control word 2, S_ZSW2 safety status word 2 - 2843 -
  

SI Advanced Functions - S_ZSW_CAM1 Safety status word Safe Cam 1 - 2844 -
  

SI Extended Functions - Control via PROFIsafe (p9601.2 = p9601.3 = 1) - 2858 -
  

SI Extended Functions - CU310-2 (F-DI 0 ... F-DI 2) - 2870 -
  

SI Extended Functions - CU310-2 failsafe digital output (F-DO 0) - 2873 -
  

SI Extended Functions - CU310-2 control interface - 2875 -
  

SI Extended Functions - CU310-2 Safe State selection - 2876 -
  

SI Extended Functions - CU310-2 assignment (F-DO 0) - 2877 -
  

SI TM54F - Overview - 2890 -
  

SI TM54F - Parameter manager - 2891 -
  

SI TM54F - Configuration, F-DI/F-DO test - 2892 -
  

SI TM54F - Failsafe digital inputs (F-DI 0 ... F-DI 4) - 2893 -
  

SI TM54F - Failsafe digital inputs (F-DI 5 ... F-DI 9) - 2894 -
  

SI TM54F - Failsafe digital outputs (F-DO 0 ... 3), digital inputs (DI 20 ... 23) - 2895 -
  

SI TM54F - Basic Functions control interface (p9601.2/3 = 0 & p9601.6 = 1) - 2900 -
  

SI TM54F - Basic Functions Safe State selection - 2901 -
  

SI TM54F - Basic Functions assignment (F-DO 0 ... F-DO 3) - 2902 -
  

SI TM54F - Extended Functions control interface (p9601.2 = 1 & p9601.3 = 0) - 2905 -
  

SI TM54F - Extended Functions Safe State selection - 2906 -
  

SI TM54F - Extended Functions assignment (F-DO 0 ... F-DO 3) - 2907 -
  

SI PROFIsafe - Standard telegrams - 2915 -
  

SI PROFIsafe - Manufacturer-specific telegrams - 2917 -
  

Setpoint channel - Overview - 3001 -
  

Setpoint channel - Fixed speed setpoints - 3010 -
  

Setpoint channel - Motorized potentiometer - 3020 -
  

Setpoint channel - Main setpoint / additional setpoint, setpoint scaling, jog - 3030 -
  

Setpoint channel - Direction limitation and direction reversal - 3040 -
  

Setpoint channel - Skip frequency bands and speed limits - 3050 -
  

Setpoint channel - Basic ramp-function generator - 3060 -
  

Setpoint channel - Extended ramp-function generator - 3070 -
  

Setpoint channel - Ramp-function generator selection, -status word, -tracking - 3080 -
  

Setpoint channel - Extended Stop and Retract (ESR, r0108.9 = 1) - 3082 -
  

Encoder evaluation - Overview - 4702 -
  

Encoder evaluation - Raw signal acquisition - 4704 -
  

Encoder evaluation - Speed actual value and pole position sensing encoder 1, n_act_filter 5 - 4715 -
  

Encoder evaluation - Absolute value for incremental encoder - 4750 -
  

Servo control - Online tuning with activated moment of inertia estimator (r0108.10 = 1) - 5045 -
  

Vector control - Closed-loop speed control and generation of the torque limits, overview - 6020 -
  

Vector control - Speed setpoint, droop - 6030 -
  

Vector control - Pre-control symmetrization reference/acceleration model - 6031 -
  

Vector control - Moment of inertia estimator (r0108.10 = 1) - 6035 -
  

Vector control - Speed controller with/without encoder - 6040 -
  

Vector control - Speed controller adaptation (Kp_n/Tn_n adaptation) - 6050 -
  

Vector control - Torque setpoint - 6060 -
  

Vector control - Vdc_max controller and Vdc_min controller - 6220 -
  

Vector control - U/f control, overview - 6300 -
  

Vector control - U/f characteristic and voltage boost - 6301 -
  

Vector control - Resonance damping and slip compensation - 6310 -
  

Vector control - Vdc_max controller and Vdc_min controller (U/f) - 6320 -
  

Vector control - Closed-loop speed control configuration - 6490 -
  

Vector control - Flux control configuration - 6491 -
  

Vector control - Excitation (SESM, p0300 = 5) - 6495 -
  

Vector control - Upper/lower torque limit - 6630 -
  

Vector control - Current/power/torque limits - 6640 -
  

Vector control - Current control, overview - 6700 -
  

Vector control - Current setpoint filter - 6710 -
  

Vector control - Iq and Id controllers - 6714 -
  

Vector control - Id setpoint (PMSM, p0300 = 2) - 6721 -
  

Vector control - Field weakening characteristic, Id setpoint (ASM, p0300 = 1) - 6722 -
  

Vector control - Field weakening controller, flux controller (ASM, p0300 = 1) - 6723 -
  

Vector control - Field weakening controller (PMSM, p0300 = 2) - 6724 -
  

Vector control - Interface to the Motor Module (ASM, p0300 = 1) - 6730 -
  

Vector control - Interface to the Motor Module (PMSM, p0300 = 2) - 6731 -
  

Vector control - Flux setpoint (RESM, p0300 = 6) - 6790 -
  

Vector control - Id setpoint (RESM, p0300 = 6) - 6791 -
  

Vector control - Interface to the Motor Module (RESM, p0300 = 6) - 6792 -
  

Vector control - Display signals - 6799 -
  

Technology functions - Friction characteristic - 7010 -
  

Technology functions - External armature short-circuit (EASC, p0300 = 2xx or 4xx) - 7014 -
  

Technology functions - Internal armature short-circuit (IASC, p0300 = 2xx or 4xx) - 7016 -
  

Technology functions - DC braking (p0300 = 1xx) - 7017 -
  

Technology functions - Synchronizing - 7020 -
  

Technology functions - Essential service mode (ESM) - 7033 -
  

General - 7200 -
  

Logic function blocks - AND (AND function block with 4 inputs) - 7210 -
  

Logic function blocks - OR (OR function block with 4 inputs) - 7212 -
  

Logic function blocks - XOR (XOR function block with 4 inputs) - 7214 -
  

Logic function blocks - NOT (inverter) - 7216 -
  

Arithmetic function blocks - ADD (adder with 4 inputs), SUB (subtractor) - 7220 -
  

Arithmetic function blocks - MUL (multiplier), DIV (divider) - 7222 -
  

Arithmetic function blocks - AVA (absolute value generator) - 7224 -
  

Timing function blocks - MFP (pulse generator), PCL (pulse contractor) - 7230 -
  

Timing function blocks - PDE (ON delay device), PDF (OFF delay device) - 7232 -
  

Timing function blocks - PST (pulse stretcher) - 7234 -
  

Memory function blocks - RSR (RS flip-flop), DFR (D flip-flop) timing function blocks - PST (pulse stretcher) - 7240 -
  

Switch function blocks - BSW (binary switch), NSW (numeric switch) - 7250 -
  

Control function blocks - LIM (limiter) - 7260 -
  

Control function blocks - PT1 (smoothing element) - 7262 -
  

Control function blocks - INT (integrator), DIF (derivative-action element) - 7264 -
  

Complex function blocks - LVM (double-sided limit monitor with hysteresis) - 7270 -
  

Technology controller - Fixed values, binary selection (r0108.16 = 1 and p2216 = 2) - 7950 -
  

Technology controller - Fixed values, direct selection (r0108.16 = 1 and p2216 = 1) - 7951 -
  

Technology controller - Motorized potentiometer (r0108.16 = 1) - 7954 -
  

Technology controller - Closed-loop control (r0108.16 = 1) - 7958 -
  

Technology controller - Kp/Tn adaptation (r0108.16 = 1) - 7959 -
  

Technology controller - DC link voltage controller (r0108.16 = 1) - 7960 -
  

Signals and monitoring functions - Overview - 8005 -
  

Signals and monitoring functions - Speed messages 1 - 8010 -
  

Signals and monitoring functions - Speed messages 2 - 8011 -
  

Signals and monitoring functions - Torque messages, motor blocked/stalled - 8012 -
  

Signals and monitoring functions - Load monitoring (r0108.17 = 1) - 8013 -
  

Signals and monitoring functions - Motor temperature status word faults/alarms - 8015 -
  

Signals and monitoring functions - Thermal monitoring motor - 8016 -
  

Signals and monitoring functions - Motor temperature model 1 (I2t) - 8017 -
  

Signals and monitoring functions - Motor temperature model 2 - 8018 -
  

Signals and monitoring functions - Motor temperature model 3 - 8019 -
  

Signals and monitoring functions - Thermal monitoring power unit - 8021 -
  

Signals and monitoring functions - Freely parameterized I2t monitoring (SESM) - 8022 -
  

Diagnostics - Overview - 8050 -
  

Diagnostics - Fault buffer - 8060 -
  

Diagnostics - Alarm buffer - 8065 -
  

Diagnostics - Faults/alarms trigger word (r2129) - 8070 -
  

Diagnostics - Faults/alarms configuration - 8075 -
  

Diagnostics - Measuring sockets (T0, T1, T2) - 8134 -
  

Diagnostics - Recorder overview (r0108.5 = 1) - 8144 -
  

Diagnostics - Recorder sequence control (r0108.5 = 1) - 8145 -
  

Data sets - Command data sets (CDS) - 8560 -
  

Data sets - Drive data sets (DDS) - 8565 -
  

Data sets - Encoder data sets (EDS) - 8570 -
  

Data sets - Motor data sets (MDS) - 8575 -
  

Data sets - Power unit data sets (PDS) - 8580 -
  

Basic Infeed - Overview - 8710 -
  

Basic Infeed - Control word sequence control infeed - 8720 -
  

Basic Infeed - Status word sequence control infeed - 8726 -
  

Basic Infeed - Sequencer - 8732 -
  

Active Infeed - Missing enables, line contactor control - 8738 -
  

Basic Infeed - Interface to the Basic Infeed power unit (control signals, actual values) - 8750 -
  

Basic Infeed - Signals and monitoring functions - 8760 -
  

Terminal Board 30 (TB30) - Overview - 9099 -
  

Terminal Board 30 (TB30) - Electrically isolated digital inputs (DI 0 ... DI 3) - 9100 -
  

Terminal Board 30 (TB30) - Electrically isolated digital outputs (DO 0 ... DO 3) - 9102 -
  

Terminal Board 30 (TB30) - Analog inputs (AI 0 ... AI 1) - 9104 -
  

Terminal Board 30 (TB30) - Analog outputs (AO 0 ... AO 1) - 9106 -
  

Communication Board CAN10 (CBC10) - Receive telegram free PDO mapping (p8744 = 2) - 9204 -
  

Communication Board CAN10 (CBC10) - Receive telegram predef conn set (p8744 = 1) - 9206 -
  

Communication Board CAN10 (CBC10) - Send telegram free PDO mapping (p8744 = 2) - 9208 -
  

Communication Board CAN10 (CBC10) - Send telegram predef conn set (p8744 = 1) - 9210 -
  

Communication Board CAN10 (CBC10) - Control word CANopen - 9220 -
  

Communication Board CAN10 (CBC10) - Status word CANopen - 9226 -
  

Terminal Module 15 (TM15) - Overview TM15 (SIMOTION) - 9389 -
  

Terminal Module 15 (TM15) - Overview TM15DI_DO (SINAMICS) - 9399 -
  

Terminal Module 15 (TM15) - Digital inputs/outputs bidirectional (DI/DO 0 ... DI/DO 7) - 9400 -
  

Terminal Module 15 (TM15) - Digital inputs/outputs bidirectional (DI/DO 8 ... DI/DO 15) - 9401 -
  

Terminal Module 15 (TM15) - Digital inputs/outputs bidirectional (DI/DO 16 ... DI/DO 23) - 9402 -
  

Terminal Module 17 High Feature (TM17 High Feature) - Overview - 9419 -
  

Terminal Module 31 (TM31) - Overview - 9549 -
  

Terminal Module 31 (TM31) - Electrically isolated digital inputs (DI 0 ... DI 3) - 9550 -
  

Terminal Module 31 (TM31) - Electrically isolated digital inputs (DI 4 ... DI 7) - 9552 -
  

Terminal Module 31 (TM31) - Electrically isolated digital relay outputs (DO 0 ... DO 1) - 9556 -
  

Terminal Module 31 (TM31) - Digital inputs/outputs, bidirectional (DI/DO 8 ... DI/DO 9) - 9560 -
  

Terminal Module 31 (TM31) - Digital inputs/outputs, bidirectional (DI/DO 10 ... DI/DO 11) - 9562 -
  

Terminal Module 31 (TM31) - Analog input 0 (AI 0) - 9566 -
  

Terminal Module 31 (TM31) - Analog input 1 (AI 1) - 9568 -
  

Terminal Module 31 (TM31) - Analog outputs (AO 0 ... AO 1) - 9572 -
  

Terminal Module 31 (TM31) - Temperature evaluation - 9576 -
  

Terminal Module 31 (TM31) - Sensor monitoring KTY/PTC/PT1000 - 9577 -
  

Terminal Module 120 (TM120) - Temperature evaluation channels 0 and 1 - 9605 -
  

Terminal Module 120 (TM120) - Temperature evaluation channels 2 and 3 - 9606 -
  

Terminal Module 150 (TM150) - Temperature evaluation structure (channel 0 … 11) - 9625 -
  

Terminal Module 150 (TM150) - Temperature evaluation 1x2-, 3-, 4-wire (channels 0 … 5) - 9626 -
  

Terminal Module 150 (TM150) - Temperature evaluation 2x2-wire (channels 0 … 11) - 9627 -
  

Terminal Module 41 (TM41) - Overview - 9659 -
  

Terminal Module 41 (TM41) - Electrically isolated digital inputs (DI 0 ... DI 3) - 9660 -
  

Terminal Module 41 (TM41) - Digital inputs/outputs, bidirectional (DI/DO 0 ... DI/DO 1) - 9661 -
  

Terminal Module 41 (TM41) - Digital inputs/outputs, bidirectional (DI/DO 2 ... DI/DO 3) - 9662 -
  

Terminal Module 41 (TM41) - Analog input 0 (AI 0) - 9663 -
  

Terminal Module 41 (TM41) - Incremental encoder emulation (p4400 = 0) - 9674 -
  

Terminal Module 41 (TM41) - Incremental encoder emulation (p4400 = 1) - 9676 -
  

Terminal Module 41 (TM41) - STW1 control word interconnection (p0922 = 3) - 9677 -
  

Terminal Module 41 (TM41) - Control word sequence control (p4400 = 0) - 9678 -
  

Terminal Module 41 (TM41) - STW2 control word interconnection (p0922 = 3) - 9679 -
  

Terminal Module 41 (TM41) - Status word sequence control - 9680 -
  

Terminal Module 41 (TM41) - ZSW1 status word interconnection (p0922 = 3) - 9681 -
  

Terminal Module 41 (TM41) - Sequencer (p4400 = 0) - 9682 -
  

Terminal Module 41 (TM41) - ZSW2 status word interconnection (p0922 = 3) - 9683 -
  

Voltage Sensing Module (VSM) - Analog inputs (AI 0 ... AI 3) - 9880 -
  

Voltage Sensing Module (VSM) - Temperature evaluation - 9886 -
  

Basic Operator Panel 20 (BOP20) - Control word interconnection - 9912 -
