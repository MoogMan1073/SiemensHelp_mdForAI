---
title: "FBD (S7-300, S7-400)"
package: ProgFUP34enUS
topics: 143
...
- [&: AND logic operation (S7-300, S7-400)](#and-logic-operation-s7-300-s7-400)
- [AND truth table (S7-300, S7-400)](#and-truth-table-s7-300-s7-400)
- [>=1: OR logic operation (S7-300, S7-400)](#1-or-logic-operation-s7-300-s7-400)
- [OR truth table (S7-300, S7-400)](#or-truth-table-s7-300-s7-400)
- [X: EXCLUSIVE OR logic operation (S7-300, S7-400)](#x-exclusive-or-logic-operation-s7-300-s7-400)
- [EXCLUSIVE OR truth table (S7-300, S7-400)](#exclusive-or-truth-table-s7-300-s7-400)
- [Insert input (S7-300, S7-400)](#insert-input-s7-300-s7-400)
- [Invert RLO (S7-300, S7-400)](#invert-rlo-s7-300-s7-400)
...
### &: AND logic operation (S7-300, S7-400)
...
| --- | --- | --- | --- | --- |
| <Operand> | Input | BOOL | I, Q, M, T, C, D, L | The operand indicates the bit whose signal state will be queried. |

#### Example

The following example shows how the instruction works:

![Example](images/71434120075_DV_resource.Stream@PNG-de-DE.png)
...
[FBD programming examples](Creating%20FBD%20programs.md#fbd-programming-examples)
  
[Basic information on FBD](Creating%20FBD%20programs.md#basic-information-on-fbd)

### AND truth table (S7-300, S7-400)
...
### >=1: OR logic operation (S7-300, S7-400)
...
![Example](images/71440089611_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/71440401547_DV_resource.Stream@PNG-de-DE.png)
...
### EXCLUSIVE OR truth table (S7-300, S7-400)
...
![Example](images/71440662283_DV_resource.Stream@PNG-de-DE.png)

The box of the instruction was extended by an additional input at which the signal state of the "TagIn_3" operand is queried. The "TagOut" output is set, when the operands "TagIn_1" and "TagIn_2" and "TagIn_3" return signal state "1".
...
![Example](images/71442011019_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/71442284811_DV_resource.Stream@PNG-de-DE.png)
...
| Parameters | Declaration | Data type | Memory area | Description |
| --- | --- | --- | --- | --- |
| <Operand> | Output | BOOL | I, Q, M, T, C, D, L | Operand that is reset if RLO = "1". |

#### Example

The following example shows how the instruction works:

![Example](images/71674395915_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/71674806027_DV_resource.Stream@PNG-de-DE.png)
...
| Parameters | Declaration | Data type | Memory area | Description |
| --- | --- | --- | --- | --- |
| S | Input | BOOL | I, Q, M, D, L, T, C | Enable setting |
...
![Example](images/71450252555_DV_resource.Stream@PNG-de-DE.png)
...
| Parameters | Declaration | Data type | Memory area | Description |
| --- | --- | --- | --- | --- |
| R | Input | BOOL | I, Q, M, T, C, D, L | Enable resetting |
| S1 | Input | BOOL | I, Q, M, T, C, D, L | Enable setting |
...
![Example](images/71450602891_DV_resource.Stream@PNG-de-DE.png)
...
| Parameters | Declaration | Data type | Memory area | Description |
| --- | --- | --- | --- | --- |
| <Operand1> | Input | BOOL | I, Q, M, T, C, D, L | Signal to be scanned |
...
![Example](images/71451247627_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/71529560203_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/71678439179_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/71679346827_DV_resource.Stream@PNG-de-DE.png)
...
You can use the "Generate pulse" instruction to set output Q for a programmed duration. The instruction is started when the result of logic operation (RLO) at input IN changes from "0" to "1" (positive signal edge). The execution of the instruction requires a preceding logic operation. It can be placed within or at the end of the network. The programmed time PT begins when the instruction starts. Output Q is set for the duration PT, regardless of the subsequent course of the input signal. Even when a new positive signal edge is detected, the signal state of the Q output is not affected as long as the PT duration is running.

You can query the current time value at the ET output. The time value starts at T#0s and ends when the value of the time duration PT is reached. If the time duration PT is reached and the signal state at input IN is "0", the ET output is reset.

Each call of the "Generate pulse" instruction must be assigned an IEC Timer in which the instruction data is stored. You can declare an IEC Timer as follows:
...
| Parameters | Declaration | Data type | Memory area | Description |
| --- | --- | --- | --- | --- |
| IN | Input | BOOL | I, Q, M, D, L, T, C | Start input |
...
You can use the "Generate on-delay" instruction to delay setting of the Q output by the time configured with the PT time. The instruction is started when the result of logic operation (RLO) at input IN changes from "0" to "1" (positive signal edge). The execution of the instruction requires a preceding logic operation. It can be placed within or at the end of the network. The programmed time PT begins when the instruction starts. When the duration PT expires, output Q has the signal state "1". Output Q remains set as long as the start input is still "1". When the signal state at the start input changes from "1" to "0", the Q output is reset. The timer function is started again when a new positive signal edge is detected at the start input.

You can query the current time value at the ET output. The timer value starts at T#0s and ends when the value of the time duration PT is reached. The ET output is reset as soon as the signal state at the IN input changes to "0".

Each call of the "Generate on-delay" instruction must be assigned an IEC Timer in which the instruction data is stored. You can declare an IEC Timer as follows:
...
You can use the "Generate off-delay" instruction to delay resetting of the Q output by the time configured with the PT time. The output Q is set when the result of logic operation (RLO) at input IN changes from "0" to "1" (positive signal edge). The execution of the instruction requires a preceding logic operation. It can be placed within or at the end of the network. When the signal state at input IN changes back to "0", the programmed time PT starts. Output Q remains set as long as the time duration PT is running. When the PT time duration expires, the Q output is reset. If the signal state at input IN changes to "1" before the PT time duration expires, the timer is reset. The signal state at the output Q will continue to be "1".

You can query the current time value at the ET output. The current timer value starts at T#0s and ends when the value of the time duration PT is reached. When the time duration PT expires, the ET output remains set to the current value until input IN changes back to "1". If input IN changes to "1" before the time duration PT has expired, the ET output is reset to the value T#0s.
...
You can find an example of how a time cell can be structured under: See also "L: Load timer value".
...
| R | Input | BOOL | I, Q, M, T, C, D, L, P | Reset input |
...
![Pulse timing diagram](images/44572673803_DV_resource.Stream@PNG-en-US.png)

##### Example

The following example shows how the instruction works:

![Example](images/44572995595_DV_resource.Stream@PNG-en-US.png)
...
![Pulse timing diagram](images/22562789003_DV_resource.Stream@PNG-en-US.png)

##### Example

The following example shows how the instruction works:

![Example](images/44572683275_DV_resource.Stream@PNG-en-US.png)
...
![Pulse timing diagram](images/22405938955_DV_resource.Stream@PNG-en-US.png)

##### Example

The following example shows how the instruction works:

![Example](images/44572678155_DV_resource.Stream@PNG-en-US.png)
...
![Pulse timing diagram](images/22407772555_DV_resource.Stream@PNG-en-US.png)

##### Example

The following example shows how the instruction works:

![Example](images/44573000587_DV_resource.Stream@PNG-en-US.png)
...
![Pulse timing diagram](images/22410524939_DV_resource.Stream@PNG-en-US.png)

##### Example

The following example shows how the instruction works:

![Example](images/44573210507_DV_resource.Stream@PNG-en-US.png)
...
The duration is made up internally of a time value and a time base and is programmed at parameter TV. When the instruction is started, the programmed timer value is counted down towards zero. The time base indicates the time period with which the time value is changed.
...
![Example](images/108809184651_DV_resource.Stream@PNG-en-US.png)
...
![Example](images/108809561355_DV_resource.Stream@PNG-en-US.png)
...
> In the time cell, the operating system reduces the time value in an interval specified by the time base by one unit until the value equals "0". The decrementation is performed asynchronously to the user program. The resulting timer is therefore at maximum up to one time interval shorter than the desired time base.
>
> You can find an example of how a time cell can be structured under: See also [L: Load timer value](STL%20%28S7-300%2C%20S7-400%29.md#l-load-timer-value-s7-300-s7-400).
...
![Example](images/121368711947_DV_resource.Stream@PNG-en-US.png)
...
![Example](images/96167398923_DV_resource.Stream@PNG-en-US.png)
...
![Example](images/96167424779_DV_resource.Stream@PNG-en-US.png)
...
![Example](images/108809566603_DV_resource.Stream@PNG-en-US.png)
...
![Example](images/108809584907_DV_resource.Stream@PNG-en-US.png)
...
You can use the "Count up" instruction to increment the value at output CV. When the signal state at the CU input changes from "0" to "1" (positive signal edge), the instruction is executed and the current counter value at the CV output is incremented by one. The counter value is increased each time a positive signal edge is detected, until it reaches the high limit of the data type (INT). When the high limit is reached, the signal state at the CU input no longer has an effect on the instruction.

You can query the counter status in the Q output. The signal state at the Q output is determined by the parameter PV. If the current counter value is greater than or equal to the value of the PV parameter, the Q output is set to signal state "1". In all other cases, the Q output has signal state "0".
...
| LD | Input | BOOL | I, Q, M, D, L, T, C, P | Load input |
...
![Example](images/30494416267_DV_resource.Stream@PNG-de-DE.png)
...
| R | Input | BOOL | I, Q, M, D, L, T, C, P | Reset input |
| LD | Input | BOOL | I, Q, M, D, L, T, C, P | Load input |
| PV | Input | INT | I, Q, M, D, L, P, or constant | Value at which the output QU is set. / Value to which the CV output is set with LD = 1. |
...
The "Assign parameters and count up" instruction requires a preceding logic operation for the edge evaluation and can be placed within or at the end of the network.
...
![Example](images/20502889611_DV_resource.Stream@PNG-en-US.png)
...
[Inserting FBD elements](Creating%20FBD%20programs.md#inserting-fbd-elements)
  
[Editing FBD elements](Creating%20FBD%20programs.md#editing-fbd-elements)
  
[Inserting operands in FBD instructions](Creating%20FBD%20programs.md#inserting-operands-in-fbd-instructions)
...
The "Assign parameters and count down" instruction requires a preceding logic operation for the edge evaluation and can be placed within or at the end of the network.
...
![Example](images/20503124363_DV_resource.Stream@PNG-en-US.png)
...
The counter value can be incremented until the high limit of "999" is reached. When the high limit value is reached, the counter value is no longer incremented on a positive signal edge. When the low limit of "0" is reached, the counter value is no longer decremented.
...
The "Assign parameters and count up / down" instruction requires a preceding logic operation for the edge evaluation and can be placed within or at the end of the network.
...
![Example](images/20502885259_DV_resource.Stream@PNG-en-US.png)
...
![Example](images/58720212491_DV_resource.Stream@PNG-en-US.png)
...
![Example](images/41334071051_DV_resource.Stream@PNG-en-US.png)
...
![Example](images/41334463627_DV_resource.Stream@PNG-en-US.png)
...
| Parameter | Declaration | Data type | Memory area | Description |
| --- | --- | --- | --- | --- |
| IN1 | Input | Bit strings, integers, floating-point numbers | I, Q, M, D, L, P or constant | First comparison value |
| IN2 | Input | Bit strings, integers, floating-point numbers | I, Q, M, D, L, P or constant | Second value to compare |
...
![Example](images/20511419147_DV_resource.Stream@PNG-de-DE.png)
...
[Selecting the data type of an FBD element](Creating%20FBD%20programs.md#selecting-the-data-type-of-an-fbd-element)
...
![Example](images/20511572235_DV_resource.Stream@PNG-de-DE.png)
...
| Parameters | Declaration | Data type | Memory area | Description |
| --- | --- | --- | --- | --- |
| IN1 | Input | Integers, floating-point numbers | I, Q, M, D, L, P or constant | First comparison value |
| IN2 | Input | Integers, floating-point numbers | I, Q, M, D, L, P or constant | Second value to compare |
...
![Example](images/20511584523_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/20192647819_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/20193124619_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/20193804683_DV_resource.Stream@PNG-de-DE.png)
...
| Parameters | Declaration | Data type | Memory area | Description |
| --- | --- | --- | --- | --- |
| EN | Input | BOOL | I, Q, M, T, C, D, L | Enable input |
...
![Example](images/71580007563_DV_resource.Stream@PNG-de-DE.png)
...
| IN2 | Input | Integers, floating-point numbers | I, Q, M, D, L, P or constant | Subtrahend |
...
![Example](images/71579951371_DV_resource.Stream@PNG-de-DE.png)
...
| IN2 | Input | Integers, floating-point numbers | I, Q, M, D, L, P or constant | Multiplicand |
...
![Example](images/71589165451_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/71589626635_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/71590256139_DV_resource.Stream@PNG-de-DE.png)
...
- Enable input EN has the signal state "0".
- The result of the instruction is outside the valid range for the data type specified at the OUT output (only applies to integers).
...
![Example](images/71598877323_DV_resource.Stream@PNG-de-DE.png)

If the operand "TagIn" has the signal state "1", the "Create twos complement" instruction is executed. The sign of the value at input "TagIn_Value" is changed and the result is provided at output "TagOut_Value". If the instruction is executed without errors, the enable output ENO has the signal state "1" and the "TagOut" output is set.
...
![Example](images/71600238731_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/71645145611_DV_resource.Stream@PNG-de-DE.png)
...
[Removing inputs and outputs (S7-1200, S7-1500)](Creating%20LAD%20programs.md#removing-inputs-and-outputs-s7-1200-s7-1500)
...
![Example](images/71647016843_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/71652994571_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/71653866891_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/71656443659_DV_resource.Stream@PNG-de-DE.png)
...
You can use the "Form natural logarithm" instruction to calculate the natural logarithm to base e (e = 2718282) of the value at input IN. The result is sent to output OUT and can be queried there. The instruction has a positive result if the input value is greater than zero. If input values are less than zero, output OUT returns an invalid floating-point number.
...
![Example](images/71657724939_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/71711089675_DV_resource.Stream@PNG-de-DE.png)
...
| EN | Input | BOOL | I, Q, M, D, L, T, C | Enable input |
| ENO | Output | BOOL | I, Q, M, D, L | Enable output |
| IN | Input | Floating-point numbers | I, Q, M, D, L, P or constant | Size of the angle in radians |
...
![Example](images/71711244555_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/71711437835_DV_resource.Stream@PNG-de-DE.png)
...
You can use the "Form tangent value" instruction to calculate the tangent of an angle. The size of the angle is specified in the radian measure at input IN. The result of the instruction is output at the OUT output and can be queried there.
...
![Example](images/71711567115_DV_resource.Stream@PNG-de-DE.png)
...
If the "TagIn" operand has signal state "1", the instruction is executed. The instruction calculates the tangent of the angle specified at input "Tag_Value" and stores the result at output "Tag_Result". If the instruction is executed without errors, the "TagOut" output is set.
...
You can use the "Form arcsine value" instruction to calculate the size of the angle from the sine value specified at input IN, which corresponds to this value. Only valid floating-point numbers within the range -1 to +1 can be specified at input IN. The calculated angle size is output in radians at the OUT output and can range in value from -π/2 to +π/2.
...
| OUT | Output | Floating-point numbers | I, Q, M, D, L, P | Size of the angle in radians |
...
![Example](images/71711696395_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/71711889675_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/71712240651_DV_resource.Stream@PNG-de-DE.png)
...
You can also use the "Move block" (BLKMOV) and "Move block uninterruptible" (UBLKMOV) instructions to copy fields and structures.
...
![Example](images/71717460363_DV_resource.Stream@PNG-de-DE.png)
...
If the "TagIn" operand has signal state "1", the instruction is executed. The instruction copies the content of the operand "TagIn_Value" to the operand "TagOut_Value" and set output "TagOut" to signal state "1".
...
If a block of data type BOOL is moved, the specified length of the area must be divisible by 8, otherwise it will not be possible to execute the instruction.
...
![Example](images/71751580171_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/71751917451_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/71752191627_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/71752594443_DV_resource.Stream@PNG-de-DE.png)
...
You can use the "Round numerical value" instruction to round the value at input IN to the nearest integer. The instruction interprets the value at input IN as a floating-point number and converts this to the next higher integer of the data type DINT. If the input value is exactly between an even and odd number, the even number is selected. The result of the instruction is output at the OUT output and can be queried there.
...
| OUT | Output | DINT | I, Q, M, D, L, P | Result of the rounding |
...
![Example](images/71753291019_DV_resource.Stream@PNG-de-DE.png)
...
| ENO | Output | BOOL | I, Q, M, D, L | Enable output |
| IN | Input | Floating-point numbers | I, Q, M, D, L, P or constant | Input value |
| OUT | Output | Integers, floating-point numbers | I, Q, M, D, L, P | Result with the next higher integer |
...
![Example](images/71756699531_DV_resource.Stream@PNG-de-DE.png)
...
| ENO | Output | BOOL | I, Q, M, D, L | Enable output |
| IN | Input | Floating-point numbers | I, Q, M, D, L, P or constant | Input value |
| OUT | Output | Integers, floating-point numbers | I, Q, M, D, L, P | Result with the next lower integer |
...
![Example](images/71758772747_DV_resource.Stream@PNG-de-DE.png)
...
| EN | Input | BOOL | I, Q, M, T, C, D, L | Enable input |
| ENO | Output | BOOL | I, Q, M, D, L | Enable output |
| IN | Input | REAL | I, Q, M, D, L or constant | Input value |
| OUT | Output | Integers | I, Q, M, D, L | Integer component of the input value |
...
![Example](images/71760849931_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/71765456779_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/71766397579_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/71766829963_DV_resource.Stream@PNG-en-US.png)
...
![Example](images/71767262859_DV_resource.Stream@PNG-en-US.png)
...
![Example](images/71767732875_DV_resource.Stream@PNG-en-US.png)

If the "TagIn_1" operand has signal state "1", the instruction is executed. The linear execution of the program is interrupted and continues in Network 3, which is identified by the jump label CAS1. If the "TagIn_3" input has signal state "1", the "TagOut_3" output is set.
...
- If the result of logic operation (RLO) at the input of the "Return" instruction is "1", the program execution is ended in the currently called program block and continued after the call function in the calling program block (for example, in the calling organization block). The signal state of the enable output ENO of the call function is determined by the RLO. The RLO is shown via the instruction as a fixed parameter value.
...
![Example](images/22441823371_DV_resource.Stream@PNG-de-DE.png)

If one of the operands "TagIn_1" or "TagIn_2" have signal state "1", the instruction is executed. Program execution is ended in the called program block and continues in the calling program block. The enable output ENO of the call function is reset to signal state "1".
...
You can use the "Open global data block" instruction to open a global data block (DB). The number of the data block is transferred to the DB register. Subsequent DB commands access the relevant blocks depending on the content of the registers.
...
| Parameters | Declaration | Data type | Memory area | Description |
| --- | --- | --- | --- | --- |
| <data block> | Input | DB_ANY | - | Data block to be opened |
...
![Example](images/22442063115_DV_resource.Stream@PNG-de-DE.png)

Network 2

![Example](images/71769393931_DV_resource.Stream@PNG-de-DE.png)
...
You can use the "Open instance data block" instruction to open a data block (DB). The number of the data block is transferred to the DI register. Subsequent DI commands access the relevant blocks depending on the content of the registers.
...
![Example](images/22442957963_DV_resource.Stream@PNG-en-US.png)
...
If an externally started compression operation has already been executed, the calling of the "Compress CPU memory" instruction will cause an error to be displayed.
...
| 8092 | The instruction cannot be executed, because one of the following conditions applies:  - The "Delete block" function is active. - A test and commissioning function affects a block (for  example, status). - The "Copy blocks" function is enabled by an external trigger. - The H-CPU is running a linking or update routine. |
| 8xyy | For more information on errors, refer to "See also". |
| *The error codes can be displayed as integer or hexadecimal values in the program editor. You will find information on changing display formats in "See also". |  |
...
| - Cold restart - Warm restart | The operating system sets the protection level to "1". All programming device functions are permitted.  If required, you can set the protection level in your program to "2" or "3" by calling the "Change protection level" instruction. |
| Buffered POWER ON | Remains unchanged |
| Operating mode transition RUN/STARTUP/HOLD -> STOP (by calling the instruction "Exit program" (STP), due to a program error without corresponding error OB or by an operator action). | Remains unchanged |
...
| EN | Input | BOOL | I, Q, M, D, L, T, C | Enable input |
| ENO | Output | BOOL | I, Q, M, D, L | Enable output |
| MODE | Input | WORD | I, Q, M, D, L or constant | Protection level  Possible values:  - W#16#0000: Setting protection level "1" - W#16#0001: Setting protection level "2" with password authentication - W#16#000C: Setting protection level "3" with password authentication   Constants can also be specified in the MODE parameter. |
...
- Disable the CiR process conditionally by specifying a high limit for the CiR synchronization time on the FRZ_TIME parameter. In this case, the download of a modified configuration from the programming device to the CPU is only accepted if the CiR synchronization time calculated in the CPU does not exceed the specified high limit.
- Enable CiR process The high limit of the CiR synchronization time is set to the default value 1 000 ms.
- Determine whether the CiR process is enabled. If the CiR process is enabled or conditionally enabled, you will get the current high limit for the CiR synchronization time in the A_FT parameter of the instruction.
...
| FRZ_TIME | Input | TIME | I, Q, M, D, L or constant | High limit of the CiR synchronization time in milliseconds.   Permitted range of values: 60 to 2500 ms (default: 1000 ms)  The FRZ_TIME parameter is only relevant when the value "3" is set on the MODE parameter. |
...
| Error code* (W#16#...) | Explanation |
| --- | --- |
| 0000 | Job executed without error  This error code can be output only if the value "1", "2", or "3" is set at the MODE parameter. |
...
![Example](images/71798864523_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/71802962443_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/71803773707_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/71804558603_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/71805640587_DV_resource.Stream@PNG-de-DE.png)

The following figure shows how the instruction works using concrete operand values:

![Example](images/20536746379_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/71806453003_DV_resource.Stream@PNG-de-DE.png)

The following figure shows how the instruction works using specific operand values:

![Example](images/20536734091_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/71806800267_DV_resource.Stream@PNG-de-DE.png)
...
You can use the "Shift right" instruction to shift the content of the operand at the input IN bit-by-bit to the right and query the result at the OUT output. The parameter N is used to specify the number of bit positions by which the specified value should be shifted.
...
![Description](images/20547345803_DV_resource.Stream@PNG-en-US.png)
...
![Example](images/71807717515_DV_resource.Stream@PNG-de-DE.png)
...
You can use the "Shift left" instruction to shift the content of the operand at the input IN bit-by-bit to the left and query the result at the OUT output. The parameter N is used to specify the number of bit positions by which the specified value should be shifted.

If the value at the parameter N is "0", the value at input IN is copied to the operand at output OUT.

If the value at the parameter N is greater than the number of bit positions, the operand value at input IN is shifted to the left by the available number of bit positions.
...
![Description](images/20547368971_DV_resource.Stream@PNG-en-US.png)
...
![Example](images/71820069003_DV_resource.Stream@PNG-de-DE.png)
...
You can use the "Rotate right" instruction to rotate the content of the operand at the input IN bit-by-bit to the right and query the result at the OUT output. The parameter N is used to specify the number of bit positions by which the specified value should be rotated. The bit positions freed by rotating are filled with the bit positions that are pushed out.
...
![Description](images/20547404427_DV_resource.Stream@PNG-en-US.png)
...
![Example](images/71820700811_DV_resource.Stream@PNG-de-DE.png)
...
You can use the "Rotate left" instruction to rotate the content of the operand at the input IN bit-by-bit to the left and query the result at the OUT output. The parameter N is used to specify the number of bit positions by which the specified value should be rotated. The bit positions freed by rotating are filled with the bit positions that are pushed out.
...
![Description](images/22590809995_DV_resource.Stream@PNG-en-US.png)
...
![Example](images/71822028811_DV_resource.Stream@PNG-de-DE.png)
...
- CC 0 and CC 1: Display bits

You determine the bits to be queried by selecting the appropriate symbols from the drop-down list of the instruction. The following table shows the options for querying the status bits:

| Symbol | Query | Explanation |
| --- | --- | --- |
| ==0 | CC 0 = 0 and CC 1 = 0 | The instruction queries whether the result of a mathematical function is equal to 0. |
| <>0 | CC 0 = 0 and CC 1 = 1  or  CC 0 = 0 and CC 1 = 0 | The instruction queries whether the result of a mathematical function is not equal to 0. |
| >=0 | CC 0 = 0 | The instruction queries whether the result of a mathematical function is greater than or equal to 0. |
| <=0 | CC 1 = 0 | The instruction queries whether the result of a mathematical function is less than or equal to 0. |
| >0 | CC 0 = 0 and CC 1 = 1 | The instruction queries whether the result of a mathematical function is greater than 0. |
| <0 | CC 0 = 1 and CC 1 = 0 | The instruction queries whether the result of a mathematical function is less than 0. |
| OV | CC 0 = 1 and CC 1 = 1 | The instruction queries whether the result of a mathematical function is invalid. An invalid result may be caused by an invalid floating-point number or a division by zero. |
| OV | OV = 1 | The instruction queries whether a number range overflow has occurred in an instruction. |
| OS | OS = 1 | The instruction queries whether a saving number range overflow has occurred in an instruction. |
| BR | BR = 1 | The instruction queries the signal state of the BR status bits. |

If the query is met, the instruction returns signal state "1". If the query is not met, the instruction returns signal state "0".

You can program a query for status bits "0" using the "Invert RLO" instruction.

#### Example

The following example shows how the instruction works:

![Example](images/24552177419_DV_resource.Stream@PNG-en-US.png)

If the operand "TagIn" has the signal state "1", the instruction "Multiply" (MUL) is executed. If an overflow occurs during the execution of the "Multiply" instruction, the OV status bit is set. After the query of the OV status bit, the "Set output" (S) instruction is executed in network 2 and the "TagOV" operand is set.
...
In the code section of a function (FC), you cannot specify any parameter of the data type BLOCK_FC as the operand for a conditional call. You can, however, specify a parameter of the type BLOCK_FC as the operand in a function block (FB).

A conditional call is then only executed if the result of logic operation (RLO) is "1". If a conditional call is not executed the RLO is "0" after the call instruction. If the instruction is executed, it works as follows:
...
- It shifts the MA bit (active MCR bit) to the block stack (B stack).
...
![Example](images/22452373899_DV_resource.Stream@PNG-de-DE.png)

The function FC3 is called if the operand "Tag_1" has the signal state "1".
...
Use the instruction "Save RLO in BR bit" to save the result of logic operation (RLO) in the binary result (BR). During processing, the instruction transfers the signal state of the current result of logic operation to the status bit BR. The instruction operates independent of the conditions and does not affect any other status bits.
...
Use the instruction "Save RLO in BR bit" to check the execution status of a block. If you program the "Save RLO in BR bit" instruction at the end of a block, for example, the BR bit is set to the signal state of the current result of logic operation of the block. If the block is executed without errors, the BR bit and the ENO enable output in the calling block are set to signal state "1". Signal state "0" is assigned to the BR bit if an error occurs during processing of the block. You can program a troubleshooting routine by querying the BR bit.

#### Example

The following example shows how the instruction works:

![Example](images/20315944075_DV_resource.Stream@PNG-de-DE.png)

The BR is set to signal state "1" when the "TagIn_1" and "TagIn_2" operands return signal state "1".
...
[Example of the use of the EN/ENO mechanism in FBD](Programming%20basics.md#example-of-the-use-of-the-eneno-mechanism-in-fbd)
...
![Example](images/20568696203_DV_resource.Stream@PNG-en-US.png)
...
The instruction "Close MCR ranges" closes an MCR dependent program section (MCR = Master Control Relay). In this case the instruction deletes the result of logic operation (RLO) from the last entry in the MCR stack.
...
Use the instruction "Enable MCR range" to specify the start of an MCR range (MCR = Master Control Relay). Within this range, you can switch MCR dependency on and off using the following instructions:
...
![Example](images/20570329867_DV_resource.Stream@PNG-en-US.png)
...
Use the instruction "Disable MCR range" to specify the start of an MCR range (MCR = Master Control Relay).
...
The start of the MCR range is programmed in network 1. If the "TagIn_1" operand returns the signal state "1", the MCR dependency is enabled. In this case, the signal state of the "TagIn_2" operand is assigned to the "TagOut" operand (network 3). If the "TagIn_1" operand returns signal state "0", MCR dependency is disabled and the "TagOut" operand is reset to "0". MCR dependency of the MCR range is disabled in network 4. The end of the MCR range is programmed in network 8.
...
![Example](images/43778336523_DV_resource.Stream@PNG-de-DE.png)
...
- The bits that are in the first and last bytes to be transferred and that do not belong to the defined bit array, get the value of the corresponding bits in the process image output table.
  ...
  | --- | --- |
  |  | **Warning** |
  | **Unintended behavior**  The setting of the bits at the value of the relevant bit in the process image output table can lead, for example, to lead to unintended reactions such as starting a motor or turning off a cooling system. |  |
...
Calling the "Set a bit array in the I/O area" instruction has no effect and the ENO enable output is set to "0" if one of the following conditions applies:
...
![Example](images/20576326411_DV_resource.Stream@PNG-de-DE.png)

If the "TagIn" operand returns the signal state "1", the instruction sets 8 bits in the I/O area based on outputs Q10.0 to Q10.7.
...
The "Set byte array" instruction sets the signal state of the bits in a specified area to "1". You define the start point of the array on the S_BYTE. You define the size of the array by specifying the number of bits to be set on the N parameter of the instruction.

> **Note**
>
> The value of the N parameter must be a multiple of the number 8 (for example, 8, 16, 24, etc.).
...
> **Note**
>
> The signal state of the corresponding bits in the process-image output table (Q) is reset to "0".
...
| N | Input | INT | I, Q, M, D, L, P or constant | Size of the array to be set  The array is specified by the number of bits as a multiple of 8 (for example 8, 16, 24 etc.). |

#### Example

The following example shows how the instruction works:

![Example](images/43778770571_DV_resource.Stream@PNG-de-DE.png)

If the "TagIn" operand returns the signal state "1", the instruction sets 2 bytes (16 bits) in the range from P2.0 to P3.7
...
The "Reset bit array" instruction sets the signal state of the bit in a specified area to "0". You use the pointer on the S_BIT parameter to define the starting point of the range. If the pointer points to the memory area of the external I/O, the instruction is not executed. At the N parameter, you specify the number of bits to be reset in the specified range. When the N parameter has the value "0", calling the instruction has no effect.
...
![Example](images/20576314763_DV_resource.Stream@PNG-de-DE.png)

If the "TagIn" operand has the signal state "1", the instruction resets 10 bits in the area from M0.0 to M1.2.
...
The instruction "Reset a bit array in the I/O area" resets the bits of an I/O area to the signal state "0". At the same time the corresponding bits in the process image of the outputs are also reset parallel to the I/O bits.
...
Calling the instruction "Reset a bit array in the I/O area" has no effect and enable output ENO is set to "0" if one of the following conditions applies:
...
The following table shows the parameters of the instruction "Reset a bit array in the I/O area":
...
![Example](images/20576350859_DV_resource.Stream@PNG-de-DE.png)
...
The "Reset byte array" instruction sets the signal state of the bits in a specified area to "0". You use the pointer on the S_BYTE parameter to define the starting point of the range. You define the size of the range by specifying the number of bits to be reset on the N parameter of the instruction.
...
| N | Input | INT | I, Q, M, D, L, P or constant | Size of the array whose bits are to be reset.  The array is specified by the number of bits as a multiple of 8 (for example 8, 16, 24 etc.). |

#### Example

The following example shows how the instruction works:

![Example](images/43778935819_DV_resource.Stream@PNG-de-DE.png)
...
![Result](images/68035281803_DV_resource.Stream@PNG-de-DE.png)
...
| EN | Input | BOOL | I, Q, M, D, L, T, C | Enable input |
| ENO | Output | BOOL | I, Q, M, D, L | Enable output |
| RESET | Input | BOOL | I, Q, M, D, L | The signal state "1" resets the sequencer. When the instruction is called for the first time, you have to set the value "1" on the RESET parameter.  Constants can also be specified in the RESET parameter. |
...
| OUTj,  0 ≤ j ≤ 15 | Output | BOOL | I, Q, M, D, L | Output bit j |
...
![Example](images/68856130315_DV_resource.Stream@PNG-de-DE.png)
...
| PT | Static | DINT | I, Q, M, D, L | Preset timer value, where one count = 1 ms. |
...
![Example](images/20590951435_DV_resource.Stream@PNG-de-DE.png)
...
| <PT |  |  | ET < PT |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
| >=PT |  |  | ET >= PT |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
| If the input parameters O_HIS and C_HIS both have the signal state "1", they are immediately set to signal state "0". In this case, the last row of the above-named table (X) is valid. Because it is therefore no longer possible to check whether the input parameters O_HIS and C_HIS have the signal state "1", the output parameters are set as follows in this case:   OO = FALSE  CO = FALSE  OA = FALSE  CA = FALSE  ET = PT  Q = TRUE |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
...
| PT | Static | DINT | I, Q, M, D, L | Preset time value, where one clock pulse = 1 ms |
| PREV_TIME | Static | DWORD | I, Q, M, D, L | Previous system time |
...
![Example](images/71825417227_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/20590974731_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/66376763275_DV_resource.Stream@PNG-de-DE.png)

If the operands "TagIn" and "Tag_Start" have the signal state "1", the instruction is started. Each time the signal state changes from "0" to "1" at the TMR_EN input, the cycle time "2" at the DELTA_T input is added to the current time value of InOut ET until the maximum time value "10" of the PV input is reached. If the instruction is executed without errors, the enable output ENO has the signal state "1" and the "TagOut" output is set.
...
![Example](images/71827263755_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/20593564171_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/20594181643_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/41942879243_DV_resource.Stream@PNG-de-DE.png)
...
![Example](images/20590986379_DV_resource.Stream@PNG-de-DE.png)
