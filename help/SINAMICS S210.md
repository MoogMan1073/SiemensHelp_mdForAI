...
- [Using measuring function (from SINAMICS firmware V6.1)](#using-measuring-function-from-sinamics-firmware-v61)
...
### Up to SINAMICS firmware V5.2.x

The following table lists the maximum quantity structure for SINAMICS S210 drives with SINAMICS firmware up to V5.2.x, which can be recorded using the trace and logic analyzer function:

| Device | Maximum number of installed traces | Maximum number of signals per trace |
| --- | --- | --- |
| **SINAMICS S210** | 2 | 8 1...4 signals; 0.0625 ms minimum recording cycle 5...8 signals; 4.0 ms minimum recording cycle |

### From SINAMICS firmware V6.1

The following table lists the maximum quantity structure for SINAMICS S210 drives with SINAMICS firmware from V6.1, which can be recorded using the trace and logic analyzer function:

| Device | Maximum number of installed traces | Maximum number of signals per trace |
| --- | --- | --- |
| **SINAMICS S210** | 2 | 8 1...8 signals; 0.0625 ms minimum recording cycle |
...
The first trace configuration you create for a SINAMICS S210 drive in each case is preassigned a preselection of signals to be recorded. Subsequently created trace configurations do not contain a preselection of signals to be recorded.
...
| - | ![Setting options and displays in "Signals"](images/114060011275_DV_resource.Stream@PNG-de-DE.png) | Button to open the signal selection table  The button is displayed when the table line is selected.  Clicking the icon opens a table which offers possible signals for selection. The selected signal is displayed in the input field. |
...
![Setting options and displays in "Recording conditions"](images/114060042251_DV_resource.Stream@PNG-en-US.png)
...
|  |  | ![Setting options and displays in "Recording conditions"](images/114060034187_DV_resource.Stream@PNG-de-DE.png) | Opens the signal selection table.  Clicking the icon opens a table which offers possible signals for selection as trigger tag. The selected signal is displayed in the input field. |
...
!["Bit pattern"](images/114060094731_DV_resource.Stream@PNG-en-US.png)
...
| Icon | Description |
| --- | --- |
| !["Bit pattern"](images/114060109067_DV_resource.Stream@PNG-de-DE.png) | Bit is not evaluated |
| !["Bit pattern"](images/114060117131_DV_resource.Stream@PNG-de-DE.png) | Bit is checked for FALSE |
| !["Bit pattern"](images/114060125195_DV_resource.Stream@PNG-de-DE.png) | Bit is checked for TRUE |
...
3. Select a signal. The following options are available:
   ...
   - In the "Name" column, click the ![Procedure](images/114060011275_DV_resource.Stream@PNG-de-DE.png) button and select a parameter.
...
2. Select a trigger parameter. The following options are available:
   ...
   - Click the !["Trigger on tag" trigger condition](images/114060011275_DV_resource.Stream@PNG-de-DE.png) button for the trigger parameter and select a parameter.
...
3. Transfer the trace configuration to the device with the !["Trigger on fault” trigger condition](images/88138288395_DV_resource.Stream@PNG-de-DE.png) button.
...
1. Select the "Trigger on alarm" entry in the drop-down list for "Trigger mode".
   ...
   The input fields for the trigger parameters are hidden.
2. Enter the pre-trigger, recording duration and cycle (see above).
3. Transfer the trace configuration to the device with the !["Trigger on alarm" trigger condition](images/88138288395_DV_resource.Stream@PNG-de-DE.png) button.

## Using measuring function (from SINAMICS firmware V6.1)
...
- The SINAMICS drive has a firmware version from FW V6.1.
...
- The SINAMICS S210 drive has a firmware version from V6.1.
- The "Startdrive Advanced" license is available.
