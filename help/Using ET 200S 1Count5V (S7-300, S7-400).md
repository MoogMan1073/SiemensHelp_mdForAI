---
title: "Using ET 200S 1Count5V (S7-300, S7-400)"
package: TFHWC1Count5VenUS
topics: 97
...
# Using ET 200S 1Count5V (S7-300, S7-400)

This section contains information on the following topics:

- [1Count5V terminal assignment (S7-300, S7-400)](#1count5v-terminal-assignment-s7-300-s7-400)
- [Configuring and assigning parameters to 1Count5V (S7-300, S7-400)](#configuring-and-assigning-parameters-to-1count5v-s7-300-s7-400)
- [1Count5V diagnostics (S7-300, S7-400)](#1count5v-diagnostics-s7-300-s7-400)

## 1Count5V terminal assignment (S7-300, S7-400)

### Wiring Requirements

The cables (terminals 1 and 8 and terminals 15 and 16) must be shielded. The shield must be supported at both ends. To do this, use the shield support (see the appendix to the [ET 200S Distributed I/O System](http://support.automation.siemens.com/WW/view/en/1144348) operating instructions).

### Terminal assignment of the 1Count5V

The following table shows the terminal assignment for 1Count5V.

| View | Terminal Assignment | Remarks |
| --- | --- | --- |
| ![Terminal assignment of the 1Count5V](images/88333257483_DV_resource.Stream@PNG-en-US.png) |  | A, /A: Track A  B, /B: Track B  N, /N: Track N  24V: Load voltage  24V DC: Sensor supply  M: Chassis ground  DI: Digital input  DO1: Digital output  DO2: Digital output |

## Configuring and assigning parameters to 1Count5V (S7-300, S7-400)
...
You define 1Count5V functionality by specifying an operating mode.
...
| [Position feedback](#position-feedback-parameters-s7-300-s7-400) | In this mode, 1Count5V counts continuously from the load value:  - If another count pulse is received after 1Count5V reaches the high count limit when counting up, it will jump to the low count limit and continue counting from there without losing a pulse. - If another count pulse is received after 1Count5V reaches the low count limit when counting down, it will jump to the high count limit and continue counting from there without losing a pulse. |
| [Fast mode](#fast-mode-parameters-s7-300-s7-400)   (supported as of firmware version V2.0) | In this mode, 1Count5V counts continuously from the initial value:  - If another count pulse is received after 1Count5V reaches the maximum value that can be represented by 25 bits when counting up (all counter bits are set), the count value will jump to "0" and resume counting from there without losing a pulse. - If 1Count5V reaches the value "0" when counting down and another count pulse is then received, the count value will jump to the maximum value that can be represented by 25 bits (all counter bits set) and resume counting from there without losing a pulse.   This operating mode can be used for position feedback in particularly short isochrone cycles. |
...
- [Selecting signal evaluation (S7-300, S7-400)](#selecting-signal-evaluation-s7-300-s7-400)
- [Activating signal pair diagnostics (S7-300, S7-400)](#activating-signal-pair-diagnostics-s7-300-s7-400)
...
- [Selecting substitute value DOx (S7-300, S7-400)](#selecting-substitute-value-dox-s7-300-s7-400)
- [Activating DOx diagnostics (S7-300, S7-400)](#activating-dox-diagnostics-s7-300-s7-400)
...
You can configure the response of the 1Count5V to failure of the higher-level controller.

| Parameter | Status of the 1Count5V at CPU/Master STOP | What happens when new parameters are assigned? |
| --- | --- | --- |
| Turn off DO | The current mode is cancelled. The gate is closed and the digital output disabled.  Comparison values 1 and 2 the load value are reset. The high and low limit, function and behavior of the digital outputs and the integration time are transferred from the parameters. | The changed parameters are accepted and become effective. |
...
| DO substitute a value | The current mode is cancelled. The gate is closed and the substitute value set for the digital output is activated. Comparison values 1 and 2 and the load value are reset. The high and low limit, function and behavior of the digital outputs and the integration time are transferred from the parameters.  If "Pulse when comparison value is reached" has been set for the output, the only available pulse duration substitute value will be 1. | The changed parameters are accepted and become effective. |
| DO keep last value | The current mode is cancelled. The gate is closed and the status of the digital output is maintained. Comparison values 1 and 2 and the load value are reset. The high and low limit, function and behavior of the digital outputs and the integration time are transferred from the parameters. | The changed parameters are accepted and become effective. |
| <sup>1)</sup>The CPU/Master must not clear the outputs if the mode is to continue working on transition from CPU/Master STOP to RUN (startup).  Possible solution: In the part of the user program that is executed during startup, set the SW_GATE control bit and transfer the values to 1Count5V. |  |  |

##### Exiting parameterized status

Under what conditions does the 1Count5V exit the parameterized status?
...
- When the 1Count5V is inserted
- Upon power on or insertion of the corresponding power module

#### Selecting signal evaluation (S7-300, S7-400)
...
- [Rotary Encoder](Counting%20and%20measuring%20%28S7-300%2C%20S7-400%29.md#overview-s7-300-s7-400)
  ...
  The 1Count5V can count the edges of the signals. Normally, only the edges at A are evaluated (single evaluation). To obtain a higher resolution, you can select whether the signals are to undergo [single](Counting%20and%20measuring%20%28S7-300%2C%20S7-400%29.md#single-evaluation-s7-300-s7-400), [double](Counting%20and%20measuring%20%28S7-300%2C%20S7-400%29.md#double-evaluation-s7-300-s7-400) or [quadruple](Counting%20and%20measuring%20%28S7-300%2C%20S7-400%29.md#quadruple-evaluation-s7-300-s7-400) evaluation when assigning parameters ("Signal evaluation" parameter).

  Multiple evaluation is only possible with asymmetric incremental encoders with A and B signals that are 90 degrees out of phase.

  > **Note**
  >
  > 500 kHz for count frequency is the maximum frequency of the A and B signals. This means a maximum frequency of 1 MHz for the count pulses in double evaluation and a maximum of 2 MHz in quadruple evaluation.

#### Activating signal pair diagnostics (S7-300, S7-400)

##### Signal pair monitoring

Signal pairs A and /A, B and /B, and N and /N can be monitored.

Activate the corresponding check box.

> **Note**
>
> You cannot activate "N, /N diagnostics" until you have activated "A, /A and B, /B diagnostics".
...
The 1Count5V allows you to store two comparison values, which are assigned to the digital outputs. The outputs can be activated in line with counter value and comparison values.

###### Digital outputs

1Count5V has two digital outputs.
...
- The output is set with the positive edge of SET_DO1 or SET_DO2.
  ...
  However, if the "Pulse on Reaching the Comparison Value" function is selected, only one pulse with the specified duration is output. When pulse duration = 0, the output can be set with SET_DO1 or SET_DO2 as long as the counter value is at the comparison value or hysteresis is active.
...
> **Note**
>
> An output set by SET_DO1 or SET_DO2 cannot be reset (by the comparator) at the comparison value.

###### Loading Comparison Values

You transfer the comparison values to the 1Count5V. The counting is not affected by this.
...
You can change the functions and behavior of the outputs during operation using the [control interface](#control-interface-for-counting-modes-s7-300-s7-400). The 1Count5V deletes the outputs and accepts the values as follows:

- **Function of digital outputs DO1 and DO2:** If you change this function so that the comparison condition is satisfied, the output will not be set until after the next count pulse. However, 1Count5V will not make any changes at the output if [hysteresis](#setting-hysteresis-s7-300-s7-400) is enabled.
...
The [status bits](#feedback-interface-for-counting-modes-s7-300-s7-400)STS_CMP1 and STS_CMP2 indicate that the relevant output is or was switched on. These status bits retain their status until they are acknowledged. If the output is still activated, the corresponding bit will be set again immediately. These status bits are also set when the control bit SET_DO1 or SET_DO2 is operated without DO1 or DO2 being enabled.

###### Isochrone mode

Outputs DO1 and DO2 are switched at time T<sub>o</sub> in isochrone mode.

##### On when counter value = comparison value or when counter value = comparison value (S7-300, S7-400)

###### Counter value ≤ comparison value and Counter value ≥ comparison value

If the comparison conditions are fulfilled, the respective comparator will switch on the output. The [status of the output](#feedback-interface-for-counting-modes-s7-300-s7-400) is indicated by STS_DO1 and STS_DO2.
...
In isochrone mode, too, the DO1 and DO2 outputs are switched as soon as the comparison condition is fulfilled and are therefore independent of the bus cycle.
...
If a main count direction is assigned, the comparator will only switch when the comparison value in the main count direction is reached.
...
#### Selecting substitute value DOx (S7-300, S7-400)

##### Substitute values DO1 and DO2

This is where you select the substitute value which is to be output at digital output DO1 or DO2 if you have selected the parameter "DO substitute a value" under "[Reaction to CPU/Master STOP](#selecting-reaction-to-cpumaster-stop-s7-300-s7-400)".

#### Activating DOx diagnostics (S7-300, S7-400)
...
Hysteresis will no longer be active if the counter value is outside the hysteresis range. The comparator switches again in line with its comparison conditions.

![Effect when counter value ≤ comparison value and counter value ≥ comparison value](images/21848390411_DV_resource.Stream@PNG-en-US.png)

> **Note**
>
> If the count direction changes at the comparison value when hysteresis is active, the output will be reset.

##### Effect when comparison value is reached and pulse duration = 0
...
Hysteresis will no longer be active if the counter value is outside the hysteresis range. The comparator deletes the result of the comparison.

![Effect when comparison value is reached and pulse duration = 0](images/21848957707_DV_resource.Stream@PNG-en-US.png)

##### Effect when comparison value is reached, output pulse duration
...
Hysteresis will no longer be active if the counter value is outside the hysteresis range.

When hysteresis is enabled, 1Count5V stores the count direction.

A pulse is output if the hysteresis range is exited in a different direction to the one stored.

![Effect when comparison value is reached, output pulse duration](images/24575776139_DV_resource.Stream@PNG-en-US.png)
...
The pulse duration can be set to suit the actuators used. The pulse duration specifies how long the output is to be set for. The pulse duration can be preselected in increments of 2 ms between 0 ms and 510 ms.

If the pulse duration = 0, the output will be set until the comparison condition is no longer fulfilled. Note that the count pulse times must be greater than the minimum switching times of the digital output.
...
You define 1Count5V functionality by setting a counting mode.

| Operating mode | Description |
| --- | --- |
| [Count continuously](Counting%20and%20measuring%20%28S7-300%2C%20S7-400%29.md#count-continuously-s7-300-s7-400)  (with or without gate) | 1Count5V counts continuously from the current counter value. |
| [Count once](Counting%20and%20measuring%20%28S7-300%2C%20S7-400%29.md#count-once-s7-300-s7-400)   (with hardware or software gate) | When the gate opens, 1Count5V counts from the load value to the count limit. |
| [Count periodically](Counting%20and%20measuring%20%28S7-300%2C%20S7-400%29.md#count-periodically-s7-300-s7-400)   (with hardware or software gate) | When the gate opens, 1Count5V counts from the load value between the count limits. |
...
- [Hardware enable for synchronization (S7-300, S7-400)](#hardware-enable-for-synchronization-s7-300-s7-400)
...
- [Hardware enable for synchronization](#hardware-enable-for-synchronization-s7-300-s7-400)

  The digital input acts as a hardware enable. The encoder zero mark sets the counter to the load value when the hardware enable is active.
...
This function saves the current internal counter value of the 1Count5V and retriggers counting when there is a positive edge at the digital input. This means that the current internal counter value at the time of the positive edge is stored (latch value), and the 1Count5V is then loaded again with the load value, from which counting resumes.
...
It is not changed when the software gate is opened.
...
If the 1Count5V is inserted behind an IM 151 that supports the reading and writing of broader user data interfaces, the current count value can be read from bytes 8 to 11 of the [feedback interface](#feedback-interface-for-counting-modes-s7-300-s7-400).

##### Synchronization with positive edge (S7-300, S7-400)

###### Synchronization

If you have configured "Synchronization with positive edge", the rising edge of a reference signal at the input will set 1Count5V to the load value.

You can choose between once only and periodic synchronization ("Synchronization" parameter).
...
- The counting mode must have been started with the software gate.
- The "Enable synchronization" (CTRL_SYN) [control bit](#control-interface-for-counting-modes-s7-300-s7-400) must be set.
- Synchronize once only: If the enable bit is set, the first edge loads 1Count5V with the load value.
- Synchronization periodically: If the enable bit is set, the first and each subsequent edge loads the 1Count5V with the load value.
- After successful synchronization, the STS_SYN[feedback bit](#feedback-interface-for-counting-modes-s7-300-s7-400) is set and the SYN LED comes on. The RES_STS[control bit](#control-interface-for-counting-modes-s7-300-s7-400) resets the feedback bit and switches off the LED.
...
The counting function is started when the software gate is opened. 1Count5V begins at the load value:
...
##### Hardware enable for synchronization (S7-300, S7-400)

###### Synchronization with DI and Zero Mark

The DI will act as the hardware enable if you have configured "Hardware enable for synchronization". When the HW enable is active, the 1Count5V is loaded with the load value by the zero mark of the encoder.

You can choose between once only and periodic synchronization ("[Synchronization](#selecting-synchronization-s7-300-s7-400)" parameter).

The figure below shows the counter values in once only and periodic synchronization.

![Synchronization with DI and Zero Mark](images/21963185675_DV_resource.Stream@PNG-en-US.png)

The following conditions apply:

- The counting mode must have been started with the software gate.
- The "Enable synchronization" (CTRL_SYN) [control bit](#control-interface-for-counting-modes-s7-300-s7-400) must be set.
- Synchronize once only: If the enable bit is set and the HW enable is present, the first zero mark loads the 1Count5V with the load value.
- Synchronization periodically: If the enable bit is set and the HW enable is present, the first and each subsequent zero mark will load 1Count5V with the load value.
- After successful synchronization, the STS_SYN[feedback bit](#feedback-interface-for-counting-modes-s7-300-s7-400) is set and the SYN LED comes on. The RES_STS[control bit](#control-interface-for-counting-modes-s7-300-s7-400) resets the feedback bit and switches off the LED.
- The signal of a bounce-free switch can serve as the reference signal.
- The [feedback bit](#feedback-interface-for-counting-modes-s7-300-s7-400)STS_DI indicates the level of the reference signal.

In isochrone mode, the set [feedback bit](#feedback-interface-for-counting-modes-s7-300-s7-400)STS_SYN indicates that the positive edge on the digital input was between time T<sub>i</sub> in the current cycle and time T<sub>i</sub> in the previous cycle.
...
This is where you can select either once only or periodic synchronization if you have selected "Synchronization with positive edge" at the "[DI function](#selecting-di-function-s7-300-s7-400)" parameter:

- Once: If the enable bit is set, the first edge loads the 1Count5V with the load value.
- Periodic: If the enable bit is set, the first and each subsequent edge loads 1Count5V with the load value.
...
> **Note**
>
> The following control interface data is consistent for 1Count5V:
>
> - Bytes 0 ... 3
> - Bytes 4 ... 7
...
The table below shows the assignment of the 1Count5V control interface (outputs) for counting modes.

| Address |  | Assignment |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Bytes 0 to 3 |  | **Load value direct, in preparation, comparison value 1 or 2** |  |  |  |  |  |
|  | Byte 0 | **Behavior of DO1, DO2 of**  **1Count5V** |  |  |  |  |  |
| Bit 2 | Bit 1 |  | Bit 0 | Function DO1 |  |  |  |
| 0 | 0 |  | 0 | Output |  |  |  |
| 0 | 0 |  | 1 | On when counter value ≥ comparison value |  |  |  |
| 0 | 1 |  | 0 | On when counter value ≤ comparison value |  |  |  |
...
| 0 |  | 0 | Output |  |  |  |  |
| 0 |  | 1 | On when counter value ≥ comparison value |  |  |  |  |
| 1 |  | 0 | On when counter value ≤ comparison value |  |  |  |  |
...
> **Note**
>
> The following feedback interface data is consistent for 1Count5V:
>
> - Bytes 0 ... 3
> - Bytes 4 ... 7
> - Bytes 8 ... 11 (extended user data interface)
...
The table below shows the assignment of the 1Count5V feedback interface (inputs) for counting modes.

| Address | Assignment |  | Name |
| --- | --- | --- | --- |
| Bytes 0 to 3 | Counter value or stored counter value in the case of the latch function on the digital input |  |  |
...
| Bit | Explanation |
| --- | --- |
| ERR_24V | Encoder supply short circuit  The error bit must be acknowledged by the EXTF_ACK[control bit](#control-interface-for-counting-modes-s7-300-s7-400).  Diagnostic alarm, if parameterized. |
| ERR_DO1 | Short circuit/wire break/overtemperature due to overload at output DO1  The error bit must be acknowledged by the EXTF_ACK[control bit](#control-interface-for-counting-modes-s7-300-s7-400).  Diagnostic alarm, if parameterized. |
| ERR_DO2 | Short circuit/wire break/overtemperature at output DO2  The error bit must be acknowledged by the EXTF_ACK[control bit](#control-interface-for-counting-modes-s7-300-s7-400).  Diagnostic alarm, if parameterized. |
| ERR_ENCODER | Short circuit / wire break in 5 V encoder signal  The error bit must be acknowledged by the EXTF_ACK[control bit](#control-interface-for-counting-modes-s7-300-s7-400).  Diagnostic alarm, if parameterized. |
...
| STS_DO2 | DO2 status  The STS_DO2 status bit indicates the status of the DO2 digital output. |
...
- [Activating signal pair diagnostics (S7-300, S7-400)](#activating-signal-pair-diagnostics-s7-300-s7-400-1)
...
- [Selecting substitute value DOx (S7-300, S7-400)](#selecting-substitute-value-dox-s7-300-s7-400-1)
- [Activating DOx diagnostics (S7-300, S7-400)](#activating-dox-diagnostics-s7-300-s7-400-1)
...
The 1Count5V counts each positive edge of a pulse and assigns it a time value in µs.
...
1Count5V measures continuously. When assigning parameters, you specify an update time.
...
In isochrone mode, the 1Count5V accepts control bits and control values from the control interface in each bus cycle and reports back the response in the same cycle.

The 1Count5V transfers a measured value and the status bits that were valid at time T<sub>i</sub> in each cycle.
...
>
> Violation of the value range limits will result in a parameter assignment error and 1Count5V will not go into isochrone mode.
...
Signal pairs A and /A, and B and /B can be monitored.
...
You can assign parameters for the digital output of the 1Count5V.
...
If you want to switch the output on or off, you must enable it with the CTRL_DO1 or CTRL_DO2[control bit](#control-interface-for-measuring-modes-s7-300-s7-400).

You can switch the outputs on and off with the [control bits](#control-interface-for-measuring-modes-s7-300-s7-400)SET_DO1 and SET_DO2.

You can query the status of the output with the STS_DO1 or STS_DO2[status bit](#feedback-interface-for-measuring-modes-s7-300-s7-400) in the feedback interface.

The outputs are switched at time T<sub>o</sub> in isochronous mode.
...
This is where you select the substitute value which is to be output at digital output DO1 or DO2 if you have selected the parameter "DO substitute a value" under "[Reaction to CPU/Master STOP](#selecting-reaction-to-cpumaster-stop-s7-300-s7-400-1)".
...
You define 1Count5V functionality by specifying a measuring mode.

|  | Description |  |
| --- | --- | --- |
| Operating mode | Measurement with integration time | Continuous measurement |
| [Frequency measurement](Counting%20and%20measuring%20%28S7-300%2C%20S7-400%29.md#frequency-measurement-s7-300-s7-400)  (with hardware or software gate) | 1Count5V counts the pulses that occur during a specified integration time. | 1Count5V counts the pulses that occur during a dynamic measurement time. |
| [Rotational speed measurement](Counting%20and%20measuring%20%28S7-300%2C%20S7-400%29.md#rotational-speed-measurement-s7-300-s7-400)  (with hardware or software gate) | 1Count5V counts the pulses received from a tachometer generator within a specified integration time and calculates the speed on the basis of this value and the number of pulses per encoder revolution. | 1Count5V counts the pulses received from a tachometer generator within a dynamic measuring time and calculates the speed on the basis of this value and the number of pulses per encoder revolution. |
| [Period measurement](Counting%20and%20measuring%20%28S7-300%2C%20S7-400%29.md#period-measurement-s7-300-s7-400)  (with hardware or software gate) | 1Count5V measures the time between two positive edges of the count signal by counting the pulses of an internal quartz accurate reference frequency (16 MHz) over a specified integration time. | 1Count5V indicates the dynamic measuring time as a period. The mean value of the period is calculated if its duration is less than the update time. |
...
The table below shows the assignment of the 1Count5V control interface (outputs) for measuring modes.
...
The table below shows the assignment of the 1Count5V feedback interface (inputs) for measuring modes.
...
| ERR_DO2 | Short circuit/wire break/overtemperature at output DO2  The error bit must be acknowledged with the [control bit](#control-interface-for-measuring-modes-s7-300-s7-400)EXTF_ACK. Diagnostic alarm if parameterized. |
| ERR_ENCODER | Short circuit / wire break / encoder signal  The error bit must be acknowledged with the [control bit](#control-interface-for-measuring-modes-s7-300-s7-400)EXTF_ACK Diagnostic alarm if parameterized. |
...
- [Selecting signal evaluation (S7-300, S7-400)](#selecting-signal-evaluation-s7-300-s7-400-1)
- [Activating signal pair diagnostics (S7-300, S7-400)](#activating-signal-pair-diagnostics-s7-300-s7-400-2)
...
- [Hardware enable for synchronization (S7-300, S7-400)](#hardware-enable-for-synchronization-s7-300-s7-400-1)
...
- [Latching with positive edge](#latching-with-positive-edge-s7-300-s7-400-1)
  ...
  Counter values are saved (latched) with positive edges at the digital input.
- [Hardware enable for synchronization](#hardware-enable-for-synchronization-s7-300-s7-400-1)
...
The latch value is preassigned its RESET status ("0"). It is not changed when the SW gate is opened.
...
If the 1Count5V is inserted behind an IM 151 that supports the reading and writing of broader user data interfaces, the current count value can be read from bytes 8 to 11 of the [feedback interface](#position-feedback-feedback-interface-s7-300-s7-400).
...
- Synchronize once only: If the enable bit is set, the first edge loads 1Count5V with the load value.
- Synchronization periodically: If the enable bit is set, the first and each subsequent edge loads the 1Count5V with the load value.
- After successful synchronization, the STS_SYN[feedback bit](#position-feedback-feedback-interface-s7-300-s7-400) is set and the SYN LED comes on. The RES_STS[control bit](#position-feedback-control-interface-s7-300-s7-400) resets the feedback bit and switches off the LED.
- The signal of a bounce-free switch can serve as the reference signal.
...
The counting function is started when the SW gate is opened. 1Count5V begins at the load value ("0").
...
You can choose between once only and periodic synchronization ("[Synchronization](#selecting-synchronization-s7-300-s7-400-1)" parameter).
...
In isochrone mode, the set [feedback bit](#position-feedback-feedback-interface-s7-300-s7-400)STS_SYN indicates that the positive edge on the digital input was between time T<sub>i</sub> in the current cycle and time T<sub>i</sub> in the previous cycle.
...
This is where you can select either once only or periodic synchronization if you have selected "Synchronization with positive edge" at the "[DI function](#selecting-di-function-s7-300-s7-400-2)" parameter:
...
The table below shows the assignment of the 1Count5V control interface (outputs) for position feedback.
...
The table below shows the assignment of the 1Count5V feedback interface (inputs) for position feedback.
...
| ERR_LOAD | Load function error  The LOAD_VAL and LOAD_PREPARE[control bits](#position-feedback-control-interface-s7-300-s7-400) cannot be set simultaneously during transfer. As when you load an incorrect value (which is not accepted), this results in the ERR_LOAD status bit being set. |
| ERR_PARA | Errors in assigned module parameters. |
| RES_STS_A | Resetting of status bits active |
| STS_C_DN | Status down |
| STS_C_UP | Status up |
...
| STS_GATE | Internal gate status: Counting. |
| STS_LOAD | Load function is running |
| STS_ND | Zero crossing in the count range during counting without a main count direction. The bit must be reset by the [control bit](#position-feedback-control-interface-s7-300-s7-400)RES_STS. |
| STS_OFLW  STS_UFLW | High count limit violated  Low count limit violated  Both bits must be reset. |
...
- [Selecting signal evaluation (S7-300, S7-400)](#selecting-signal-evaluation-s7-300-s7-400-2)
- [Activating signal pair diagnostics (S7-300, S7-400)](#activating-signal-pair-diagnostics-s7-300-s7-400-3)
...
- [Hardware enable for synchronization (S7-300, S7-400)](#hardware-enable-for-synchronization-s7-300-s7-400-2)
...
- [Synchronization with positive edge](#synchronization-with-positive-edge-s7-300-s7-400-2)
  ...
  The counter is set to the load value with positive edges at the digital input.
- [Hardware enable for synchronization](#hardware-enable-for-synchronization-s7-300-s7-400-2)
...
If you have configured "Synchronization with positive edge", the positive edge of a reference signal at the input will set 1Count5V to the initial value (= load value).
...
- "Fast mode" must be active (hardware gate).
  ...
  - When synchronization is activated, the first edge and each additional edge loads 1Count5V with the initial value (=load value).
...
The DI will act as the hardware enable if you have configured "Hardware enable for synchronization". When the HW enable is active, 1Count5V is loaded with the initial value (load value) by the zero mark of the encoder.

The figure below shows the counter status following hardware enable at the DI and synchronization with a positive edge.

![Synchronization with DI and Zero Mark](images/21986990347_DV_resource.Stream@PNG-en-US.png)

The following conditions apply:

- The counting mode must have been started with the SW gate.
- The "Enable synchronization" (CTRL_SYN) control bit must be set.
- After successful synchronization, the STS_SYN[feedback bit](#fast-mode-feedback-interface-s7-300-s7-400) is set and the SYN LED comes on. The RES_STS control bit resets the feedback bit and switches the LED off.
- The signal of a bounce-free switch can serve as the reference signal.
- The [feedback bit](#fast-mode-feedback-interface-s7-300-s7-400)STS_DI indicates the level of the reference signal.

In isochronous mode, the set STS_SYN feedback bit indicates that the positive edge on the digital input was between time T<sub>i</sub> in the current cycle and time T<sub>i</sub> in the previous cycle.
...
You can specify a load value for 1Count5V.
...
The table below shows the assignment of the 1Count5V feedback interface (inputs) for fast mode.
...
|  | Bit 28 | Group error  - Encoder supply short circuit - Short circuit / wire break / encoder signal | EXTF |
...
| --- | --- |
| ERR_PARA | Errors in assigned module parameters. |
| EXTF | Group error  Possible causes:  - Encoder supply short circuit - Short circuit or encoder signal wire break   EXTF is reset when the causes of the errors have been eliminated. |
...
## 1Count5V diagnostics (S7-300, S7-400)
...
#### LED display on the 1Count5V

![LED display on the 1Count5V](images/22616891019_DV_resource.Stream@PNG-de-DE.png)
...
#### Status and error displays by means of LEDs on the 1Count5V

The table below shows the status and fault displays on the 1Count5V.
...
|  |  |  | On |  |  |  | Synchronization is performed (only in counting modes; image of STS_SYN acknowledgement bit). |  |
|  |  |  |  | On |  |  | DO 1 is activated. |  |
|  |  |  |  |  | On |  | DO 2 is activated. |  |
|  |  |  |  |  |  | On | DI is activated. |  |
...
#### 1Count5V error types

The following table shows the 1Count5V error types.
...
| 26<sub>D</sub> | 11010:  External error | Wire break/short circuit of the 5 V sensor signals:  A, /A, B, /B, N, /N, | Correct the parameter assignment. |
