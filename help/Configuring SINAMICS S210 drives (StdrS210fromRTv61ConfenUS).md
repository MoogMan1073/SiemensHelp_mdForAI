...
package: StdrS210fromRTv61ConfenUS
topics: 113
...
- [Overview of SINAMICS S210](#overview-of-sinamics-s210)

### Overview of SINAMICS S210

The drive system comprises a converter and a motor. Both devices are optimally coordinated with one another, and are intended for use with a higher-level control system (PLC). PROFINET is used to establish the connection to the control system.

An example of a S210 drive system with the connected components is provided below:

![Example: S210_3AC](images/159551773835_DV_resource.Stream@PNG-en-US.png)
...
- [Rotate &amp; optimize S210](#rotate-optimize-s210)
- [S210 guided quick startup](#s210-guided-quick-startup)

### S210 hardware catalog

This section contains information on the following topics:

- [Power variants S210](#power-variants-s210)
- [Available components](#available-components)

#### Power variants S210

##### Power variants and components

In the hardware catalog, the available SINAMICS S210 power variants are stored as follows:

![SINAMICS S210 hardware catalog](images/172103713675_DV_resource.Stream@PNG-en-US.png)

| Symbol | Meaning |
| --- | --- |
| ① | SINAMICS S210 converter  Available power variants of the SINAMICS S210 type |
| ② | Motors |
| ③ | Measuring systems  These can be assigned to the motors. A maximum of 2 encoders can be inserted. |

SINAMICS S210 hardware catalog

#### Available components

In Startdrive all available hardware components can be configured in the function view or in the parameter view. Below is a list of the components that can be used for the SINAMICS S210 in a Startdrive project.

Which hardware components can be used?

| Component / drive system | S210 |
| --- | --- |
| **Control Unit**  <sup>1)</sup> |  |
| S210 PN (from SINAMICS FW V6.1) | X |
| **Motors** |  |
| 1FK2 synchronous motor | X |
| 1FS2 synchronous motor | X |
| 1FT2 synchronous motor | X |
| **Measuring systems** |  |
| DRIVE-CLiQ encoder <sup>2)</sup> | X |
| <sup>1)</sup> Integrated in the drive. Not separately selectable in the device configuration and via the hardware selection. Is indirectly determined by selecting the S210 type in the "Add new device" dialog.    <sup>2)</sup> Depending on the motor type, different singleturn and multiturn encoders can be assigned using the inspector window. The first encoder is managed with the motor. A second encoder can be added subsequently. |  |
...
![S210 device view](images/171642965899_DV_resource.Stream@PNG-en-US.png)
...
![S210 network view](images/160258949003_DV_resource.Stream@PNG-en-US.png)
...
![S210 topology view](images/160258634123_DV_resource.Stream@PNG-en-US.png)
...
> **All parameters**
>
> You can find all drive parameters in the "[Parameter view](#s210-parameter-view)". Experts can then comprehensively parameterize the drive. However, the parameter view can only be called for the parameter assignment. The parameter view is not available for commissioning optimization.

#### Structure

The following figure shows an example of a function view structure:

![S210 function view](images/171655496715_DV_resource.Stream@PNG-en-US.png)
...
| ⑦ | Drop-down list: Permits the selection of a specified setting |
...
The "Parameter view" provides a clearly organized display of the parameters available for the device. For the "Parameterization", you can switch the working area between the "[Function view](#s210-function-view)" and the "Parameter view".
...
![S210 parameter view](images/160104350731_DV_resource.Stream@PNG-en-US.png)
...
![Example: S210 inspector window](images/160168068619_DV_resource.Stream@PNG-en-US.png)

| Symbol | Meaning |
| --- | --- |
| ① | Secondary tab: In this example, the main "Properties" tab  Different tabs are displayed depending on the marked component. |
...
>
> If the "Positioning" operating mode is set, selected EPOS functions can be tested via the control panel (see [Testing EPOS settings with the control panel](Commissioning%20SINAMICS%20S210%20drives%20%28StdrS210fromRTv61CommenUS%29.md#testing-epos-settings-with-the-control-panel)).
...
### S210 guided quick startup

#### Overview

The "guided quick startup" is a commissioning wizard. You can use this to make the most important basic settings of a S210 drive.
...
![Example: Guided quick startup in offline mode](images/171650876555_DV_resource.Stream@PNG-en-US.png)
...
- [Use of data sets](#use-of-data-sets)
...
[Example: Function rights required for specific activities](User%20administration%20and%20security.md#example-function-rights-required-for-specific-activities)
...
A firmware update is required if you want to use a new firmware version with an extended range of functions. Updating the firmware using a memory card is explained in detail in the following.

The available SINAMICS firmware versions are listed on this [Internet page](https://support.industry.siemens.com/cs/us/en/view/109812303):
...
- You have an SD card with the appropriate firmware, e.g. 6SL3570-0GB00-0AA0.
  ...
  You have an empty SD card with a maximum of 32 GB memory capacity (e.g. 6SL5970-0AA00-0AA0) onto which you can load the firmware.
- With protection activated (UMAC):
...
The following maximum number of data sets is possible with SINAMICS S210:

- 1 CDS (command data set)

  A number of signal interconnection parameters (signal sinks) that are responsible for the control are combined in this command data set.
- 1 DDS (drive data set)

  The drive data set contains various adjustable parameters that are relevant for open-loop and closed-loop drive control.
- 2 EDS (encoder data sets)

  The encoder data set contains various adjustable parameters of the connected encoder which are relevant for configuring the drive, such as the encoder type.
- 1 MDS (motor data set)

  A motor data set contains various adjustable parameters of the connected motor which are relevant when configuring the drive.

##### Schematic diagram

![Maximum permissible number of data sets](images/171270612491_DV_resource.Stream@PNG-en-US.png)

Maximum permissible number of data sets

##### Rules

- Both encoder data sets are permanently linked to parameters and cannot be interchanged.

  - EDS 0 = p0187
  - EDS 1 = p0188
- The EDS 0 encoder data set is always defined as a motor encoder.
- The drive data set DDS can consist of 2 encoder data sets EDS 0 and EDS 1. However, the encoder data sets cannot be interchanged.
- A data set switchover is not possible.
...
> - A "Security Wizard" usually appears when creating new drives in the project. Using this wizard, you can already make the most important security settings for this drive within the project when creating the drive.
...
3. [Make basic settings offline via the guided quick startup](Commissioning%20SINAMICS%20S210%20drives%20%28StdrS210fromRTv61CommenUS%29.md#guided-quick-startup)
...
6. [Perform optimization](Commissioning%20SINAMICS%20S210%20drives%20%28StdrS210fromRTv61CommenUS%29.md#rotate-optimize-1)
...
5. Make basic settings online via the guided quick startup in editing mode
   ...
   - [Make basic settings in quick startup screen forms](Commissioning%20SINAMICS%20S210%20drives%20%28StdrS210fromRTv61CommenUS%29.md#guided-quick-startup)
   - [Perform optimization](Commissioning%20SINAMICS%20S210%20drives%20%28StdrS210fromRTv61CommenUS%29.md#rotate-optimize)
...
2. Create the device configuration in Startdrive offline
   ...
   - [Insert the SINAMICS drive into the project](#creating-a-device-configuration-manually-offline)
   - Optional: Making security settings for project, drive and controller
   - [Configure motor and encoder](#motor-and-measuring-system)
...
5. Make basic settings online via the guided quick startup in editing mode
   ...
   - [Make basic settings in quick startup screen forms](Commissioning%20SINAMICS%20S210%20drives%20%28StdrS210fromRTv61CommenUS%29.md#guided-quick-startup)
   - [Perform optimization](Commissioning%20SINAMICS%20S210%20drives%20%28StdrS210fromRTv61CommenUS%29.md#rotate-optimize-1)
...
- [Optional: Uploading a device configuration as new station](#optional-uploading-a-device-configuration-as-new-station)
...
For more information on project protection, see the "[User administration and security](User%20administration%20and%20security.md#user-management-and-access-control-umac)" page.
...
You can add new drives either in the project view (see below) or in the portal view. For SINAMICS drives from SINAMICS Firmware V6.1, you can already define the security settings to access the drive data when creating the drive.
...
2. Click "Drives" to display the available drives.
   ...
   ![Selecting a drive](images/160253908491_DV_resource.Stream@PNG-en-US.PNG)

   ![Selecting a drive](images/160253908491_DV_resource.Stream@PNG-en-US.PNG)

   Selecting a drive
3. Select the "SINAMICS S210" folder in the path "SINAMICS drives/SINAMICS S".
...
If the version numbers (offline project in Startdrive compared to the drive) do not match, it will not be possible to go online later. When creating, the current firmware version is always suggested. If required, you can change the version number using the "Version" drop-down list.
...
In the device configuration and project tree of an S210 drive, you can replace the current device with a device at any time:
...
1. In the project tree, select the drive to be replaced. Open shortcut menu "Change device".
   ...
   Dialog "Change device - S210 PN" opens:

   ![Changing the device using the project tree](images/165078299915_DV_resource.Stream@PNG-en-US.png)
...
1. Select the new S210 drive in the hardware catalog. In the device view, drag the drive to the placeholder for the current drive.

   Dialog "Change device - S210 PN" opens:

   ![Changing the device using the hardware catalog](images/165078511371_DV_resource.Stream@PNG-en-US.png)
...
- [Adding and specifying an encoder](#adding-and-specifying-an-encoder)
- [Make detailed settings of the measuring system](#make-detailed-settings-of-the-measuring-system)
...
> - Drag the unspecified "1Fxx synchronous motor" to the lower area of the S210 drive.  
>   A motor placeholder is displayed.
...
2. If required, select the "Motor - Selection 1Fxx" entry in the secondary navigation in the Inspector window.
3. In the list, select the motor power rating and an associated encoder based on the article number. Then specify whether the motor should be equipped with a holding brake - or not.

   ![Specifying the motor](images/160254233739_DV_resource.Stream@PNG-en-US.PNG)
...
![Motor specified](images/173314639499_DV_resource.Stream@PNG-en-US.png)

Motor specified

The motor has been added. The assigned encoder and the encoder evaluation are automatically also added and specified. If required, you can subsequently add a second encoder.
...
When creating an S210 drive in the device configuration, a motor placeholder is always used for motor type "1FK2". If you want to use another motor type (e.g. "1FS2") from the motor list, you can replace the motor placeholder before specifying.

###### Requirements

- The drive is offline.
- A project has been created.
- An S210 Control Unit with an unspecified motor placeholder is inserted into the device configuration.
- With protection activated (UMAC):
...
1. Select the required motor type in the hardware catalog. Drag the corresponding placeholder in the device view onto the placeholder for the currently used motor type.
   ...
   ![Replacing the motor placeholder using hardware selection](images/165079599499_DV_resource.Stream@PNG-en-US.png)
...
1. Select the motor placeholder of the drive in the device configuration. Open shortcut menu "Change device".
   ...
   ![Manually changing the motor placeholder](images/165079759755_DV_resource.Stream@PNG-en-US.png)
...
##### Adding and specifying an encoder

###### Overview

A DRIVE-CLiQ encoder is always automatically inserted and specified with the motor placeholder. You can insert and specify a second encoder in parallel to the first encoder. Only X101 is available as a free DRIVE-CLiQ interface on the Control Unit.

###### Requirements

- A project has been created.
- A Control Unit has been inserted in the device configuration.
- A motor has been inserted.
- With protection activated (UMAC):
...
1. Open the "Measuring systems" entry in the hardware catalog.
2. Drag the unspecified "DRIVE-CLiQ encoder" into the light gray frame area of the Control Unit in the device view. An encoder and a Sensor Module are created.

   ![Inserting an encoder S210](images/173317969035_DV_resource.Stream@PNG-en-US.png)

   ![Inserting an encoder S210](images/173317969035_DV_resource.Stream@PNG-en-US.png)

   Inserting an encoder S210

   When the encoder placeholder is inserted, a DRIVE-CLiQ connection to a free DQ interface of the Control Unit is automatically established (X101).
3. Click the unspecified encoder in the device view.

   ![Specifying the encoder](images/173315530379_DV_resource.Stream@PNG-en-US.PNG)
...
5. Select the desired DRIVE-CLiQ encoder in the measuring system list based on the article number.

   The data of the selected encoder is assigned to the encoder placeholder. The white area turns gray.

   Furthermore, a Sensor Module (encoder evaluation) is inserted and automatically specified. For each DRIVE-CLiQ encoder type there is always only one specific Sensor Module.

###### Result

The second encoder has been inserted and specified. An encoder data set (EDS) is automatically created for each of the two available encoders, which are assigned to a drive data set, but cannot be exchanged (see [Use of data sets](#use-of-data-sets)).

##### Make detailed settings of the measuring system

###### Overview

When commissioning, for measuring systems, you can view various encoder details and in some instances configure. The settings differ depending on the encoder type.

###### Requirements

- A project has been created.
- The Control Unit has been inserted in the device configuration.
- With protection activated (UMAC):
...
3. To configure the detail configuration, click on icon ![Procedure](images/145963165963_DV_resource.Stream@PNG-de-DE.png) next to the "Basic parameterization:" entry.

   The "Basic parameterization" function view opens in the function view.

   Make the required settings here (see "[Basic parameterization](Commissioning%20SINAMICS%20S210%20drives%20%28StdrS210fromRTv61CommenUS%29.md#basic-parameterization)").
...
### Optional: Uploading a device configuration as new station

#### Overview

For all control modules, using the "Upload device as new station" function, you have the option of reading out the device configuration of a converter and to create as station (drive) in your current Startdrive project.

#### Requirements

- A project has been created, or an existing project is open.
- There is a physical LAN connection between the drive and the operating unit. The drive is switched on.
- With protection activated (UMAC):

  Your user account has the corresponding function rights (see [Access control](User%20administration%20and%20security.md#access-control)). To access a drive online, you need both engineering and runtime function rights.
...
The project data has been loaded as a new drive into your Startdrive project and created in the project tree.
...
The procedure for "Going online" is subsequently explained based on the example of an S210 Control Unit. The procedure is identical for the other Control Unit.
...
![S210 PN interfaces](images/163220805515_DV_resource.Stream@PNG-en-US.png)
...
- Activate project protection and create the user account with the required access rights to the drive (see [User administration and security](User%20administration%20and%20security.md#user-management-and-access-control-umac)).
...
- Safety Integrated Function status
  ...
  For detailed information on these functions, refer to the "[Function status](Commissioning%20SINAMICS%20S210%20drives%20%28StdrS210fromRTv61CommenUS%29.md#function-status)" section.
- PROFINET interface (X150)
...
As standard, the components and interconnections of PROFIdrive telegrams in the receive direction for the converter are displayed in this function view (e.g. 105, 700, or 750).

You can use the telegram configuration (![Description](images/148900571915_DV_resource.Stream@PNG-de-DE.PNG)) to add additional telegrams (see table). The content of this selected telegram is then displayed in the "PROFIsafe" or "Supplementary data" area.

###### Telegram structure (S210)
...
| PROFIdrive (speed control)  3, 4, 5, 6, 102, 103, 105, 106  For EPOS replaced by:   7, 9, 111, 112 | X | X | X | X |
| PROFIsafe  30, 901, (902) | X | X | X | X |
...
As standard, the components and interconnections of PROFIdrive telegrams in the send direction for the converter are displayed in this function view (e.g. 105, 700, or 750).
...
3. Call up the "Power unit fan" function view again in the Startdrive project.
   ...
   The wear of the replaced fan is still displayed in the "Wear meter" field ([r0277](SINAMICS%20Parameter%20S210.md#r02770-power-unit-heat-sink-fan-wear-counter)).
...
- [Assigning an IP address](#assigning-an-ip-address-1)
- [Updating the firmware online](#update-the-firmware-from-a-memory-card)
...
>
> - If the device already has an IP address, run function "[Restore factory settings](#restoring-factory-settings)" to reset the IP address to 0.0.0.0.
