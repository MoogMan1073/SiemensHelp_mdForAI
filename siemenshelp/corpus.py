...
def _unique_name(base: str, pack: str, used: set) -> str:
    name = base or pack
    if name not in used:
        return name
...
def convert_corpus(root: str, out_dir: str, *, progress=None, keep_images: bool = True) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="siemens_corpus_") as tmp:
        ...
        for src in sources:
            ...
            name = _unique_name(_safe_filename(scan.root_title), scan.pack, used_names)
            used_names.add(name)
        ...
        # Pass 2 — render each pack with the global resolver.
        results = []
